...
namespace Fask.Extension
{
    public static class FASK_Events_Extension
    {
            ...
            public static FASK_Events DataSetToBO(this Vyroba_konzola.FASK_EventsDataTable dt)
            {
                try
                {
                FASK_Events listObjektu_FZ = new FASK_Events(); //vytvoreni tabulky
                    List<FASK_Events_row> rowList = new List<FASK_Events_row>(); //vytvoreni docasne tabulky

                    foreach (var item in dt)
                    {
                    FASK_Events_row row = new FASK_Events_row(); //docasny radek
            ...
            public static Vyroba_konzola.FASK_EventsDataTable BOToDataSet(this FASK_Events bo)
            {

                try
                {
                Vyroba_konzola.FASK_EventsDataTable dt = new Vyroba_konzola.FASK_EventsDataTable();

                    foreach (var item in bo.rows.ToList<FASK_Events_row>())
                    {
