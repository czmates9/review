...
namespace Fask.Columns
{
	public  class TypDokladu
	{
		...
		private  string TableName = "CZMST092";
		public  Dictionary<string, ColumnType> ColumnsInfo_CZMST092 = new Dictionary<string, ColumnType>();

		#region c'tor

		public TypDokladu(DS_Information ds)
		{

            #region z XML souboru

            EnumerableRowCollection<Fask.Columns.CreateSQL.DS_Information.SchemaColumnsRow> DT_typDoklad  = ds.SchemaColumns.Where(x => x.TABLE_NAME.Trim() == TableName.Trim());

            foreach (var dr in DT_typDoklad)
            {
