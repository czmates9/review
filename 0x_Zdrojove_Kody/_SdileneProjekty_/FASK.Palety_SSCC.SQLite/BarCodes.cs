...
namespace FASK.Palety_SSCC.SQLite
{
    public class BarCodes
    {
        ...
        /// <summary>
        /// Vypocet kontrolni cislice pro GTIN-8, GTIN-12, GTIN-13, GTIN-14, GSIN(17znaku), SSCC(18znaku)
        /// http://www.gs1.org/how-calculate-check-digit-manually
        /// </summary>
        /// <param name="codewithoutcheckdigit">kod bez checkdigit a pripadny prefixu ...</param>
        /// <returns></returns>
        public static string CalculateCheckDigit(string codewithoutcheckdigit)
        {
            int sumLiche = 0;
            int sumSude = 0;
            ...
            // 2-19 : cisla
            // 20 : kontrolni cislo
            for (int i = codewithoutcheckdigit.Length - 1; i >= 0; i--)
            {

                //if ((i + 1) % 2 == 0) //Sude poradove cislo
                if (((codewithoutcheckdigit.Length - i) % 2) == 0)
                    sumSude += int.Parse(codewithoutcheckdigit[i].ToString());
                else //je liche poradove cislo
                    sumLiche += int.Parse(codewithoutcheckdigit[i].ToString());
            }

            //return ((10 - (sumLiche * 3 + sumSude) % 10) % 10).ToString();
            return (10 - (sumLiche * 3 + sumSude) % 10).ToString().Substring(0, 1);
        }
    }
}
