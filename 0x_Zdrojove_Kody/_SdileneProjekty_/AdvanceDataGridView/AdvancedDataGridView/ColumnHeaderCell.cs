...
namespace Zuby.ADGV
{
    ...
    internal class ColumnHeaderCell : DataGridViewColumnHeaderCell
    {
        ...
        #region 25.9.2025 MaR chybovost na datasetu kdyz byl neco jineho
        //public Type columnHeaderType(DataGridViewColumnHeaderCell column)
        //{
        //    try
        //    {
        //        var type = column.DataGridView.DataSource;

        //        BindingSource bs = column.DataGridView.DataSource as BindingSource;
        //        System.Data.DataSet ds = bs.DataSource as System.Data.DataSet;

        //        string datamember = bs.DataMember;
        //        System.Data.DataTable dt = ds.Tables[datamember];

        //        string datapropertyname = column.OwningColumn.DataPropertyName;
        //        Type typ = dt.Columns[datapropertyname].DataType;

        //        return typ;

        //        //return (((System.Data.DataSet)((BindingSource)(column.DataGridView.DataSource)).DataSource).Tables[((BindingSource)(column.DataGridView.DataSource)).DataMember].Columns[column.OwningColumn.DataPropertyName].DataType);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        /// <summary>
        /// Rozsirena metoda pro typ hlavicek sloupcu, oprava od predchozi z 25.9.2025
        /// </summary>
        ...
        public Type columnHeaderType(DataGridViewColumnHeaderCell column)
        {
            ...
            {
                BindingSource bs = column.DataGridView.DataSource as BindingSource;
                if (bs == null) return typeof(string);

                DataTable dt = null;

                if (bs.DataSource is DataSet ds)
                {
                    dt = ds.Tables[bs.DataMember];
                }
                else if (bs.DataSource is DataTable tbl)
                {
                    dt = tbl;
                }

                if (dt == null) return typeof(string);

                string datapropertyname = column.OwningColumn.DataPropertyName;
                return dt.Columns[datapropertyname].DataType;
            }
            catch
            {
                return typeof(string);
        ...
        public ColumnHeaderCell(DataGridViewColumnHeaderCell oldCell, bool filterEnabled)
        {
            ...
            else
            {
                ...
                MenuStrip = new MenuStrip(columnHeaderType(oldCell));
                MenuStrip.FilterChanged += new EventHandler(MenuStrip_FilterChanged);
                MenuStrip.SortChanged += new EventHandler(MenuStrip_SortChanged);
            }
            ...
            DataGridViewPaintParts paintParts)
        {
            ...
            //    return;
            if (this.columnHeaderType(this) == typeof(System.Drawing.Bitmap))
                return;
        ...
        protected override void OnMouseUp(DataGridViewCellMouseEventArgs e)
        {
            ...
            sw.Stop();
            var time = sw.Elapsed;
            System.Diagnostics.Debug.WriteLine(string.Format("MouseUp Column index : '{0}'. čas : {1}:{2}:{3}.{4}", e.ColumnIndex , time.Hours, time.Minutes, time.Seconds, time.Milliseconds));
#endif
        }
