...
namespace PrintReportLibrary
{
    ...
    public partial class CoolPrintPreviewDialog 
    {
        ...
        /// <summary>
        /// Podpora více datových sad pro RDLC report
        /// </summary>
        ...
        protected bool InitPrint() 
        {
            ...
            {
                throw new ApplicationException("ReportPath není nastavený – report definition chybí.");
            }
            bool existuje = File.Exists(report.ReportPath);
            if (!File.Exists(report.ReportPath))
            {
                throw new ApplicationException($"RDLC soubor neexistuje na cestě: {report.ReportPath}");
            }
