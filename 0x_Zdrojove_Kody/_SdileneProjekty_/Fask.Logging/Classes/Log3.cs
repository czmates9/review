...
namespace Fask.Logging
{
	internal class Log3 : ILog2
	{
		...
		private string FileName_NotImplemented = "Log_NotImplemented.txt";
		...
		public bool EnablePrint { get { return _enablePrint; } set { _enablePrint = value; } }
        ...
        public bool write(LogLevel level, string Message)
		{
			...
			switch (level)
			{
				...
				case LogLevel.NotImplemented:
					return SaveToFile(Message, FileName_NotImplemented);
			}
		...
		public bool writeToFile(string message, string FileName, string FileExtension)
		{
            try
            {
                return SaveToFile(message, FileName + "." + FileExtension);
            }
            catch
            {
		...
		public bool write(System.Data.DataTable table)
		{
			try
			{
				if (table.HasErrors)
				{
					...
					msg = "Ma chybu tato tabulka: " + table.HasErrors.ToString() + Environment.NewLine;
					msg += "Jmeno tabulky:" + table.TableName + Environment.NewLine;
					if (table.HasErrors)
					{
						foreach (System.Data.DataRow row in table.Rows)
						{
							if (row.HasErrors)
							{
								msg += "\t " + "Chybny radek: " + row.RowError + Environment.NewLine;

								foreach (System.Data.DataColumn coll in row.GetColumnsInError())
								{
										msg += "\t " + "\t "+ "Chybny sloupec: " + coll.ColumnName + Environment.NewLine;
								}
							}
						}						
					}
		...
		public void WriteScannerCSV(int message, string ean)
		{
			if (!_enablePrint)
				return ;
			...
			try
			{
				sw = new System.IO.StreamWriter(System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), "Scanner.csv"), true);
		...
		public void WriteDataStoreCSV(string login, decimal qty, decimal qtyreal, string barcode)
		{
			...
			try
			{
				sw = new System.IO.StreamWriter(System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), "DataStore.csv"), true);
			...
			try
			{
				switch (level)
				{
					...
					case LogLevel.Licence:
						...
						break;
					case LogLevel.NotImplemented:
						DeleteFile(FileName_NotImplemented);
						break;
					default:
						break;
				}
