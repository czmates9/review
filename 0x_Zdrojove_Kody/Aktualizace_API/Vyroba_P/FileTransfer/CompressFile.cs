...
namespace Fask.Aktualizace_API.FileTransfer
{
    public class CompressFile
    {
        ...
        public static bool DeCompressFromZip(string filePathZip, string filePath)
        {
            try
            {
                ...
                using (Ionic.Zip.ZipFile zip = Ionic.Zip.ZipFile.Read(filePathZip))
                {
                    ...
                    zip.ExtractAll(System.IO.Path.GetDirectoryName(filePath),Ionic.Zip.ExtractExistingFileAction.OverwriteSilently);
                }
            }
            catch (Exception ex)
            {
