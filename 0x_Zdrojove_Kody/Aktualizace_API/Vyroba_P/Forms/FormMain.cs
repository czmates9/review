...
namespace Fask.Aktualizace_API.Forms
{
    ...
    public partial class FormMain : Form
    {
        ...
        public static Fask.Aktualizace_API.Scanner.ScannerBase Scanner = null;
        ...
        private Button btnKonzola = null;
        private Button btnDotisk = null;
        private Button btnKorekce = null;
        private Button btnUdalosti = null;
        private Fask.Aktualizace_API.ucDataViews.Historie.ucHistory ucHistory = null;
        private Fask.Aktualizace_API.OtevreneProdukce.ucOpenedProduction ucOpenedProduction = null;
        ...
        private void init()
        {
            ...
            {
                FlexibleMessageBox.Show(this, "Pouze jedna instance aplikace !!!");
                Application.Exit();
                return;
            }
            ...
            // INTERNAL STATE
            string internalPrdPath = Path.Combine(MySystem.MyPath.DataDirectory, Constants.InternalState + Constants.PRD);
            string internalTmpPath = Path.Combine(MySystem.MyPath.DataDirectory, Constants.InternalState + Constants.PRD + Constants.TMP);

            if (!File.Exists(internalPrdPath))
            {
                DownloadInternalStateStart();

                if (!File.Exists(internalPrdPath))
                {
                    if (File.Exists(internalTmpPath))
                    {
                        try
                        {
                            File.Copy(internalTmpPath, internalPrdPath, false);
                        }
                        catch (Exception ex)
                        {
                            Fask.Logging.ExceptionHandler2.Handle("init", "File.Copy InternalState", ex);
                        }
                    }
                    else
                    {
                        Fask.Logging.ExceptionHandler2.Handle(Logging.LogLevel.Warn, $"Soubor {internalTmpPath} nebyl nalezen. Pokračuji bez kopírování.");
                    }
                }
            ...
            // VYROBA
            string vyrobaPrdPath = Path.Combine(MySystem.MyPath.DataDirectory, Constants.Vyroba + Constants.PRD);
            string vyrobaTmpPath = Path.Combine(MySystem.MyPath.DataDirectory, Constants.Vyroba + Constants.PRD + Constants.TMP);

            if (!File.Exists(vyrobaPrdPath))
            {
                DownloadInternalVyroba();

                if (!File.Exists(vyrobaPrdPath))
                {
                    if (File.Exists(vyrobaTmpPath))
                    {
                        try
                        {
                            File.Copy(vyrobaTmpPath, vyrobaPrdPath, false);
                        }
                        catch (Exception ex)
                        {
                            Fask.Logging.ExceptionHandler2.Handle("init", "File.Copy Vyroba", ex);
                        }
                    }
                    else
                    {
                        Fask.Logging.ExceptionHandler2.Handle(Logging.LogLevel.Warn, $"Soubor {vyrobaTmpPath} nebyl nalezen. Pokračuji bez kopírování.");
                    }
                }
            ...
            UpdateStatusBarInfo();
            UpdateStatuBarLogins();
            UpdateStatusBarMachine();

            switch (Settings.ScannerType)
            {
                case Fask.Aktualizace_API.Scanner.ScannerTypes.COM:
                    Scanner = new Fask.Aktualizace_API.Scanner.ScannerCOM();
                    break;
                case Fask.Aktualizace_API.Scanner.ScannerTypes.None:
                default:
                    Scanner = new Fask.Aktualizace_API.Scanner.ScannerNone();
                    break;
            }
        }


        #region MaR 12.6.2025 zakomentoval
        ///// <summary>
        ///// Load formu
        ///// </summary>
        ///// <param name="sender"></param>
        ...
        //    if (!File.Exists(Path.Combine(MySystem.MyPath.DataDirectory, Constants.Vyroba + Constants.PRD)))
        //    {
        //        // TODO : TaD dodelat stahovani internalstate.sdf z serveru platnou verzi tu i terminal
        //        DownloadInternalVyroba();
        ...
        //                        Path.Combine(MySystem.MyPath.DataDirectory, Constants.Vyroba + Constants.PRD), false
        //                      );
        //        }
        ...
        //    UpdateStatusBarMachine();
        //    // DownloadInternalVyroba();
        //    // timerDownloadThreadStart_vyroba();
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            //#region odstraneni MaR 13.3 2024
                    ...
                    using (FormInputKod frmInputKod = new FormInputKod())
                    {
                        ...
                        if (frmInputKod.ShowDialog(this) == DialogResult.OK)
                        {
                            try
                            {
                                ...
                                bool result = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.ApplicationEndPass(Settings.TerminalID, frmInputKod.Kod.Trim());
        ...
        private void FormMain_KeyDown(object sender, KeyEventArgs e)
        {
            if (!e.Shift && !e.Control && !e.Alt)
            {
                ...
                else if (Settings.ModulKonzola && (e.KeyCode == Keys.D1 || e.KeyCode == Keys.NumPad1)) //Odvod
                {
                    PerfomKonzola();
                }
        ...
        private void timerUploadThreadStart_z_VyrobaW()
        {
            ...
            try
            {
                ...
                Fask.Aktualizace_API.FileTransfer.CompressFile.CompressToZip(filename, Odkud);

                if (Fask.Aktualizace_API.FileTransfer.Uploading.SendFile_API(Odkud, Kam) != "OK")
                    throw new Exception("Nepodařilo se odeslat davku");

                //if (Fask.Vyroba_P.FileTransfer.Uploading.SendFileCalcTime(URL, Odkud, Kam) != "OK")
                //    throw new Exception("Nepodařilo se odeslat davku");

                bool processed = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.ProcessProductionData2(
                    Settings.TerminalID,
                    guid
                    );
        ...
        private void DownloadInternalVyroba(bool jizProbehlo = false)
        {
            try
            {
                if (jizProbehlo)
                ...
                bool remotefilename = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.VyrobaDBDateTimePrepareZip(Settings.TerminalID, lastDownload);
                ...
                //    var dt = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.GetProductionUpdateByDateEve_Production(lastDownload);

                //    //aktualizace predlohy na zaklade aktualniho stavu

                //    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRDTMP.Connection_Open();

                //    foreach (SQLiteDBs.DataSets.Vyroba.Production_updateRow row in dt)
                //    {
                //        Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRDTMP.UpdateQtyCntOdvedenoLSTMod_CZPRO_VPP(row.QTYODVEDENO, row.CNTODVEDENO, row.LSTMod, row.CountEntries, row.SOPNUMBE, row.ORD, row.ITEMNMBR, row.ITEMTYPE, row.QTYPACK, row.BarcodeP);
                //    }

                //    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRDTMP.Connection_Close();
                //} 
                #endregion
        ...
        private void timerDownloadCallBack(object state)
        {
           ...
           // threadTimerDownload = new Thread(new ThreadStart(timerDownloadThreadStart_vyroba));
        ...
        private void SynchronizeDatabases(SynchronizationType synchType)
        {
            try
            {
                ...
                if (typS == SynchronizationType.Souborove)
                {
                    ...
                    //sconn.Open();
                    //sconn.Close();
                    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.Void_Command(); // ale asi nechapu vyznam otevřeni a hned zavřeni komunikace...
                }
        ...
        private void LoadModules()
        {
            try
            {
                ...
                if (btnKonzola != null)
                    tableLayoutPanel1.Controls.Remove(btnKonzola);
                if (btnKorekce != null)
                ...
                // povolení odvádění
                if (Settings.ModulKonzola)
                {
                    btnKonzola = new Button();
                    btnKonzola.Parent = this.tableLayoutPanel1;
                    btnKonzola.Text = "Servis Konzola";
                    btnKonzola.BackColor = Color.IndianRed;
                    btnKonzola.Dock = DockStyle.Fill;
                    //b.Font = new Font(;   //Tahoma; 20,25pt; style=Bold
                    btnKonzola.Font = new Font("Tahoma", 20.25f, FontStyle.Bold);
                    btnKonzola.UseVisualStyleBackColor = false;
                    btnKonzola.Click += new EventHandler(button_Konzola_click);
                    btnKonzola.Margin = new Padding(2);
                    btnKonzola.KeyDown += new KeyEventHandler(btnKonzola_KeyDown);
                    this.btnKonzola.Enabled = Settings.ModulKonzola;
                    tableLayoutPanel1.SetColumn(btnKonzola, Settings.ModulKonzolaColumn);
                    tableLayoutPanel1.SetRow(btnKonzola, Settings.ModulKonzolaRow);
                    tableLayoutPanel1.SetRowSpan(btnKonzola, Settings.ModulKonzolaRowSpan);
                    tableLayoutPanel1.SetColumnSpan(btnKonzola, Settings.ModulKonzolaColumnSpan);
                }
                ...
                {
                    ucHistory = new Fask.Aktualizace_API.ucDataViews.Historie.ucHistory();
                ...
                if (Settings.ModulPovolitNedokonceneZakazky)
                {
                    ...
                    ucOpenedProduction = new Fask.Aktualizace_API.OtevreneProdukce.ucOpenedProduction();
        ...
        void btnKonzola_KeyDown(object sender, KeyEventArgs e)
        {
            if (!e.Shift && !e.Control && !e.Alt)
            {
        ...
        void button_Konzola_click(object sender, EventArgs e)
        {
            this.PerfomKonzola();
                ...
                if (Settings.VyberZakazkyPoPrihlaseni)
                {
                    using (FormInputKod fik = new FormInputKod())
                    {
                        ...
                        { // dokud neni dobre vybrano ... 
                            ...
                            var zakazky = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataByBarcodeH_CZPRO_VPH(zakazka);
                            if (zakazky.Count > 1)
                            {
        ...
        private bool LogOut(bool showConfirm)
        {
            try
            {
                if (Settings.UEventPracovnikLoginEnabled && Globals.Pracovnik != null)
                {
                    ...
                    {
                        using (Reports.FormReportUzivatelVyrobaDen fuvd = new Fask.Aktualizace_API.Reports.FormReportUzivatelVyrobaDen())
                        {
                    ...
                    try
                    {
                       ...
                       // Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.Insert_UserEvents(Settings.LastProductionUserID, Settings.MachineID, DateTime.Now, Settings.UEventPracovnikOdhlaseni, Globals.Pracovnik.id, Settings.TerminalID, string.Empty, Guid.NewGuid());
                ...
                if (Settings.UEventSmenaEnabled)
                {
                    ...
                    try
                    {
                        ...
                        //ueta.Insert(Settings.LastProductionUserID, null, DateTime.Now, UEventStatusTypes.SmenaLogout, Settings.LastProductionUserID, Settings.TerminalID, Guid.NewGuid());
                        //ueta.Insert(Settings.LastProductionUserID, null, DateTime.Now, Settings.UEventSmenaLogout, Settings.LastProductionUserID, Settings.TerminalID, string.Empty, Guid.NewGuid());
                        //Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.Insert_UserEvents(Settings.LastProductionUserID, Settings.MachineID, DateTime.Now, Settings.UEventSmenaLogout, Settings.LastProductionUserID, Settings.TerminalID, string.Empty, Guid.NewGuid());
        ...
        private void PerfomUdalosti()
        {
            try
            {
                ...
                //using (FormInputKod fik = new FormInputKod())
                using (Udalosti.FormUdalosti fik = new Fask.Aktualizace_API.Udalosti.FormUdalosti())
                {
                    ...
                    //Fask.SQLiteDBs.DataSets.VyrobaTableAdapters.UserEventsTableAdapter ueta = new Fask.SQLiteDBs.DataSets.VyrobaTableAdapters.UserEventsTableAdapter();
                    //ueta.Connection.ConnectionString = "Data source=" + Path.Combine(MySystem.MyPath.DataDirectory, Constants.Production + Constants.PRD);
                    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.Insert_UserEvents(Settings.LastProductionUserID, string.Empty, DateTime.Now, statusRow.statusid, idPracovnik, Settings.TerminalID, string.Empty, Guid.NewGuid());
        ...
        private void PerformKorekce()
        {
            try
            {
                ...
                if (Settings.CorrectionType == "1")
                {
                    ...
                    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.FillByUserIDMachineIDNULL_Production(dsV.Production ,pracovnik.id);

                    try
                    {
                        WebServiceVyroba.VyrobaDataSet dsVWeb = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.Production_OpenedCorrection(pracovnik.id, string.Empty); //korekce mimo vyrobu ...
                    ...
                    {
                        Fask.SQLiteDBs.DataSets.Vyroba.CorrectsDataTable cdt = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataByID_Corrects(correctionOpened.First().TIMECRID);
                        if (cdt.Count <= 0)
                        {
        ...
        #region Konzola aktualizace

        private void PerfomKonzola()
        {
            try
            {
                ...
                //{
                //    using (Odvadeni.FormIDPracovnikaBezHesla frmIDPracovnika = new Fask.Aktualizace_API.Odvadeni.FormIDPracovnikaBezHesla())
                //    {
                ...
                //else
                //{
                //    using (Odvadeni.FormIDPracovnika frmIDPracovnika = new Fask.Aktualizace_API.Odvadeni.FormIDPracovnika())
                //    {
                //        if (frmIDPracovnika.ShowDialog(this) == DialogResult.Cancel)
                //            return;
                ...
                //    //Fask.SQLiteDBs.DataSets.VyrobaTableAdapters.UserEventsTableAdapter ueta = new Fask.SQLiteDBs.DataSets.VyrobaTableAdapters.UserEventsTableAdapter();
                //    //ueta.Connection.ConnectionString = "Data source=" + Path.Combine(MySystem.MyPath.DataDirectory, Constants.Production + Constants.PRD);
                //    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.Insert_UserEvents(Settings.LastProductionUserID, string.Empty, lstopertimePracovnik, Settings.UEventPracovnikPrihlaseni, idPracovnik.id, Settings.TerminalID, string.Empty, Guid.NewGuid());
                //}
                ...
                using (Fask.Aktualizace_API.Konzola.Forms.FormKonzolaMain frmKonzola = new Fask.Aktualizace_API.Konzola.Forms.FormKonzolaMain())
                {
                    frmKonzola.Owner = this;
                    frmKonzola.ShowDialog();
                }
        ...
        #region performKonzola old
        private void PerfomKonzola_old()
        {
            try
            {
                Fask.SQLiteDBs.DataSets.Vyroba.LoginsRow idPracovnik = null;
                ...
                if (Settings.OdvadeniPozadovatZadaniStroje)
                {
                    if (!String.IsNullOrEmpty(Settings.MachineID))
                    {
                        ...
                        Fask.SQLiteDBs.DataSets.Vyroba.MachinesDataTable dtMachines = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataByID_Machines(Settings.MachineID);
                    ...
                    {
                        using (Odvadeni.FormIDMachine frmIDMachine = new Fask.Aktualizace_API.Odvadeni.FormIDMachine())
                        {
                ...
                {
                    using (Odvadeni.FormOdvadeni frmOdvadeni = new Fask.Aktualizace_API.Odvadeni.FormOdvadeni(idPracovnik, idMachine))
                    {
                ...
                {
                    using (OdvadeniNadop.FormOdvadeni frmOdvadeni = new Fask.Aktualizace_API.OdvadeniNadop.FormOdvadeni(idPracovnik, idMachine))
                    {
        ...
        {
            using (Fask.Aktualizace_API.Ukolovani.UkolovaniMain ukolovani = new Fask.Aktualizace_API.Ukolovani.UkolovaniMain()) 
            {
        ...
        private void DownloadInternalStateStart(bool jizProbehlo=false)
        {
            try
            {
                DateTime lastDownload = DateTime.Now;

                bool remotefilename = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.VyrobaDBInternalStatePrepareZip(Settings.TerminalID, lastDownload);
        ...
        private void DownloadProductionStart()
        {
            try
            {
                ...
                bool remotefilename = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.VyrobaDBProductionPrepareZip(Settings.TerminalID, lastDownload);
        ...
        private void SystemTimeSynchronize()
        {
            ...
            try
            {
                DateTime servertime = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.ServerDateTime();
                Fask.Aktualizace_API.MySystem.SystemDateTime.SetDeviceTime(servertime);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        public void TasksSynchronizationStart()
        {
            if (Settings.TasksEnable)
                Fask.Aktualizace_API.Ukolovani.Ukolovani_Checker.SynchronizationStart();
        }

        public void TasksSynchronizationStop()
        {
            Fask.Aktualizace_API.Ukolovani.Ukolovani_Checker.SynchronizationStop();
        }
            ...
            try
            {
                ...
                { //pokud je prave jeden, tak umoznit ihned editaci na ok ...
                    ...
                    { // => zobrazit editaci ...
                        ...
                        Fask.Aktualizace_API.Data.UkolyTableAdapters.CZ_UKOLTableAdapter ta_ukol = new Fask.Aktualizace_API.Data.UkolyTableAdapters.CZ_UKOLTableAdapter();
                        Fask.Aktualizace_API.Data.UkolyTableAdapters.CZ_UKOL_UZIVTableAdapter ta_ukol_uziv = new Fask.Aktualizace_API.Data.UkolyTableAdapters.CZ_UKOL_UZIVTableAdapter();
                        ...
                        Fask.Aktualizace_API.Data.Ukoly.CZ_UKOL_UZIVRow uuziv = ta_ukol_uziv.GetDataByID(newTasks[0].id)[0];
                        Fask.Aktualizace_API.Data.Ukoly.CZ_UKOLRow u = ta_ukol.GetDataByID(uuziv.UkolID)[0];

                        using (Fask.Aktualizace_API.Ukolovani.UkolovaniUkolEdit uedit = new Fask.Aktualizace_API.Ukolovani.UkolovaniUkolEdit(u, uuziv))
                        {
            ...
            try
            {
                ...
                { //pokud je prave jeden, tak umoznit ihned editaci na ok ...
                    ...
                    { // => zobrazit editaci ...
                        ...
                        Fask.Aktualizace_API.Data.Ukoly.CZ_UKOL_UZIVRow uuziv = ta_ukol_uziv.GetDataByID(notificationTasks[0].id)[0];
                        Fask.Aktualizace_API.Data.Ukoly.CZ_UKOLRow u = ta_ukol.GetDataByID(uuziv.UkolID)[0];
        ...
        {
            #region MaR 12.6.2025 zakomentovano
            //using (FormInputKod frmInputKod = new FormInputKod()) 
            //{
            //    frmInputKod.Owner = this;
            //    frmInputKod.ShowKod = false;
                ...
                if (frmConfig.ShowDialog(this) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        if (btnKorekce != null && Settings.ModulPovolitKorekce)
                            this.btnKorekce.Enabled = Settings.ModulPovolitKorekce;
                        if(btnKonzola != null && Settings.ModulKonzola)
                            this.btnKonzola.Enabled = Settings.ModulKonzola;
                    ...
                    try
                    {
                        Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.Url = Settings.WebServiceAddressVyroba + Constants.Vyroba_asmx;
                        Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.Timeout = Settings.TimeOut;
        ...
        {
            using (Test.FormFilesharingviolation ffsv = new Fask.Aktualizace_API.Test.FormFilesharingviolation())
            {
                ffsv.ShowDialog();
            }
        }
