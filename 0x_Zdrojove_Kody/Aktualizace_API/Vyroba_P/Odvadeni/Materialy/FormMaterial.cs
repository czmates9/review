    ...
    public partial class FormMaterial : Form
    {
        ...
        private bool NajdiZbozi(string barcode, Fask.SQLiteDBs.DataSets.Vyroba.FASK_CONS_095DataTable zboziDatatable, bool pokracuj=false)
        {
            try
            {
                if(pokracuj)
                {
                     Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.FillByITEMNMBR_CZMST_095(true, zboziDatatable, barcode);

                }
                else
                    Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.FillByBarcode_CZMST_095(zboziDatatable, barcode);
        ...
        private void PridejZbozi(Fask.SQLiteDBs.DataSets.Vyroba.FASK_CONS_095DataTable zboziDatatable, bool pokracuj = false, bool nevydavat=false, string bcode = null)
        {
            ...
            try
            {
                ...
                if (!nevydavat)
                {
                    ...
                    {
                        jineVlakno = true;
                        using (FormInputQuantity fiq = new Fask.Aktualizace_API.Odvadeni.FormInputQuantity())
                        {
                            fiq.Owner = this;
                ...
                if (!preskocit)
                {
                    ...
                    if (Settings.Production_Material_Source_SKLID_Enter)
                    {
                        ...
                        { //zadani skladu rucne
                            ...
                            using (Aktualizace_API.Forms.FormInputKod fik = new Aktualizace_API.Forms.FormInputKod())
                            {
                                fik.Text = "Zadejte zdrojový sklad";
                                if (!nPS.IsSKL_IDNull()
                                    && !string.IsNullOrEmpty(nPS.SKL_ID))
                                {
                                    var listSklady = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataBySklid_CZMST093(nPS.SKL_ID);
                    ...
                    if (Settings.Production_Material_Source_LOCNCODE_Enter)
                    {
                        ...
                        using (Aktualizace_API.Forms.FormInputKod fik = new Aktualizace_API.Forms.FormInputKod())
                        {
                            ...
                            {
                                var listLokace = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataBySklidLocncode_CZMST094(nPS.SKL_ID, nPS.LOCNCODE);
                            ...
                            while (true)
                            {
                                ...
                                var listLokace = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataBySklidBarcode_CZMST094(nPS.SKL_ID, fik.Kod);
                                if (listLokace.Count() == 0)
                                {
        ...
        private void HledejCarkod()
        {
            try
            {
                ...
                using (Fask.Aktualizace_API.Forms.FormInputKod fik = new  Fask.Aktualizace_API.Forms.FormInputKod())
                {
        ...
        private bool NajdiZbozi_TP(string barcode, out Fask.SQLiteDBs.DataSets.Vyroba.FASK_Vyroba_TPDataTable zboziDatatable)
        {
            try
            {
                zboziDatatable = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataBy_itemnmbrDef_IDHNULL_FASK_Vyroba_TP(barcode);
        ...
        private bool NajdiMaterial_TP(string barcode, out Fask.SQLiteDBs.DataSets.Vyroba.FASK_Vyroba_TPDataTable zboziDatatable)
        {
            try
            {
                zboziDatatable = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.GetDataBy_ITEMNMBR_fol_IDHNULL_FASK_Vyroba_TP(barcode);
