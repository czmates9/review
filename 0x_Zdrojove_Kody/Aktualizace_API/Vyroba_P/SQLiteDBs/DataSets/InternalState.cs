...
namespace Fask.SQLiteDBs.DataSets
{
    ...
    partial class InternalState
    {
        public static void UpdateInternalStateLstOperationUser(string userID, DateTime lastProduction)
        {
            ...
            try
            {
                InternalState.LstOperationUserDataTable loudt = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_InternalState.GetDataByUserID(userID);
                if (loudt.Count == 0)
                    loudt.AddLstOperationUserRow(userID, lastProduction);
                else
                    loudt[0].LastOper = lastProduction;
                Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_InternalState.Update(loudt);
            }
            catch (Exception ex)
            {
        ...
        public static void DeleteInternalStateLstOperationUser(string userID)
        {
            ...
            try
            {
                Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_InternalState.DeleteUserID(userID);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle("InternalStateDataSet", System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        public static DateTime? GetInternalStateLstOperationUser(string userID)
        {
            try
            {
                ...
                return Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_InternalState.GetLastOperationDateTime(userID);
        ...
        public static int DeleteAll()
        {
            try
            {
                return Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.controller_InternalState.DeleteAll();
            }
            catch (Exception ex)
            //catch
            {
