...
namespace Fask.Aktualizace_API.Ukolovani
{
    ...
    {
        public Fask.Aktualizace_API.UkolovaniService.Ukoly.CZ_UKOL_ServiceManRow CZ_UKOL_ServiceManVybrana
        {
            get
            {
                try
                {
                    return (bs_ServiceMan.Current as DataRowView).Row as Fask.Aktualizace_API.UkolovaniService.Ukoly.CZ_UKOL_ServiceManRow;
                }
                catch (Exception ex)
                {
        ...
        private void RefreshTable()
        {
            try
            {
                ...
                this.ds_ServiceMan = Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.ukolovaniService.GetServiceMan();
        ...
        private void SendServiceMan() 
        {
            try
            {
                ...
                {
                    if (!Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.ukolovaniService.SendEmailToServiceManFull(dt, Settings.TerminalID, Settings.MachineID, CZ_UKOL_ServiceManVybrana.EMAIL, Settings.Email_Predmet_email, Poznamka))
                    {
                ...
                {
                    if (!Fask.Aktualizace_API.Forms.FormMain.Instance_FormMain.globalObject.ukolovaniService.SendEmailToServiceManFull(dt, Settings.TerminalID, Settings.MachineID, CZ_UKOL_ServiceManVybrana.OPERATOR, Settings.Email_Predmet_sms, Poznamka))
                    {
