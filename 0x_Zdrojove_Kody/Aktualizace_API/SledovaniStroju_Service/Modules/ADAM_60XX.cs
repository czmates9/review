...
namespace FASK.ADAM
{
    public class ADAM_60XX : ModulBase
    {
        ...
        {
            public AdamEventHandlerArgs(int data, bool[] DI, uint[] counter, string IP)
            {
                this._data = data;
                this._di = DI;
                this._CounterValues = counter;
                this._ip = IP;


            }

            private uint[] _CounterValues = new uint[] { };
            public uint[] CounterValues
            {
        ...
        private int iDiTotal_counter = 8;
        private int iD_counter = 4;
        ...
        /// Konstruktor adam
        /// </summary>
        /// /// <param name="cisloAlgoritmu"></param>
        ...
        {
            this.cisloAlgoritmu = cisloAlgoritmu;
        ...
        void adamP2P_GetDataEvent(Advantech.Adam.P2P_Config config)
        {
            ...
            try
            {
                ...
                if (cidlaStateActual.Length == cidlaStatePrevious.Length)
                {
                    ...
                    if (cisloAlgoritmu ==1)
                    {
                        bool Flag_Vzestupna = false;
                        bool Flag_Sestupna = false;
                        int J = 0;

                        for (int j = 0; j < 8; j++)//prochazi primo bity v bytech
                        {
                            bool stateActual = (((cidlaStateActual[0] >> j) & 0x01) == 0x01); //jak zjistit co je pri mo na danem bitu???
                            bool statePrevious = (((cidlaStatePrevious[0] >> j) & 0x01) == 0x01);
                            ...
                            else if (statePrevious && !stateActual)// pouze zmena z 1 => 0
                            {
                                Flag_Sestupna = true;
                                J = j;
                            }
                            ...
                            {
                                //nic se nedeje, neni zmena na DI
                            }
                        }



                        // prev  | Actual | vysledek
                        // 0 | 0 | 0
                        // 0 | 1 | 1
                        // 1 | 0 | 0
                        // 1 | 1 | 0
                        ...
                        if (Flag_Vzestupna) // pouze zmena z 0 => 1
                        {
                            getDIValues();
                            // Exceptions.Handler.ErrorHandle(string.Format("IP:{0} >> P2P nabezna >> {1}", this.ipAddress, i * 8 + j));
                            OnDataReady_NabeznaHrana(new AdamEventHandlerArgs(0 * 8 + J, tmp_bool.ToArray(), _DIValuesLast, IP)); // cislo cidla, ktere se zmenilo ...
                        }
                        else if (Flag_Sestupna)// pouze zmena z 1 => 0
                        {
                            getDIValues();
                            // Exceptions.Handler.ErrorHandle(string.Format("IP:{0} >> P2P sestupna >> {1}", this.ipAddress, i * 8 + j));
                            OnDataReady_SestupnaHrana(new AdamEventHandlerArgs(0 * 8 + J, tmp_bool.ToArray(), _DIValuesLast, IP));
                        }
                        else
                        {
                            //nic se nedeje, neni zmena na DI
                    ...
                    else
                    {
                        ...
                        if (Flag_Vzestupna) // pouze zmena z 0 => 1
                        {
                            getDIValues();
                            // Exceptions.Handler.ErrorHandle(string.Format("IP:{0} >> P2P nabezna >> {1}", this.ipAddress, i * 8 + j));
                            OnDataReady_NabeznaHrana(new AdamEventHandlerArgs(I * 8 + J, tmp_bool.ToArray(), _DIValuesLast, IP)); // cislo cidla, ktere se zmenilo ...
                        }
                        else if (Flag_Sestupna)// pouze zmena z 1 => 0
                        {
                            getDIValues();
                            // Exceptions.Handler.ErrorHandle(string.Format("IP:{0} >> P2P sestupna >> {1}", this.ipAddress, i * 8 + j));
                            OnDataReady_SestupnaHrana(new AdamEventHandlerArgs(I * 8 + J, tmp_bool.ToArray(), _DIValuesLast, IP));
                        }
                        else
                        {
                            //nic se nedeje, neni zmena na DI
       ...
       // private bool[] _DIStatusLast_H = new bool[] { };
        ...
        public bool[] getDIStatus()
        {
            ...
            if(cisloAlgoritmu == 1 )
            {
                cidlaData = new bool[iDiTotal_counter];

                Connect();

                if (adamTCP.Modbus().ReadInputStatus(iDiStart, iDiTotal_counter, out bDiData))
                {
                    cidlaData = new bool[bDiData.Length];
                    Array.Copy(bDiData, 0, cidlaData, 0, iDiTotal_counter);
                }
                else
                {
        ...
        private uint[] _DIValuesLast = new uint[] { };
        public uint[] DIValuesLast
        {
        ...
        private uint[] getDIValues()
        {
            ...
            uint[] DIoutValues;//= new uint[] { 0, 0, 0, 0 };

            //uint[] bDiValues;
            uint[] iValues = new uint[iTotal / 2];
            ...
            if (!adamTCP.Modbus().ReadHoldingRegs(iStart, iTotal, out DIoutValues))
            {
                //Exceptions.Handler.ErrorHandle("Nepodařilo se načíst stavy Výstupů z ADAMa. LastError: " + adamTCP.LastError.ToString(), "adamTCP", false);
                return iValues;
            }

            for (int i = 0; i < DIoutValues.Length / 2; i++)
            {
                //iValues[i] = (((int)bDiValues[i+1]) << 8 + (int)bDiValues[i]);
                iValues[i] = DIoutValues[i + 2];
            }
            _DIValuesLast = new uint[iValues.Length];
        ...
        {
            if (communication_TCP && !communication_P2P)
            {
                if (timerSetDOsAndReadDIaDOStatus != null)
        ...
        private void timerSetDOsAndReadDIaDOStatusCallBack(object state)
        {
            ...
            try
            {
                ...
                if (communication_TCP)
                {
                    ...
                    if (state_Hrana == 1)
                    {
                        OnDataReady_NabeznaHrana(new AdamEventHandlerArgs(senzor, _DIStatusLast, _DIValuesLast, IP));
                    }
                    else if (state_Hrana == 2)
                    {
                        OnDataReady_SestupnaHrana(new AdamEventHandlerArgs(senzor, _DIStatusLast, _DIValuesLast, IP));
                    }
                }
