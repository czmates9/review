...
namespace FASK.RFID_SR_5102
{
    ...
    public class RFID_SR_5102_Provider : IRFIDProvider.IRFIDProvider
    {
...
#region TID
        ...
        /// </summary>
            public RFID_SR_5102_Provider()
            {
                try
                {
                    ...
                    string CmdComAddr = Konzola.Konfigurace.Globals_Konfig_Konzola.Konfigurace.Sklady_RFID[0].Address;
                    ComPorty port = (ComPorty)Enum.Parse(typeof(ComPorty), Konzola.Konfigurace.Globals_Konfig_Konzola.Konfigurace.Sklady_RFID[0].ComPort, true);
                    Baudrate baud = (Baudrate)Enum.Parse(typeof(Baudrate), Konzola.Konfigurace.Globals_Konfig_Konzola.Konfigurace.Sklady_RFID[0].Baudrate, true);
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                if (status != "OK")
                {
                    if (status == "ComPort Opened")
                    {
                        Stop();
                    }
                    Fask.Logging.ExceptionHandler2.Handle(Fask.Logging.LogLevel.Info, "RFID Start: " + status);
                    //timer.Dispose();
                ...
                {
                    timer.Change(0,100);// = new Timer(, .Change(0, 100);
                    isopen = true;
                }
            }
            catch (Exception ex)
        ...
        public void Stop()
        {
            try
            {
                ...
                if (status != "OK" && status != "Port is not open.")
                {
                    ...
                    timer.Change(System.Threading.Timeout.Infinite, System.Threading.Timeout.Infinite);
                    isopen = false;
                    Fask.Logging.ExceptionHandler2.Handle(Fask.Logging.LogLevel.Info , "RFID Stop: " + status);
