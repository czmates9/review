...
{
    public partial class FormZdrojStavOkruhEdit : Form
    {
        ...
        private Fask.Interfaces.DataSets.Servis.CZMST_Servis_OkruhRow selectedOkruh { get; set; }

        public FormZdrojStavOkruhEdit()
        {
            try
            {
                InitializeComponent();
        ...
        private void PerformOK()
        {
            try
            {
                ...
                else   // nový záznam
                {
                    ...
                    // vlozeni do zdroj seznam
                    Fask.Interfaces.DataSets.Servis.CZMST_Servis_ZdrojSeznamRow zdrojSeznamRow = ds.CZMST_Servis_ZdrojSeznam.NewCZMST_Servis_ZdrojSeznamRow();
                    zdrojSeznamRow.ID = selectedOkruh.ZdrojSeznamID;
                    zdrojSeznamRow.ZdrojID = newZdrojRow.ID;
                    zdrojSeznamRow.SetPoradiNull();
                    //zdrojSeznamRow.SetIDStavNull();
                    zdrojSeznamRow.IDStav = selectedStav.ID;
                    //zdrojSeznamRow.SetIDCinnostNull();
                    if (selectedStav.IsIDCinnostNull())
                        zdrojSeznamRow.SetIDCinnostNull();
                    else
                        zdrojSeznamRow.IDCinnost = selectedStav.IDCinnost;
                    ((Fask.Interfaces.Servis.IServis)providerServis).InsertZdrojSeznam(zdrojSeznamRow);

                    // vlozeni zdrojstav                    
                    Fask.Interfaces.DataSets.Servis.CZMST_Servis_ZdrojStavRow zdrojStavRow = ds.CZMST_Servis_ZdrojStav.NewCZMST_Servis_ZdrojStavRow();
                    zdrojStavRow.IDZdroj = newZdrojRow.ID;
                    if (selectedStav.IsIDCinnostNull())
                        zdrojStavRow.SetIDCinnostNull();
                    else
                        zdrojStavRow.IDCinnost = selectedStav.IDCinnost;

                    zdrojStavRow.IDStav = selectedStav.ID;
                    zdrojStavRow.Modified = DateTime.Now;
                    zdrojStavRow.IDTerminal = Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].TerminalID;
                    zdrojStavRow.SetIDUserNull();   // TODO: poresit
                    zdrojStavRow.GUID = Guid.NewGuid();
                    //zdrojStavRow.CinnostValue = string.Empty;
                    //zdrojStavRow.CinnostType = string.Empty;
                    zdrojStavRow.SetCinnostValueNull();
                    zdrojStavRow.SetCinnostTypeNull();
                    zdrojStavRow.SetCountEntriesNull();
                    // odberatel
                    if (selectedOkruh.IsODB_IDNull())
                        zdrojStavRow.SetODB_IDNull();
                    else
                        zdrojStavRow.ODB_ID = selectedOkruh.ODB_ID;

                    // okruh
                    zdrojStavRow.OkruhID = selectedOkruh.ID;
                    ((Fask.Interfaces.Servis.IServis)providerServis).InsertZdrojStav(zdrojStavRow);
        ...
        private bool ValidateData()
        {
            try
            {
                ...
                // kontrola, zdali jiz zdroj neni v ZdrojSeznam (pokud je, neni mozne pokracovat ...)
                Fask.Interfaces.DataSets.Servis.CZMST_Servis_ZdrojStavRow zdrojstavrow = ((Fask.Interfaces.Servis.IServis)providerServis).GetZdrojStavByZdrojID(tbID.Text);
                if (zdrojstavrow != null)
                {
                    errorProvider1.SetError(tbID, "Vybraný zdroj již má přiřazen počáteční stav v tabulce stavů");
                }

                // pocatecni stav
                if(selectedStav == null)
                    errorProvider1.SetError(tbIDStav, "Musíte vybrat počáteční stav");

                // vybrany okruh
                if (selectedOkruh == null)
                    errorProvider1.SetError(tbIDOkruh, "Musíte vybrat počáteční okruh");

                // 20.5.2016 PeV: vypnuta kontrola na zadani oznaceni
        ...
        private void formZdrojeEdit_Enter(object sender, EventArgs e)
        {
            ...
            try
            {
                if (sender is TextBox)
                {
                    ...
                    {
                        text = "Typ zdroje. Slouží pouze k zobrazení doplňující informace na terminálu.";
                    }
                    else if (tbIDStav == tb)
                    {
                        text = "Počáteční stav zdroje.";
                    }
                    else if (tbIDOkruh == tb)
                    {
                        text = "Okruh, ve kterém se zdroj bude nacházet.";
                    } else if (tbMisto == tb)
                    {
        ...
        private void btnOkruhID_Click(object sender, EventArgs e)
        {
            // aktualizace napovedy ... po kliknuti se nezavola
            formZdrojeEdit_Enter(sender, null);
            PerformPriraditOkruh();
        }

        private void PerformPriraditOkruh()
        {
            try
            {
                if (!MySystem.LoginTest.UserLoginTest())
                ...
                Fask.Interfaces.DataSets.Servis dsSelectedRows = new Fask.Interfaces.DataSets.Servis();
                using (Servis.FormOkruhySelect frmcinnosti = new FormOkruhySelect(false, selectedOkruh))
                {
                    frmcinnosti.Text = "Přiřadit okruh";
                    //frmstavy.rowsSelected = rowStav;
                    if (frmcinnosti.ShowDialog(this) != DialogResult.OK)
                        return;

                    selectedOkruh = frmcinnosti.SelectedRow;

                    tbIDOkruh.Text = selectedOkruh.Oznaceni.Trim() + " (" + selectedOkruh.ID.Trim() + ")";
                }
            }
            catch (Exception ex)
            {
