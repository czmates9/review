...
{
    public partial class FormStavyList : Form
    {
        /// <summary>
        ...
        public Fask.Interfaces.DataSets.Servis.CZMST_Servis_StavRow SelectedRow
        {
            get
            {
                try
                {
                    return ((DataRowView)(dgServis.BindingContext[bsServis].Current)).Row as Fask.Interfaces.DataSets.Servis.CZMST_Servis_StavRow;
        ...
        private void FormStavyList_Load(object sender, EventArgs e)
        {
            try
            {
                this.ShowIcon = false;
        ...
        private void FormStavyList_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void FormStavyList_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        /// </summary>
        private void UpdateForm(Fask.Interfaces.DataSets.Servis.CZMST_Servis_StavRow row)
        {
            try
            {
                ...
                dsServis.Clear();
                dsServis = ((Fask.Interfaces.Servis.IServis)providerServis).GetStavy();
                ...
                try
                {
                    ...
                    {
                        int index = bsServis.Find(dsServis.CZMST_Servis_Stav.IDColumn.ColumnName, id);
                        this.bsServis.Position = index;
                    }
                    //dsServis.CZMST_Servis_Zdroj.
                }
                catch
                {
                }
        ...
        private void PerformCreateRecord()
        {
            try
            {
                ...
                using (Servis.FormStavyEdit frmzdroje = new FormStavyEdit())
                {
                    frmzdroje.Text = "Nový stav";
        ...
        private void PerformEditRecord()
        {
            try
            {
                ...
                using (Servis.FormStavyEdit frmstavy = new FormStavyEdit())
                {
                    frmstavy.stavrow = SelectedRow;
                    frmstavy.Text = "Úprava stavu";
                    if (frmstavy.ShowDialog(this) != DialogResult.OK)
                        return;

                    UpdateForm(frmstavy.returnrow);
                }
            }
            catch (Exception ex)
            {
        ...
        private void PerformDeleteRecord()
        {
            try
            {
                ...
                if (MessageBox.Show("Opravdu chcete odstranit stav '" + SelectedRow.Oznaceni.Trim() + "'?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    return;
                ...
                bool result = ((Fask.Interfaces.Servis.IServis)providerServis).DeleteStav(SelectedRow.ID); //, odstranitNavaznosti);
        ...
        private void dataGridView1_Sorted(object sender, EventArgs e)
        {
            try
            {
                int pos = this.bsServis.Find(dsServis.CZMST_Servis_Cinnost.IDColumn.ColumnName, selectedStavID);
