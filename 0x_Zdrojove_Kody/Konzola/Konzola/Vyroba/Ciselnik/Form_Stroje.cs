...
{
    public partial class Form_Stroje : Form
    {
        ...
        private List<Fask.Interfaces.DataSets.Vyroba.MachinesRow> Machines_selectedRows
        {
            //get
            //{
            ...
            get
            {
                return dg_Stroj.SelectedRows
                    .Cast<DataGridViewRow>()
                    .OrderBy(r => r.Index)               // zarovnání na vizuální pořadí v gridu
                    .Select(r => ((DataRowView)r.DataBoundItem).Row as Fask.Interfaces.DataSets.Vyroba.MachinesRow)
                    .Where(r => r != null)
                    .ToList();
            }
        }

        public Fask.Interfaces.DataSets.Vyroba.MachinesRow Machines_selectedRow
        {
            get
            {
                try
                {
                    return ((DataRowView)(dg_Stroj.BindingContext[bs_Machines].Current)).Row as Fask.Interfaces.DataSets.Vyroba.MachinesRow;
                }
                catch
                {
                    return null;
        ...
        public Form_Stroje()
        {
            ...
            this.dg_Stroj.UpdateColumnHeaderCellsByDatasource();



        }

        public Form_Stroje(Opravneni opravneni) : this()   // ROZŠÍŘENÝ (správně)
        {
        ...
        public Form_Stroje(bool allowMultiSelect, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni) : this()
        {
            dg_Stroj.MultiSelect = allowMultiSelect;
            Zobrazeni = typZobrazeni;
            ApplyZobrazeniSettings();
        }

        public Form_Stroje(bool allowMultiSelect, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni, Opravneni opravneni)
    : this(allowMultiSelect, typZobrazeni)
        ...
        private void FormOdvod_MachineStateSetList_Load(object sender, EventArgs e)
        {
            ...
            {
                this.ShowIcon = false;
                dg_Stroj.AutoGenerateColumns = false;  // KLÍČ
                dg_Stroj.DataSource = bs_Machines;     // bind až za běhu
                //this.WindowState = FormWindowState.Maximized;
                this.dg_Stroj.LoadConfiguration(this.GetType().ToString());
                ...
                advancedDataGridViewSearchToolBar1.SetColumns(dg_Stroj.Columns);
               ...
               // cB_description.Items.Add("Nevybráno");
                ...
                FillComboBoxFromDatabase(cB_name);
        ...
        private void FormOdvod_MachineStateSetList_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                this.progressIndicator1.Location = new Point(this.dg_Stroj.Location.X + (this.dg_Stroj.Width / 2) - (progressIndicator1.Size.Width / 2), this.dg_Stroj.Location.Y + (this.dg_Stroj.Height / 2) - (progressIndicator1.Size.Height / 2));
            }
            catch { }
        }
        ...
        public string GetDescriptionFromFilter()
        {
            ...
            {
                description = cB_name.SelectedItem as string;
            }
        ...
        private void PerformEditRecord()
        {
            try
            {
                ...
                {
                    frmuziv.machinesrow = Machines_selectedRow;
                    ...
                    if ((provider != null) && provider is Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)
                        ((Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)provider).UpdateMachines(frmuziv.machinesrow);
                    else
                        throw new Exception("UpdateMachines not implementet");

                    this.ds_Machines.AcceptChanges();
                    //UpdateForm();
                    PerformVyhledat();
                }
            }
        ...
        private void PerformDeleteRecord()
        {
            try
            {
                ...
                if (MessageBox.Show(
                        "Chcete odstranit stroj " + Machines_selectedRow.name.Trim() + "?",
                        this.Text,
                ...
                //Machines_selectedRow.Delete();

                // 2) pošli změny do DB (musí existovat DeleteCommand)
                if ((provider != null) && provider is Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)
                    ((Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)provider).DeleteMachines(Machines_selectedRow);
                else
                    throw new Exception("DeleteMachines not implementet");

                // 3) reload až po úspěšném Update
                PerformVyhledat();

                // 4) AcceptChanges sem vůbec nemusíš (DataAdapter to dělá sám po úspěchu)
                // ds_Machines.AcceptChanges();
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void PerformCreateRecord()
        {
            try
            {
                ...
                Fask.Interfaces.DataSets.Vyroba.MachinesRow newmachinesrow = null;
                ...
                using (FormStrojeEdit frmuziv = new FormStrojeEdit())
                {
                    ...
                    newmachinesrow = frmuziv.newmachinesrow;
                    ...
                    if ((provider != null) && provider is Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)
                        ((Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)provider).InsertMachines(newmachinesrow);
                    else
                        throw new Exception("InsertMachines not implementet");


                    //lta.Fill(vyrobaDataSet1.Groups);
                    PerformVyhledat();
                    //if ((provider != null) && provider is Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)
                    //    ((Fask.Interfaces.Vyroba.Machines.IMachines_IudCommand)provider).Groups_Fill(this.dsSkupiny);
                    //else
        ...
        private void FormUzivateleList_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                this.dg_Stroj.SaveConfiguration(this.GetType().ToString());
        ...
        private void buttonVyhledat_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewColumn column in dg_Stroj.Columns)
                {
                    // Získání typu dat pro sloupec
                    Type dataType = null;

                    if (dg_Stroj.DataSource is DataTable dataTable)
                    {
                    ...
                    else if (dg_Stroj.DataSource is BindingSource bindingSource)
                    {
        ...
        public virtual void WaithToEndThread()
        {
            ...
            try
            {
                if (bw_Stroje.IsBusy)
                {
                    bw_Stroje.CancelAsync();
                    while (bw_Stroje.IsBusy)
                    {
        ...
        public  void PerformVyhledat() {
            ...
            try
            {
                DataTable dtchanged = this.ds_Machines.Machines.GetChanges();
                ...
                int FirstDisplayedScrollingRowIndex = this.dg_Stroj.FirstDisplayedScrollingRowIndex; //Save Current Scroll Index

                bw_Stroje.RunWorkerAsync(filtr);

                if ((FirstDisplayedScrollingRowIndex >= 0) && ((this.dg_Stroj.Rows.Count - 1) >= FirstDisplayedScrollingRowIndex)) this.dg_Stroj.FirstDisplayedScrollingRowIndex = FirstDisplayedScrollingRowIndex; //Restore Scroll Index
        ...
        public void PerformPridatFiltr() 
        {
            try
            {
                ...
                this.dg_Stroj.SaveConfiguration(nameFile);
        ...
        public void PerformNastavitFiltr(Fask.Interfaces.Filtry.Odvod_MachineStateSetListFiltr filtr) 
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(filtr.name))
                {
                    ...
                    cB_name.SelectedItem = filtr.name;
                }
                ...
                if (!string.IsNullOrEmpty(filtr.Description))
                {
                    ...
                    tB_popis.Text = filtr.Description;
                }
                ...
                if (string.IsNullOrEmpty(filtr.NameFileDataGridView))
                    ...
                    this.dg_Stroj.LoadConfiguration(filtr.NameFileDataGridView);
        ...
        private bool CreateFilter(ref Fask.Interfaces.Filtry.Odvod_MachineStateSetListFiltr filtr)
        {
            ...
            filtr.Description = tB_popis.Text;
        ...
        private void exportDoCSVVseToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dg_Stroj.ExportToCSV(Fask.Interfaces.Classes.EXPORT_DAT.VSE);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void exportDoCSVOznaceneToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dg_Stroj.ExportToCSV(Fask.Interfaces.Classes.EXPORT_DAT.OZNACENE);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void exportDoExcelVseToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dg_Stroj.ExportToExcel(Fask.Interfaces.Classes.EXPORT_DAT.VSE);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void exportDoExceOznaceneToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dg_Stroj.ExportToExcel(Fask.Interfaces.Classes.EXPORT_DAT.OZNACENE);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void exportDoXMLVseToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dg_Stroj.ExportToXML(Fask.Interfaces.Classes.EXPORT_DAT.VSE);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void exportDoXMLOznaceneToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dg_Stroj.ExportToXML(Fask.Interfaces.Classes.EXPORT_DAT.OZNACENE);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void advancedDataGridViewSearchToolBar1_Search(object sender, Zuby.ADGV.AdvancedDataGridViewSearchToolBarSearchEventArgs e)
        {
            ...
            if (!e.FromBegin)
            {
                bool endcol = dg_Stroj.CurrentCell.ColumnIndex + 1 >= dg_Stroj.ColumnCount;
                bool endrow = dg_Stroj.CurrentCell.RowIndex + 1 >= dg_Stroj.RowCount;

                if (endcol && endrow)
                {
                    startColumn = dg_Stroj.CurrentCell.ColumnIndex;
                    startRow = dg_Stroj.CurrentCell.RowIndex;
                }
                else
                {
                    startColumn = endcol ? 0 : dg_Stroj.CurrentCell.ColumnIndex + 1;
                    startRow = dg_Stroj.CurrentCell.RowIndex + (endcol ? 1 : 0);
                }
            }
            DataGridViewCell c = dg_Stroj.FindCell(
                e.ValueToSearch,
        ...
        private void bw_OdvodEvents_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if (bw_Stroje.CancellationPending)
                {
                    e.Cancel = true;
                    return;
                }
                ...
                // nacteni dat v oddelenem vlakne
                if ((provider != null) && (provider is Fask.Interfaces.Vyroba.Machines.IMachines_GetFilterData))
                {
                    ds = ((Fask.Interfaces.Vyroba.Machines.IMachines_GetFilterData)provider).Machines_GetFilterData(filtr);
                }
                else
                {
                    throw new NotImplementedException("Provider neobsahuje implemetaci IMachines_GetFilterData");
                }
    ...
    MessageBoxIcon.Error
        ...
        private void bw_OdvodEvents_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                {
                    // handle the error
                    ds_Machines = new Fask.Interfaces.DataSets.Vyroba();
                    bs_Machines.DataSource = ds_Machines;
                ...
                else
                {
                    ...
                    ds_Machines = (Fask.Interfaces.DataSets.Vyroba)e.Result;
                    if (ds_Machines == null)
                        ds_Machines = new Fask.Interfaces.DataSets.Vyroba();

                    bs_Machines.DataSource = ds_Machines;

                    if (ds_Machines.Machines.Count == 0)
                        SetStatusLabelText_Events(-1);
                    else
                    {
                        foreach (DataGridViewRow row in dg_Stroj.SelectedRows)
                        {
        ...
        private void SetStatusLabelText_Events(int index)
        {
            ...
            tssl_Eventu_Count.Text = string.Format("{0}/{1}", index + 1, ds_Machines.Machines.Count);
        }
        ...
        private void PerformPrint()
        {
            try
            {
                ...
                if (Machines_selectedRows.Count > 1)
                {
                ...
                foreach (Fask.Interfaces.DataSets.Vyroba.MachinesRow row in Machines_selectedRows)
                {
                    try
                    {
                        Fask.Interfaces.DataSets.Vyroba.MachinesRow newRow = dt.NewMachinesRow();
                ...
                foreach (Fask.Interfaces.DataSets.Vyroba.MachinesRow item in dt)
                {
        ...
        private void PrintReport(Fask.Interfaces.DataSets.Vyroba.MachinesDataTable dt)
        {
            try
            {
        ...
        private void Perform_Tisk_Selected_Primy_Tisk(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                foreach (var item in Machines_selectedRows)
                {
                ...
                else
                {
                    ...
                    Tisk_selected_rows_Primy_Tisk(plr_Path, true, MN_ToTisk, Machines_selectedRows, NazevVychoziTiskarny);
        ...
        private void Tisk_selected_rows_Primy_Tisk(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.Vyroba.MachinesRow> selected_Rows, string nazevVychTiskarny)
        {
            try
            {
        ...
        private void PerformPrint(bool primyTisk, string klicTyp, string nazevVychTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                //foreach (Fask.Interfaces.DataSets.Vyroba.MachinesRow row in dt)
                //{
        ...
        private void PrintReport_RDLC(Fask.Interfaces.DataSets.Vyroba.MachinesDataTable dt, bool primyTisk, string path_sablona, string nazevVychTiskarny, int pocetVytisku)
        {
            try
            {
        ...
        private void Perform_Tisk_Selected(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                else
                {
                    ...
                    Tisk_selected_rows(plr_Path, true, MN_ToTisk, Machines_selectedRows);
        ...
        private void Tisk_selected_rows(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.Vyroba.MachinesRow> selected_Rows)
        {
            try
            {
        ...
        private string PrepareDataToTisk(
            string strPath,
            Fask.Interfaces.DataSets.Vyroba.MachinesRow row,
            int? mnozstviDoTisku
            )
        {
            try
        ...
        private Dictionary<string, string> PrepareDataToTisk(Fask.Interfaces.DataSets.Vyroba.MachinesRow row_data)
        {
            try
            {
                while (true)
