...
{
    public partial class FormProductionEdit3 : Form
    {
        ...
        private void FormProductionEdit_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // 7.6.2022 - Rozhodnuti JaS
                // Pokud je TIMEMODE == 0 (pouze STOP)
                // tak je automaticky ODVOD_STOP

                //Pokud je jiny, tak se dohledava logika niže

                if (rowProduct.TIMEMODE == 0)
        ...
        private void tsmiCarKodVyrobek_Click(object sender, EventArgs e)
        {
            HledatCarovyKod();
        }
        ...
        private void PerformOK()
        {
            try
            {
                ...
                {
                    #region Schvalovací logika stloupce ??
                    ...
                    if ((item.IsqtyOldNull() && (item.id == rowProduct.id)))
                        item.qtyOld = rowProduct.qty;
                    ...
                    {
                        item.status = Convert.ToInt32(textBoxStatus.Text.Trim());
                    }
                    else
                    {
                        item.SetstatusNull();
                    ...
                    item.TYPEPAL = textBoxTYPEPAL.Text.Trim();
                    item.PackType = textBoxPackType.Text.Trim();


                    #region ukladani hodnoty vahy
                    if (!string.IsNullOrEmpty(textBoxWEIGHT.Text))
                    {
                        decimal weight;
                        if (decimal.TryParse(textBoxWEIGHT.Text.Trim(), out weight))
                        {
                            if (item.IsWEIGHT_OLDNull())
                            {

                                if (!item.IsWEIGHTNull())
                                {
                                    item.WEIGHT_OLD = item.WEIGHT;

                                }
                                else
                                {
                                    //20.11.2025 MaR zmena
                                    //item.WEIGHT_OLD = -666;
                                   item.WEIGHT_OLD = 0;
                                }
                        ...
                        {
                            MessageBox.Show("Špatně zadaná hodnota váhy!", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                            // item.SetWEIGHTNull();
                    ...
                    if (!string.IsNullOrEmpty(tB_REZ_1.Text))
                    {
                        item.REZ_1 = tB_REZ_1.Text.Trim();
                    }
                    if (!string.IsNullOrEmpty(tB_REZ_2.Text))
                    {
                        item.REZ_2 = tB_REZ_2.Text.Trim();
                    }
                    if (!string.IsNullOrEmpty(tB_REZ_3.Text))
                    {
                        item.REZ_3 = tB_REZ_3.Text.Trim();
                    }
                    if (!string.IsNullOrEmpty(tB_REZ_4.Text))
                    {
                        item.REZ_4 = tB_REZ_4.Text.Trim();
                    }
                    if (!string.IsNullOrEmpty(tB_REZ_5.Text))
                    {
                        item.REZ_5 = tB_REZ_5.Text.Trim();
                    }
                    ...
                    if (!item.IsTIMESTOPNull())
                    {
                        ...
                        {
                            var sklad = cb_Sklad.SelectedItem as Fask.Interfaces.DataSets.Vyroba.CZMST093Row;
                            if (sklad != null)
                                item.SKL_ID = (sklad).skl_id.Trim();
                        }

                        if (cb_Lokace.SelectedItem != null)
                        {
                            var lokace = cb_Lokace.SelectedItem as Fask.Interfaces.DataSets.Vyroba.CZMST094Row;
                            if (lokace != null)
        ...
        private void LoadProductionRow()
        {
            ...
            dateTimePickerTIMEPREPSTART.Value = rowProduct.IsTIMEPREPSTARTNull() ? new DateTime(1900, 1, 1) : rowProduct.TIMEPREPSTART;
            dateTimePickerTIMEPREPSTOP.Value = rowProduct.IsTIMEPREPSTOPNull() ? new DateTime(1900, 1, 1) : rowProduct.TIMEPREPSTOP;
            ...
            dateTimePickerTIMESTART.Value = rowProduct.IsTIMESTARTNull() ? new DateTime(1900, 1, 1) : rowProduct.TIMESTART;
            dateTimePickerTIMESTOP.Value = rowProduct.IsTIMESTOPNull() ? new DateTime(1900, 1, 1) : rowProduct.TIMESTOP;
            dateTimePickerTIMECORSTART.Value = rowProduct.IsTIMECORSTARTNull() ? new DateTime(1900, 1, 1) : rowProduct.TIMECORSTART;
            dateTimePickerTIMECORSTOP.Value = rowProduct.IsTIMECORSTOPNull() ? new DateTime(1900, 1, 1) : rowProduct.TIMECORSTOP;
            ...
            dateTimePickerISOK.Value = rowProduct.IsISOKNull() ? new DateTime(1900, 1, 1) : rowProduct.ISOK;
            ...
            dateTimePickerDateedit.Value = rowProduct.IsdateeditNull() ? new DateTime(1900, 1, 1) : rowProduct.dateedit;
            textBoxSTORNOGUID.Text = rowProduct.IsSTORNOGUIDNull() ? string.Empty : rowProduct.STORNOGUID.ToString();
            textBoxSTORNOGUID.Enabled = false;
            textBoxTYPEPAL.Text = rowProduct.IsTYPEPALNull() ? string.Empty : rowProduct.TYPEPAL;
            textBoxPackType.Text = rowProduct.IsPackTypeNull() ? string.Empty : rowProduct.PackType;
            textBoxStatus.Text = rowProduct.IsstatusNull() ? string.Empty : rowProduct.status.ToString();
            textBoxWEIGHT.Text = rowProduct.IsWEIGHTNull() ? string.Empty : rowProduct.WEIGHT.ToString();
            textBoxWEIGHT_OLD.Text = rowProduct.IsWEIGHT_OLDNull() ? string.Empty : rowProduct.WEIGHT_OLD.ToString();
            tB_REZ_1.Text = rowProduct.IsREZ_1Null() ? string.Empty : rowProduct.REZ_1;
            tB_REZ_2.Text = rowProduct.IsREZ_2Null() ? string.Empty : rowProduct.REZ_2;
            tB_REZ_3.Text = rowProduct.IsREZ_3Null() ? string.Empty : rowProduct.REZ_3;
            tB_REZ_4.Text = rowProduct.IsREZ_4Null() ? string.Empty : rowProduct.REZ_4;
            tB_REZ_5.Text = rowProduct.IsREZ_5Null() ? string.Empty : rowProduct.REZ_5;


            cb_Sklad.Items.Clear();
            cb_Sklad.Items.AddRange(dsVyroba.CZMST093.ToArray());
            if (!rowProduct.IsSKL_IDNull() && !string.IsNullOrEmpty(rowProduct.SKL_ID.Trim()))
            {
                ...
                if (sklady.Count() > 0)
                    cb_Sklad.SelectedItem = sklady.First();
            }

            cb_Lokace.Items.Clear();
            cb_Lokace.Items.Add(string.Empty);
            cb_Lokace.Items.AddRange(dsVyroba.CZMST094.Where(x => !rowProduct.IsSKL_IDNull() && x.SKL_ID.Trim() == rowProduct.SKL_ID.Trim()).ToArray());
            cb_Lokace.SelectedIndex = 0;
            ...
            if (!rowProduct.IsLOCNCODENull() && !string.IsNullOrEmpty(rowProduct.LOCNCODE.Trim()))
            {
                ...
                if (lokace.Count() > 0)
                    cb_Lokace.SelectedItem = lokace.First();
            }
        ...
        private bool ValidateData()
        {
            try
            {
                ...
                // validace status
                    if (!string.IsNullOrEmpty(textBoxStatus.Text.Trim()))
                    {

                        int _status;
                        status = Int32.TryParse(textBoxStatus.Text, out _status);
                        if (!status)
                        {
                            errorProvider1.SetError(textBoxStatus, "Status musí být číslo");
                        }
                        else
                        {
                            if (_status < 0)
                            {
                                errorProvider1.SetError(textBoxORD, "Status musí být větší než nebo rovno 0");
                            }
                        }
                    }
                ...
                if (!string.IsNullOrEmpty(textBoxLoginid.Text.Trim()))
                {
                    ...
                    var _validUserid = FASK.Logins.Uzivatel.Instance.Komunikace.GetLoginsByID(textBoxLoginid.Text.Trim());
                ...
                { // je zadan sklad, tak se bude menit ...
                    if (sklad == null)
                    {
                        errorProvider1.SetError(cb_Sklad, "Sklad není nastaven");
                    }
                }

                if (!rowProduct.IsLOCNCODENull() && !string.IsNullOrEmpty(rowProduct.LOCNCODE))
                {
                    var lokace = cb_Lokace.SelectedItem as Fask.Interfaces.DataSets.Vyroba.CZMST094Row;
                    if (lokace == null)
                    {
                        errorProvider1.SetError(cb_Lokace, "Lokace není nastavena");
                    }
                    if (sklad == null)
                    {
                        errorProvider1.SetError(cb_Sklad, "Sklad není nastaven");
                    ...
                    if (lokace != null && sklad != null)
                    {
                        ...
                        {
                            errorProvider1.SetError(cb_Lokace, "Sklad vybrané lokace neodpovídá vybranému skladu");
                        }
                    }
                }
        ...
        private void textBoxSklad_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var lokaceAkt = cb_Lokace.SelectedItem as Fask.Interfaces.DataSets.Vyroba.CZMST094Row;
                ...
                cb_Lokace.Items.Clear();
                cb_Lokace.Items.AddRange(dsVyroba.CZMST094.Where(x => x.SKL_ID == sklad.skl_id).ToArray());

                if (lokaceAkt != null)
                {
                    var lokaceNove = dsVyroba.CZMST094.Where(x => x.SKL_ID == sklad.skl_id && x.Barcode == lokaceAkt.Barcode);
                    if (lokaceNove.Count() > 0)
                        cb_Lokace.SelectedItem = lokaceNove.First();
                    else
                        cb_Lokace.SelectedIndex = 0;
                }
                else 
