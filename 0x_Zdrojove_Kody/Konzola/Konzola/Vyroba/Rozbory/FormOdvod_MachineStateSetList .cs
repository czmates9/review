...
{
    public partial class FormOdvod_MachineStateSetList : Form
    {
        ...
        private List<Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow> MachineStateSetHistory_selectedRows
        {
            //get
            //{
            ...
            get
            {
                return dg_OdvodMachineStateSet.SelectedRows
                    ...
                    .Select(r => ((DataRowView)r.DataBoundItem).Row as Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow)
                    .Where(r => r != null)
                    .ToList();
            }
        }

        public Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow MachineStateSetHistory_selectedRow
        {
            get
            {
                try
                {
                    return ((DataRowView)(dg_OdvodMachineStateSet.BindingContext[bs_OdvodMachineStateSet].Current)).Row as Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow;
                }
                catch
                {
                    return null;
        ...
        public FormOdvod_MachineStateSetList() 
        {
            InitializeComponent();
        ...
        public FormOdvod_MachineStateSetList(bool allowMultiSelect, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni) : this()
        {
        ...
        public  void PerformVyhledat() {
            ...
            try
            {
                DataTable dtchanged = this.ds_OdvodMachineStateSet.MachineStateSetHistory.GetChanges();
        ...
        private void bw_OdvodEvents_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (ds_OdvodMachineStateSet.MachineStateSetHistory.Count == 0)
                        SetStatusLabelText_Events(-1);
                    else
                    {
        ...
        private void SetStatusLabelText_Events(int index)
        {
            ...
            tssl_Eventu_Count.Text = string.Format("{0}/{1}", index + 1, ds_OdvodMachineStateSet.MachineStateSetHistory.Count);
        }
        ...
        private void PerformPrint()
        {
            try
            {
                ...
                if (MachineStateSetHistory_selectedRow == null)
                {
                    MessageBox.Show("Není vybrán záznam pro tisk!", this.Text, MessageBoxButtons.OK);
                    return;
                }

                if (MachineStateSetHistory_selectedRows.Count > 1)
                {
                ...
                Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryDataTable dt = new Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryDataTable();

                foreach (Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow row in MachineStateSetHistory_selectedRows)
                {
                    try
                    {
                        Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow newRow = dt.NewMachineStateSetHistoryRow();
                        ...
                        dt.AddMachineStateSetHistoryRow(newRow);
                    }
                    catch (Exception ex)
                    {
                        // Ošetření vyjimky
                ...
                foreach (Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow item in dt)
                {
        ...
        private void PrintReport(Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryDataTable dt)
        {
            try
            {
        ...
        private void Perform_Tisk_Selected_Primy_Tisk(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                foreach (var item in MachineStateSetHistory_selectedRows)
                {

                    if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start vyroba/rozbory/prehled stavy stroju TISK zpl zaznam: " + index_zaznamu.ToString() + "-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END vyroba/rozbory/prehled stavy stroju TISK zpl zaznam: " + index_zaznamu.ToString() + "-------------------------------------");
                ...
                else
                {
                    ...
                    Tisk_selected_rows_Primy_Tisk(plr_Path, true, MN_ToTisk, MachineStateSetHistory_selectedRows, NazevVychoziTiskarny);
        ...
        private void Tisk_selected_rows_Primy_Tisk(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow> FASK_ZASOBY_selected_Rows, string nazevVychTiskarny)
        {
            try
            {
        ...
        private void PrintReport_RDLC(Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryDataTable dt, bool primyTisk, string path_sablona, string nazevVychTiskarny, int pocetVytisku)
        {
            try
            {
                ...
                if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                {
                    foreach (var item in dt)
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start vyroba/rozbory/prehled stavy stroju TISK rdlc-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END vyroba/rozbory/prehled stavy stroju TISK rdlc-------------------------------------");
                    }
                }
        ...
        private void Perform_Tisk_Selected(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                else
                {
                    ...
                    Tisk_selected_rows(plr_Path, true, MN_ToTisk, MachineStateSetHistory_selectedRows);
        ...
        private void Tisk_selected_rows(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow> FASK_ZASOBY_selected_Rows)
        {
            try
            {
        ...
        private string PrepareDataToTisk(
            string strPath,
            Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow row,
            int? mnozstviDoTisku
            )
        {
            try
        ...
        private Dictionary<string, string> PrepareDataToTisk(Fask.Interfaces.DataSets.Vyroba.MachineStateSetHistoryRow productionRow_data)
        {
            try
            {
                while (true)
