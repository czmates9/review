...
namespace Konzola.Ciselniky
{
    ...
    /// </summary>
    public partial class FormLokaceMapaList : Form
    {
        ...
        public FormLokaceMapaList(bool allowMultiSelect, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni, Opravneni opravneni)
        {
            InitializeComponent();
        ...
        public FormLokaceMapaList(bool allowMultiSelect, Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow selectRow, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni, Opravneni opravneni)
        ...
        /// </summary>
        private List<Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr> filtry = new List<Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr>();

        /// <summary>
        /// Vybrany filtr.
        /// </summary>
        private Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr rowFiltr
        {
            get
            {
                try
                {
                    return tscbFiltry.SelectedItem as Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr;
                }
                catch
                {
                    return null;
        ...
        /// </summary>
        private Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow selectRow { get; set; }

        /// <summary>
        /// Vybrany radek.
        /// </summary>
        public Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow Lokace_Mapa_selectedRow
        {
            get
            {
                try
                {
                    return ((DataRowView)(dgSkladLokace.BindingContext[bsSkladLokace].Current)).Row as Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow;
                }
                catch
                {
                    return null;
        ...
        private List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow> Lokace_Mapa_selectedRows
        {
            //get
            //{
            //    List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow> rows = new List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow>();

            //    foreach (DataGridViewRow selectedRow in dgSkladLokace.SelectedRows)
            //    {
            //        Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow row = ((DataRowView)selectedRow.DataBoundItem).Row as Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow;
            //        rows.Add(row);
            //    }
            ...
            get
            {
                return dgSkladLokace.SelectedRows
                    ...
                    .Select(r => ((DataRowView)r.DataBoundItem).Row as Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow)
                    .Where(r => r != null)
                    .ToList();
            }
        }
        ...
        public FormLokaceMapaList(bool allowMultiSelect, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni)
        {
            InitializeComponent();
            this.dgSkladLokace.UpdateColumnHeaderCellsByDatasource();
            this.dgSkladLokace.MultiSelect = allowMultiSelect;
        ...
        /// <param name="allowMultiSelect">Povoleni oznaceni vice zaznamu.</param>
        /// <param name="selectRow">Radek, ktery se oznaci.</param>
        public FormLokaceMapaList(bool allowMultiSelect, Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow selectRow, Fask.Interfaces.Classes.ZOBRAZENI_TYP typZobrazeni)
            : this(allowMultiSelect, typZobrazeni)
        {
            this.selectRow = selectRow;
        }
        ...
        private void FormUzivateleList_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.filtry = Extensions.ListExt.ReadFromXML<Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr>(this.GetType().ToString() + ".filtr");
        ...
        private void FormUzivateleList_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                try
                {
                    if (bwLoadSkladLokaceMapa.IsBusy)
                    {
                        bwLoadSkladLokaceMapa.CancelAsync();
                        while (bwLoadSkladLokaceMapa.IsBusy)
                        {
                ...
                if (providerSkladLokace == null) //inicializace se provede pouze pokud nebyla provedena ... 
                {
                    ...
                    foreach (Type t in types)
                    {
                        try
                        {
                            ...
                            {
                                providerSkladLokace = (Fask.Interfaces.Ciselniky.SkladLokace_Mapa.ISkladLokace_Mapa2)providerAssemlby.CreateInstance(t.FullName);
                                if (providerSkladLokace != null)
                                    break;
                            }
                        }
        ...
        private void PerformOK()
        {
            try
            {
                ...
                if (Lokace_Mapa_selectedRow == null)
                {
        ...
        private void dataGridView1_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                if (e.RowIndex == -1)
                {
                    if (Lokace_Mapa_selectedRow != null)
                    {
                        selectedSortSklID = Lokace_Mapa_selectedRow.SKL_ID;
                        selectedSortLocncode = Lokace_Mapa_selectedRow.IsLOCNCODENull() ? string.Empty : Lokace_Mapa_selectedRow.LOCNCODE;
                    }
                }
            }
            catch
        ...
        private void dataGridView1_Sorted(object sender, EventArgs e)
        {
            try
            {
                ...
                int pos = bsSkladLokace.Find(
                    new Key { PropertyName = dsSkladLokace.CZMST_SkladLokace_Mapa.SKL_IDColumn.ColumnName, Value = selectedSortSklID },
                    new Key { PropertyName = dsSkladLokace.CZMST_SkladLokace_Mapa.LOCNCODEColumn.ColumnName, Value = selectedSortLocncode });
        ...
        private void bwLoadZbozi_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr filtr = (Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr)e.Argument;
                Fask.Interfaces.DataSets.SkladLokace ds = new Fask.Interfaces.DataSets.SkladLokace();

                if (bwLoadSkladLokaceMapa.CancellationPending)
                {
                    e.Cancel = true;
                    return;
                }
                ...
                if ((providerSkladLokace != null) && (providerSkladLokace is Fask.Interfaces.Ciselniky.SkladLokace_Mapa.ISkladLokace_Mapa2_GetFiltrovaneSkladLokace_Mapa))
                    ds = ((Fask.Interfaces.Ciselniky.SkladLokace_Mapa.ISkladLokace_Mapa2_GetFiltrovaneSkladLokace_Mapa)providerSkladLokace).GetFiltrovaneSkladLokace_Mapa(filtr);
                else
        ...
        private void PerformVyhledat()
        {
            try
            {
                ...
                Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr filtr = new Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr();
                ...
                bwLoadSkladLokaceMapa.RunWorkerAsync(filtr);
        ...
        /// <returns></returns>
        private bool CreateFilter(ref Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr filtr)
        {
            if (filtr == null)
                filtr = new Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr();
        ...
        /// </summary>
        /// <param name="filtr"></param>
        private void PerformNastavitFiltr(Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr filtr)
        {
            try
            {
                if (filtr == null)
        ...
        private void PerformZmenitFiltr()
        {
            try
            {
                ...
                Fask.Interfaces.Filtry.SkladLokaceMapaListFiltr filtr = rowFiltr;
        ...
        private void PerformCreateRecord()
        {
            try
            {
                ...
                using (Ciselniky.FormLokaceMapaEdit frm = new FormLokaceMapaEdit())
                {
                    ...
                    // pridat ...
                    this.dsSkladLokace.CZMST_SkladLokace_Mapa.ImportRow(frm.returnrow);
                    ...
                    try
                    {
                        this.bsSkladLokace.Position = dsSkladLokace.CZMST_SkladLokace_Mapa.Count - 1;
                    }
                    catch { }
                }
            }
        ...
        private void PerformEditRecord()
        {
            try
            {
                ...
                using (Ciselniky.FormLokaceMapaEdit frm = new FormLokaceMapaEdit())
                {
                    frm.lokacerow = Lokace_Mapa_selectedRow;
                    ...
                    if (frm.returnrow.IsSKL_IDNull())
                        this.Lokace_Mapa_selectedRow.SetSKL_IDNull();
                    else this.Lokace_Mapa_selectedRow.SKL_ID = frm.returnrow.SKL_ID;
                    //this.SelectedRow.FIRSTNAME= frm.returnrow.FIRSTNAME;
                    if (frm.returnrow.IsLOCNCODENull())
                        this.Lokace_Mapa_selectedRow.SetLOCNCODENull();
                    else this.Lokace_Mapa_selectedRow.LOCNCODE= frm.returnrow.LOCNCODE;
                    if (frm.returnrow.IsTYPENull())
                        this.Lokace_Mapa_selectedRow.SetTYPENull();
                    else this.Lokace_Mapa_selectedRow.TYPE = frm.returnrow.TYPE;
                    if (frm.returnrow.IsBarcodeNull())
                        this.Lokace_Mapa_selectedRow.SetBarcodeNull();
                    else this.Lokace_Mapa_selectedRow.Barcode= frm.returnrow.Barcode;
                    if (frm.returnrow.IsDescriptionNull())
                        this.Lokace_Mapa_selectedRow.SetDescriptionNull();
                    else this.Lokace_Mapa_selectedRow.Description = frm.returnrow.Description;                    
                    this.Lokace_Mapa_selectedRow.DEX_ROW_ID = frm.returnrow.DEX_ROW_ID;
        ...
        private void PerformDeleteRecord()
        {
            try
            {
                ...
                Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable dtt = new Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable();

                if (SelectedRows.Count > 0)
                {
                   

                    Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable dt = ((Fask.Interfaces.DataSets.SkladLokace)bsSkladLokace.DataSource).CZMST_SkladLokace_Mapa;
                    ...
                    foreach (Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow item in dtt)
                    {
                        ...
                        if ((providerSkladLokace != null) && (providerSkladLokace is Fask.Interfaces.Ciselniky.SkladLokace_Mapa.ISkladLokace_Mapa2_DeleteSkladLokace_Mapa))
                            result = ((Fask.Interfaces.Ciselniky.SkladLokace_Mapa.ISkladLokace_Mapa2_DeleteSkladLokace_Mapa)providerSkladLokace).DeleteSkladLokace_Mapa(item.IsSKL_IDNull() ? string.Empty : item.SKL_ID, item.IsLOCNCODENull() ? string.Empty : item.LOCNCODE);
                        else
        ...
        private void tiskVseToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DialogResult dr = MessageBox.Show(string.Format("Opravdu tisknout vše? Počet:{0}", this.dsSkladLokace.CZMST_SkladLokace_Mapa.Count), this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);


                if (dr == System.Windows.Forms.DialogResult.Yes)
                {
                    if (this.dsSkladLokace.CZMST_SkladLokace_Mapa.Count > 0)
                        Tisk(this.dsSkladLokace.CZMST_SkladLokace_Mapa, true);
                    else
                    {
                        MessageBox.Show("Nenalezena data k tisku.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    } 
        ...
        private void Tisk(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable dt, bool MnozstvuAutoJedna)
        {
            try
            {
        ...
        private void label1_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (Lokace_Mapa_selectedRow != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(Lokace_Mapa_selectedRow.LOCNCODE))
                    {
                        ZXing.BarcodeWriter bw = new ZXing.BarcodeWriter();
                        bw.Options.Width = 400;
                        bw.Options.Height = 400;
                        bw.Format = ZXing.BarcodeFormat.QR_CODE;
                        Bitmap bmp = bw.Write(Lokace_Mapa_selectedRow.LOCNCODE.Trim());
        ...
        private void label3_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (Lokace_Mapa_selectedRow != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(Lokace_Mapa_selectedRow.SKL_ID))
                    {
                        ZXing.BarcodeWriter bw = new ZXing.BarcodeWriter();
                        bw.Options.Width = 400;
                        bw.Options.Height = 400;
                        bw.Format = ZXing.BarcodeFormat.QR_CODE;
                        Bitmap bmp = bw.Write(Lokace_Mapa_selectedRow.SKL_ID.Trim());
        ...
        private void PerformPrint()
        {
            try
            {
                ...
                if (Lokace_Mapa_selectedRows.Count > 1)
                {
                    //MessageBox.Show("Je vybráno více záznamů než jeden.", this.Text, MessageBoxButtons.OK);
                    //return;
                }

                Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable dt = new Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable();

                foreach (Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow row in Lokace_Mapa_selectedRows)
                {
                    try
                    {
                        Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow newRow = dt.NewCZMST_SkladLokace_MapaRow();
                        ...
                        dt.AddCZMST_SkladLokace_MapaRow(newRow);
                    }
                    catch (Exception ex)
                    {
                        // Ošetření vyjimky
        ...
        private void PrintReport(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable dt)
        {
            try
            {
                ...
                if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                {
                    ...
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start sklady,mapa lokaci TISK rdlc-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END sklady,mapa lokaci TISK rdlc-------------------------------------");
                    } 
                }
        ...
        private void Perform_Tisk_Selected_Primy_Tisk(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                foreach (var item in Lokace_Mapa_selectedRows)
                {

                    if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start sklady/ciselniky/mapa lokace TISK zpl zaznam: " + index_zaznamu.ToString() + "-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END sklady/ciselniky/mapa lokace TISK zpl zaznam: " + index_zaznamu.ToString() + "-------------------------------------");
                ...
                else
                {
                    ...
                    Tisk_selected_rows_Primy_Tisk(plr_Path, true, MN_ToTisk, Lokace_Mapa_selectedRows, NazevVychoziTiskarny);
        ...
        private void Tisk_selected_rows_Primy_Tisk(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow> selected_Rows, string nazevVychTiskarny)
        {
            try
            {
        ...
        private void PrintReport_RDLC(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaDataTable dt, bool primyTisk, string path_sablona, string nazevVychTiskarny, int pocetVytisku)
        {
            try
            {
                ...
                if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                {
                    foreach (var item in dt)
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start sklady/ciselniky/mapa lokace TISK rdlc-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END sklady/ciselniky/mapa lokace TISK rdlc-------------------------------------");
                    }
                }
        ...
        private void Perform_Tisk_Selected(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                else
                {
                    ...
                    Tisk_selected_rows(plr_Path, true, MN_ToTisk, Lokace_Mapa_selectedRows);
        ...
        private void Tisk_selected_rows(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow> selected_Rows)
        {
            try
            {
                PrintDialog printDialog1 = new PrintDialog();
        ...
        private Dictionary<string, string> PrepareDataToTisk(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_MapaRow row_data)
        {
            try
            {
                while (true)
        ...
        private bool ImportDatZFileCSV(List<Tuple<string, string, bool>> tableInfo)
        {
            ...
            try
            {
                ...
                uspech = this.dgSkladLokace.ImportzCSV_SkladLokace_Mapa(tableInfo);
                ...
                using (FormImport form = new FormImport("Importovat", "Procedura", "Soubor CSV"))
                {
                    ...
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        else if (selectedOption == "SQL_Procedura")
                        {
                            ...
                            //DataTable dtchanged = this.dsStrediska.CZMST091.GetChanges();
                            //if (dtchanged != null && dtchanged.Rows.Count > 0) //obsahuje zmeny ... 
                            //{
                            //    DialogResult dr = MessageBox.Show("Obsahuje změny, pokračovat?", "data", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                            //    if (dr == System.Windows.Forms.DialogResult.No)
                            ...
                            //int FirstDisplayedScrollingRowIndex = this.dgStrediska.FirstDisplayedScrollingRowIndex; //Save Current Scroll Index

                            //bwImport.RunWorkerAsync();

                            //if ((FirstDisplayedScrollingRowIndex >= 0) && ((this.dgStrediska.Rows.Count - 1) >= FirstDisplayedScrollingRowIndex)) this.dgStrediska.FirstDisplayedScrollingRowIndex = FirstDisplayedScrollingRowIndex; //Restore Scroll Index
                        }
                        else
                        {
