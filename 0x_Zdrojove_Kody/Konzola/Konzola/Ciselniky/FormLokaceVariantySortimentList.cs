...
{
    public partial class FormLokaceVariantySortimentList : Form
    {
        ...
        public FormLokaceVariantySortimentList(Opravneni opravneni)
        {
            InitializeComponent();
        ...
        //private Fask.Interfaces.Ciselniky.ISklady providerSklady = null;
        private Fask.Interfaces.IMES providerVariantySortiment = null;

        private List<Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr> filtry = new List<Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr>();
        //private Fask.Interfaces.DataSets.Sklady dsSklady = new Fask.Interfaces.DataSets.Sklady();

        /// <summary>
        /// Vybrany filtr.
        /// </summary>
        private Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr rowFiltr
        {
            get
            {
                try
                {
                    return tscbFiltry.SelectedItem as Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr;
                }
                catch
                {
                    return null;
        ...
        public Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow CZMST_SkladLokace_LokaceVariantySortiment_selectedRow
        {
            get
            {
                try
                {
                    return ((DataRowView)(dgSkladLokace.BindingContext[bsSkladLokace].Current)).Row as Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow;
        ...
        private List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow> CZMST_SkladLokace_LokaceVariantySortiment_selectedRows
        {
            //get
            //{
            //    List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow> rows = new List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow>();

            //    foreach (DataGridViewRow selectedRow in dgSkladLokace.SelectedRows)
            //    {
            //        Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow row = ((DataRowView)selectedRow.DataBoundItem).Row as Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow;
            //        rows.Add(row);
            //    }
            ...
            get
            {
                return dgSkladLokace.SelectedRows
                    ...
                    .Select(r => ((DataRowView)r.DataBoundItem).Row as Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow)
                    .Where(r => r != null)
                    .ToList();
            }
        }
        ...
        public FormLokaceVariantySortimentList()
        {
            InitializeComponent();
            this.dgSkladLokace.UpdateColumnHeaderCellsByDatasource();
            panelButtons.Menu = menuStrip1;
        ...
        private void FormLokaceVariantySortimentList_Load(object sender, EventArgs e)
        {
            try
            {
                this.ShowIcon = false;
                ...
                this.filtry = Extensions.ListExt.ReadFromXML<Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr>(this.GetType().ToString() + ".filtr");
        ...
        private void FormLokaceVariantySortimentList_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        private void FormLokaceVariantySortimentList_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void FormLokaceVariantySortimentList_Shown(object sender, EventArgs e)
        {
            try
            {
        ...
        private void LoadDataAsync()
        {
            try
            {
                Fask.Interfaces.DataSets.SkladLokace dsSkladLokace = new Fask.Interfaces.DataSets.SkladLokace();
                
                // naplneni ciselniku skladu
                //dsSkladLokace = ((Fask.Interfaces.Ciselniky.ISkladLokace_LokaceVariantySortiment)providerVariantySortiment).GetSkladLokace_LokaceVariantySortiment();

                if ((providerVariantySortiment != null) && (providerVariantySortiment is Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_GetSkladLokace_LokaceVariantySortiment))
                    dsSkladLokace = ((Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_GetSkladLokace_LokaceVariantySortiment)providerVariantySortiment).GetSkladLokace_LokaceVariantySortiment();
                else
                    throw new NotImplementedException("Provider neimplementuje ISkladLokace_LokaceVariantySortiment2_GetSkladLokace_LokaceVariantySortiment.");
        ...
        private void PopulateUI(Fask.Interfaces.DataSets.SkladLokace dsSkladyLokaceData)
        {
            try
            {
                // naplneni a zobrazeni dat
                if (dsSkladyLokaceData == null)
                    dsSkladyLokaceData = new Fask.Interfaces.DataSets.SkladLokace();

                this.dsSkladLokace = dsSkladyLokaceData;
                this.bsSkladLokace.DataSource = this.dsSkladLokace;
                //cbMaterialSKLID.Items.AddRange(dsSkladyLokaceData.CZMST093.Select(null, "skl_desc asc"));
                //cbMaterialSKLID.SelectedItem = null;
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(ex);
        ...
        /// <returns></returns>
        private bool CreateFilter(ref Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr filtr)
        {
            if (filtr == null)
                filtr = new Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr();
            ...
            filtr.MaterialTypLokace = cbTypLokace.Text.Trim();
            filtr.MaterialSKLID = cbMaterialSKLID.Text.Trim();
            filtr.MaterialLocncode = cbMaterialLOCNCODE.Text.Trim();
        ...
        private void PerformOK()
        {
            try
            {
                DataTable dtchanged = this.dsSkladLokace.CZMST_SkladLokace_LokaceVariantySortiment.GetChanges();
        ...
        private void buttonOdznacitVse_Click(object sender, EventArgs e)
        {
            try
            {
                this.dgSkladLokace.ClearSelection();
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        private void bwSkladPohyb_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                //bwLoadSkladPohyb.CancelAsync();
                Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr filtr = (Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr)e.Argument;
                ...
                //ds = ((Fask.Interfaces.Ciselniky.ISkladLokace_LokaceVariantySortiment)providerVariantySortiment).GetFiltrovanySkladLokace_LokaceVariantySortiment(filtr);


                if ((providerVariantySortiment != null) && (providerVariantySortiment is Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_GetFiltrovanySkladLokace_LokaceVariantySortiment))
                    ds = ((Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_GetFiltrovanySkladLokace_LokaceVariantySortiment)providerVariantySortiment).GetFiltrovanySkladLokace_LokaceVariantySortiment(filtr);
                else
                    throw new NotImplementedException("Provider neimplementuje ISkladLokace_LokaceVariantySortiment2_GetFiltrovanySkladLokace_LokaceVariantySortiment.");
        ...
        private void PerformZmenitFiltr()
        {
            try
            {
                ...
                Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr filtr = rowFiltr;
        ...
        /// </summary>
        /// <param name="filtr"></param>
        private void PerformNastavitFiltr(Fask.Interfaces.Filtry.LokaceVariantySortimentListFiltr filtr)
        {
            try
            {
                if (filtr == null)
                ...
                cbMaterialITEMNMBR.Text = null;
                cbMaterialOznaceni.Text = null;
                cbMaterialITEMCODE.Text = null;
                cbMaterialBarcode.Text = null;
                cbTypLokace.Text = null;
                cbMaterialSKLID.Text = null;
                cbMaterialLOCNCODE.Text = null;

                cbMaterialITEMNMBR.Text = string.Empty;
                cbMaterialOznaceni.Text = string.Empty;
                cbMaterialITEMCODE.Text = string.Empty;
                cbMaterialBarcode.Text = string.Empty;
                cbTypLokace.Text = string.Empty;
                cbMaterialSKLID.Text = string.Empty;
                cbMaterialLOCNCODE.Text = string.Empty;

                cbMaterialITEMNMBR.Text = filtr.MaterialID;
                cbMaterialOznaceni.Text = filtr.MaterialNazev;
                cbMaterialITEMCODE.Text = filtr.MaterialItemcode;
                cbMaterialBarcode.Text = filtr.MaterialBarcode;
                cbTypLokace.Text = filtr.MaterialTypLokace;
                cbMaterialSKLID.Text = filtr.MaterialSKLID;
                cbMaterialLOCNCODE.Text = filtr.MaterialLocncode;
                ...
                //    if (sklady.Count() > 0)
                //        cbMaterialSKLID.SelectedItem = sklady.First();
                //    else
                //        cbMaterialSKLID.Text = filtr.rowMaterialSKLID;
                //}
            }
            catch (Exception ex)
            {
        ...
        private void PerformCreateRecord()
        {
            try
            {
                ...
                using (Ciselniky.FormLokaceVariantySortimentEdit frmodberatele = new FormLokaceVariantySortimentEdit())
                {
                    frmodberatele.Text = "Nová varianta lokace sortimentu";
        ...
        private void PerformEditRecord()
        {
            try
            {
                ...
                if (CZMST_SkladLokace_LokaceVariantySortiment_selectedRow == null)
                {
                ...
                using (Ciselniky.FormLokaceVariantySortimentEdit frmodberatele = new FormLokaceVariantySortimentEdit())
                {
                    frmodberatele.variantarow = CZMST_SkladLokace_LokaceVariantySortiment_selectedRow;
        ...
        private void PerformDeleteRecord()
        {
            try
            {
                ...
                if (this.dgSkladLokace.SelectedRows.Count > 1)
                {
                    MessageBox.Show("Je možné odstraňovat pouze po jednom záznamu", this.Text, MessageBoxButtons.OK);
                    return;
                }
                ...
                if (MessageBox.Show("Opravdu chcete odstranit vybraný záznam?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    return;

                //bool result = ((Fask.Interfaces.Ciselniky.ISkladLokace_LokaceVariantySortiment)providerVariantySortiment).DeleteSkladLokace_LokaceVariantySortiment(SelectedRow.SKL_ID, SelectedRow.LOCNCODE, SelectedRow.ITEMNMBR);
                bool result;

                if ((providerVariantySortiment != null) && (providerVariantySortiment is Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_DeleteSkladLokace_LokaceVariantySortiment))
                    result = ((Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_DeleteSkladLokace_LokaceVariantySortiment)providerVariantySortiment).DeleteSkladLokace_LokaceVariantySortiment(CZMST_SkladLokace_LokaceVariantySortiment_selectedRow.SKL_ID, CZMST_SkladLokace_LokaceVariantySortiment_selectedRow.LOCNCODE, CZMST_SkladLokace_LokaceVariantySortiment_selectedRow.ITEMNMBR);
                else
                    throw new NotImplementedException("Provider neimplementuje ISkladLokace_LokaceVariantySortiment2_DeleteSkladLokace_LokaceVariantySortiment.");



                UpdateForm(CZMST_SkladLokace_LokaceVariantySortiment_selectedRow);
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
        ...
        /// </summary>
        private void UpdateForm(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow row)
        {
            try
            {
                int id = (row != null && !row.IsDEX_ROW_IDNull()) ? row.DEX_ROW_ID : 0;
                ...
                //dsSkladLokace = ((Fask.Interfaces.Ciselniky.ISkladLokace_LokaceVariantySortiment)providerVariantySortiment).GetFiltrovanySkladLokace_LokaceVariantySortiment(filtr);

                if ((providerVariantySortiment != null) && (providerVariantySortiment is Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_GetFiltrovanySkladLokace_LokaceVariantySortiment))
                    dsSkladLokace = ((Fask.Interfaces.Ciselniky.SkladLokace_LVS.ISkladLokace_LVS2_GetFiltrovanySkladLokace_LokaceVariantySortiment)providerVariantySortiment).GetFiltrovanySkladLokace_LokaceVariantySortiment(filtr);
                else
                    throw new NotImplementedException("Provider neimplementuje ISkladLokace_LokaceVariantySortiment2_GetFiltrovanySkladLokace_LokaceVariantySortiment.");
                ...
                try
                {
                    ...
                    int index = bsSkladLokace.Find(dsSkladLokace.CZMST_SkladLokace_LokaceVariantySortiment.DEX_ROW_IDColumn.ColumnName, id);
        ...
        private void PerformPrint()
        {
            try
            {
                ...
                if (CZMST_SkladLokace_LokaceVariantySortiment_selectedRows.Count > 1)
                {
                    //MessageBox.Show("Je vybráno více záznamů než jeden.", this.Text, MessageBoxButtons.OK);
                    //return;
                }

                Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentDataTable dt = new Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentDataTable();

                foreach (Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow row in CZMST_SkladLokace_LokaceVariantySortiment_selectedRows)
                {
                    try
                    {
                        Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow newRow = dt.NewCZMST_SkladLokace_LokaceVariantySortimentRow();
                        ...
                        dt.AddCZMST_SkladLokace_LokaceVariantySortimentRow(newRow);
                    }
                    catch (Exception ex)
                    {
                        // Ošetření vyjimky
        ...
        private void PrintReport(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentDataTable dt)
        {
            try
            {
                ...
                if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                {
                    ...
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start sklady,varianty lokaci TISK rdlc-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END sklady,varianty lokaci TISK rdlc-------------------------------------");
                    }
        ...
        private void Perform_Tisk_Selected_Primy_Tisk(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                foreach (var item in CZMST_SkladLokace_LokaceVariantySortiment_selectedRows)
                {

                    if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start ciselniky/varianty lokaci TISK zpl zaznam: " + index_zaznamu.ToString() + "-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END ciselniky/varianty lokaci TISK zpl zaznam: " + index_zaznamu.ToString() + "-------------------------------------");
                ...
                else
                {
                    ...
                    Tisk_selected_rows_Primy_Tisk(plr_Path, true, MN_ToTisk, CZMST_SkladLokace_LokaceVariantySortiment_selectedRows, NazevVychoziTiskarny);
        ...
        private void Tisk_selected_rows_Primy_Tisk(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow> FASK_ZASOBY_selected_Rows, string nazevVychTiskarny)
        {
            try
            {
        ...
        private void PrintReport_RDLC(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentDataTable dt, bool primyTisk, string path_sablona, string nazevVychTiskarny, int pocetVytisku)
        {
            try
            {
                ...
                if (Konfigurace.Globals_Konfig_Konzola.Konfigurace.System[0].Log_Tisky_hl)
                {
                    foreach (var item in dt)
                    {
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------Start ciselniky/varianty lokaci TISK rdlc-------------------------------------");
                        ...
                        Fask.Logging.ExceptionHandler2.Handle(LogLevel.PrintInfo, "---------------------------------------END ciselniky/varianty lokaci TISK rdlc-------------------------------------");
                    }
                }
        ...
        private void Perform_Tisk_Selected(string klicTyp, string NazevVychoziTiskarny, string pocetVytisku)
        {
            try
            {
                ...
                else
                {
                    ...
                    Tisk_selected_rows(plr_Path, true, MN_ToTisk, CZMST_SkladLokace_LokaceVariantySortiment_selectedRows);
        ...
        private void Tisk_selected_rows(string plr_Path, bool MnozstvuAutoJedna, int MN_ToTisk, List<Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow> FASK_ZASOBY_selected_Rows)
        {
            try
            {
        ...
        private string PrepareDataToTisk(
            string strPath,
            Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow row,
            int? mnozstviDoTisku
            )
        {
            try
        ...
        private Dictionary<string, string> PrepareDataToTisk(Fask.Interfaces.DataSets.SkladLokace.CZMST_SkladLokace_LokaceVariantySortimentRow productionRow_data)
        {
            try
            {
                while (true)
