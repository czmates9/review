...
{
    public partial class FormUkolyHistoryList : Form
    {
        ...
        /// <summary>
        /// Zvolený ukol z historie.
        /// </summary>
        private Fask.Interfaces.DataSets.Ukolovani.CZ_UKOL_UZIV_HISTRow rowUkolUzivHist
        {
            get
            {
                try
                {
                    return ((DataRowView)(dgUkolovani.BindingContext[this.bsUkolovani].Current)).Row as Fask.Interfaces.DataSets.Ukolovani.CZ_UKOL_UZIV_HISTRow;
                }
                catch
                {
                    return null;
        ...
        private Fask.Interfaces.DataSets.Ukolovani.CZ_UKOLRow rowUkol
        {
            get
            {
                try
                {
                    //return ((DataRowView)(comboBoxUzivatel.SelectedItem)).Row as Fask.Interfaces.DataSets.Vyroba.LoginsRow;
        ...
        private void FormUkolyHistoryList_Load(object sender, EventArgs e)
        {
            try
            {
                this.ShowIcon = false;
                ...
                //if ((providerUkol != null) && (providerUkol is Fask.Interfaces.Ciselniky.IUzivatele2_GetUzivatele))
                //     ds = ((Fask.Interfaces.Ciselniky.IUzivatele2_GetUzivatele)providerUkol).GetUzivatele();
                //else
                //    throw new NotImplementedException("Provider neobsahuje implemetaci IUzivatele2_GetUzivatele");
                ...
                // naplnění priorit
                var dtPrior = this.dsUkolovani.CZ_UKOL.GroupBy(g => g.Priority).Select(s => s.First()).ToList();
                comboBoxPriorita.DataSource = dtPrior;
                ...
                if ((providerUkol != null) && (providerUkol is Fask.Interfaces.Ukolovani.IUkolovani_Fill_CZ_UKOL_STATE))
                    ((Fask.Interfaces.Ukolovani.IUkolovani_Fill_CZ_UKOL_STATE)providerUkol).Fill_CZ_UKOL_STATE(this.dsUkolovani);
                else
        ...
        private void FormUkolyHistoryList_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void FormUkolyHistoryList_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        private void PerformOK()
        {
            try
            {
                System.Data.DataTable dtchanged = this.dsUkolovani.CZ_UKOL_UZIV_HIST.GetChanges();
                ...
                da_filter.SelectCommand.CommandText = "Select u.Name UkolName, u.Description UkolDescription, u.Priority UkolPriority, u.DateCreated UkolDateCreated, u.DateFrom UkolDateFrom, u.DateTo UkolDateTo, l.FIRSTNAME UserFirstname, l.SECONDNAME UserSecondname, l.LOGIN UserLogin ,uhist.* from CZ_UKOL_UZIV_HIST uhist ";
                da_filter.SelectCommand.CommandText += "left join CZMSTPWD l on l.ID = uhist.UserID ";
                da_filter.SelectCommand.CommandText += "left join CZ_UKOL u on u.ID = uhist.UkolID ";
                da_filter.SelectCommand.CommandText += "left join CZ_UKOL_STATE s on s.State = uhist.State ";
                ...
                da_filter.SelectCommand.CommandText += "order by ID_HIST desc";

                int FirstDisplayedScrollingRowIndex = this.dgUkolovani.FirstDisplayedScrollingRowIndex; //Save Current Scroll Index
                this.dsUkolovani.CZ_UKOL_UZIV_HIST.Clear();
                this.dsUkolovani.CZ_UKOL_UZIV_HIST.AcceptChanges();

                this.dsUkolovani.CZ_UKOL_UZIV_HIST.BeginLoadData();

                da_filter.Fill(this.dsUkolovani.CZ_UKOL_UZIV_HIST);

                this.dsUkolovani.CZ_UKOL_UZIV_HIST.EndLoadData();
        ...
        private void buttonOdznacitVse_Click(object sender, EventArgs e)
        {
            try
            {
                this.dgUkolovani.ClearSelection();
            }
            catch (Exception ex)
            {
                Fask.Logging.ExceptionHandler2.Handle(this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
