...
namespace FASK.SledovaniVyroby.ScannerProviderSerialPortQX830
{

    public class ScannerProviderSerialPortQX830 : FASK.SledovaniVyroby.IScannerProvider.IScannerProvider
    {
        private string _configScanner = "ScannerProviderSerialPortQX830.xml";
        ...
        public ScannerProviderSerialPortQX830()
        {
            try
            {
        ...
        public void ScannerSettingLoad()
        {
            ...
            debug = int.Parse(LoadElement(xmldoc, "/Scanner/Debug"));
        }
        ...
        void serialport_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            ...
            {
                //Log.Write("Spatne nacteny kod ze scanneru!" + ex.Message);
                ExceptionHandler2.Handle(ex);
            }
        }

        private void SendCode(object sender, string pom)
        {
            //Code tmpR = Code.NoData;
       
            string barcode = pom.Trim();
            if (barcode.Length == 13)
            {
                //if (!IsAllDigits(barcode))
                //    Log.Write("Dosel spatny carovy kod! EAN: " + barcode);
                if (debug == 1)
                    ExceptionHandler2.HandleCSV(1, barcode);


                //tmpR = Code.Read;
                //tmpNewCode = pom.Trim();
                //nove
                DataReady(sender, new ScannerEventArgs(barcode, 0, "", (uint)barcode.Length, Code.Read));

            }
            else
            {
                if (debug == 1)
                    ExceptionHandler2.HandleCSV(2, barcode);

                //Log.Write("Prichozi kod nema delku rovnou 13! EAN: " + barcode);
                //tmpNewCode = pom.Trim();
                //tmpR = GetRS485Code(pom.Trim(), out tmpNewCode);
                DataReady(sender, new ScannerEventArgs(barcode, 0, "", (uint)barcode.Length, Code.NoRead));
            }
        ...
        public FASK.SledovaniVyroby.IScannerProvider.IScannerProvider Scanner
        {
            get
            {
                throw new NotImplementedException();
