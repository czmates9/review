...
namespace FASK.SledovaniVyroby.ErrorLog
{
	...
	public class Log {
        ...
        public static void Write_ToCustomFile(Exception ex, string FileName = "log_testy.txt")
        {
            StringBuilder sbError = new StringBuilder();

            sbError.AppendLine(string.Format("{0}\n{1}\n{2}", ex.Message, ex.Source, ex.StackTrace));
            string errmsg = sbError.ToString();

            Write_ToCustomFile(errmsg, string.Empty, FileName);
        }

        public static void Write_ToCustomFile(string message, string FileName = "log_testy.txt")
        {
            Write_ToCustomFile(message, string.Empty, FileName);
        }

        public static void Write_ToCustomFile( string message, string context, string FileName = "log_testy.txt")
        {
            if (!_doLogging)
                return;
            ...
            try
            {
                sw = new System.IO.StreamWriter(System.IO.Path.Combine(_dirlog, FileName), true);
                sw.WriteLine(DateTime.Now.ToString() + ": " + (context.Length != 0 ? "(" + context + ") " : context) + message);
            }
            catch
            {
