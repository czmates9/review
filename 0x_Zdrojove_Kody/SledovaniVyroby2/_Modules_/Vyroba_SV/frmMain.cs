...
using FASK.SledovaniVyroby.Module.Vyroba_SV.Vozikova_matice;
using FASK.SledovaniVyroby.Module.Vyroba_SV.Classes;
using FASK.SledovaniVyroby.IRFIDProvider;
using System.Threading;
using Fask.Logging;
using static FASK.SledovaniVyroby.Module.Vyroba_SV.Classes.Logika_Voziky;
...
namespace FASK.SledovaniVyroby.Module.Vyroba_SV
{
    public partial class frmMain : Form, IModuleConnector
    {
        ...
        private int rfid_pocet = 0;
        private System.Threading.Timer timer_rfid_linka;
        ...
        private bool priznakFalNakladky = false;
        ...
        public object balik_TAG_lock = new object();
        List<Bag_Tags> balikTagu = null;

        #endregion

        #region info + paralelni proces

        private bool paralelniProcesZapnut = false;
        ...
        public void ParalelniProcesy(bool start = false)
        {
            if(start)
            {
                if(paralelniProcesZapnut)
                {

                    this.UpdateStatusBarInfo("Info proces: Nefunkční " + DateTime.Now.ToString());
                    Show_ERROR_LED(1);
                }
                else
                {
                    this.UpdateStatusBarInfo("Info proces: Spuštěn - POZOR! " + DateTime.Now.ToString());
                    Show_ERROR_LED(3);


                    paralelniProcesZapnut = true;
                }
            }
            else
            {
                ...
                if (paralelniProcesZapnut)
                {
                    ...
                    paralelniProcesZapnut = false;
                    this.UpdateStatusBarInfo("Info proces: OK " + DateTime.Now.ToString());
                    Show_ERROR_LED(2);
                ...
                {
                    paralelniProcesZapnut = false;
                   // this.UpdateStatusBarInfo("Info proces: již byl dokončen? " + DateTime.Now.ToString());
                }

            }
        }

        public void Show_ERROR_LED(int barva)
        {
            ...
            if (barva == 1)
            {
                btn_ERROR_LED.BackColor = Color.Red;
                HoukackaRizeni(HoukackaStav.ON);
                houkackaState = true;
                btn_ERROR_LED.Enabled = true;
                btn_ERROR_LED.Text = "!";

                // adam_1.zapniHoukacku();
                //  houkackaState = true;
            }
            else if (barva == 2)
            {
                btn_ERROR_LED.BackColor = Color.Green;
                btn_ERROR_LED.Text = string.Empty;
                adam_1.vypniHoukacku();
                houkackaState = false;
                //Show_ERROR_LED(55);
                btn_ERROR_LED.Enabled = false;
                HoukackaRizeni(HoukackaStav.OFF);
            }
            else if (barva == 3)
            {
                btn_ERROR_LED.BackColor = Color.Orange;
                btn_ERROR_LED.Text = string.Empty;
                //TODO MaR 2.5.2024 sepnuti rele !! dodelat
            }
            else
            {
                btn_ERROR_LED.BackColor = Color.White;
                btn_ERROR_LED.Text = string.Empty;
            }
        ...
        private bool houkackaStateTest = false;
        private void btn_houkackaTest_Click(object sender, EventArgs e)
        {
            if (houkackaStateTest)
            {

                adam_1.vypniHoukacku();
                houkackaStateTest = false;
                //Show_ERROR_LED(55);
               // btn_ERROR_LED.Enabled = false;
        ...
        private void btn_houkacka_Click(object sender, EventArgs e)
        {
            if (houkackaState)
            {
       ...
       // private int _HoukackaPocetNepruchoduMax = AgroConfig.config.Agro[0].PocetNepruchoduSepnutiReleHoukacky;
        ...
        #region Event overeni nakladky

        public delegate void Nakladka_EventHandler(Object sender, Nakladka_EventArgs e);

        public event Nakladka_EventHandler nakladka_EventHandler;

        private void OnNakladkaEvent(Nakladka_EventArgs e)
        {
            Nakladka_EventHandler handler = nakladka_EventHandler;
...
#region RFID inicializace User Control
                ...
                rfiD_UC2.Tag_Text_R = NenactenoEPC;
                rfiD_UC2.Tag_Text_W = NenactenoEPC;
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            nakladka_EventHandler += FrmMain_nakladka_EventHandler;

            timer_rfid_linka = new System.Threading.Timer(CallBack_Timer_Rfid_Linka, null, System.Threading.Timeout.Infinite, System.Threading.Timeout.Infinite);
        ...
        private void CallBack_Timer_Rfid_Linka(object state)
        {
            try
            {
                ...
                //    foreach (var ttag in balikTagu)
                //    {
                //        string rfid_hlaska_11 = string.Format("[ EPC {0} Počet: {1} Anténa: {2} ]" + Environment.NewLine, ttag.EPC, ttag.CountTag, ttag.ID_Antena);
                //        ExceptionHandler2.Handle(rfid_hlaska_11, "Log_RFID_Tagy", "txt");
                //    }
                ...
                //string rfid_hlaska_0 = string.Format("----END---- Načteno {0} " + Environment.NewLine , rfid_pocet);
                //ExceptionHandler2.Handle(rfid_hlaska_0, "Log_RFID_Tagy", "txt");
...
#region vizualizace
            ...
            try
            {
                ...
                if (reader == "reader_1")
                {
                    switch (pozice)
                    {
                        case 1:
                            // rfiD_UC1.ChB_R1_ANT1 = true;
                            rfiD_UC2.chB1_ANT_1.BackColor = Color.LightGreen;
                            break;
                        case 2:
                            //  rfiD_UC1.ChB_R1_ANT2 = true;
                            rfiD_UC2.chB1_ANT_2.BackColor = Color.LightGreen;
                            break;
                        case 3:
                            // rfiD_UC1.ChB_R1_ANT3 = true;
                            rfiD_UC2.chB1_ANT_3.BackColor = Color.LightGreen;
                            break;
                        case 4:
                            // rfiD_UC1.ChB_R1_ANT4 = true;
                            rfiD_UC2.chB1_ANT_4.BackColor = Color.LightGreen;
                            break;
                        default:
                            break;
                    }
                ...
                if (reader == "reader_2")
                {
                    switch (pozice)
                    {
                        case 1:
                            // rfiD_UC1.ChB_R2_ANT1 = true;
                            rfiD_UC2.chB2_ANT_1.BackColor = Color.LightGreen;
                            break;
                        case 2:
                            // rfiD_UC1.ChB_R2_ANT2 = true;
                            rfiD_UC2.chB2_ANT_2.BackColor = Color.LightGreen;
                            break;
                        case 3:
                            // rfiD_UC1.ChB_R2_ANT3 = true;
                            rfiD_UC2.chB2_ANT_2.BackColor = Color.LightGreen;
        ...
        private void RFID_Linky_DataReadyZEBRA(object sender, IRFIDProvider.RFID_ZABRA_EventArgs e)
        {
            ...
            #region TEST MaR 29.4.2024 RFID

            //if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vse_ON || AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON)
            if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON || !AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vse_ON)
            {
                ...
                try
                {
                    ...
                    foreach (var item in e.TagIDs)
                    {
                        ...
                        //string rfid_hlaska = string.Format("EPC: {0} Antena ID: {1} Načteno {2}" + Environment.NewLine, item.EPC, item.ID_Antena, rfid_pocet);
                        //ExceptionHandler2.Handle(rfid_hlaska, "Log_RFID_Tagy", "txt");
                    ...
                    lock (balik_TAG_lock)
                    {
                        if (balikTagu != null)
                        {
                            //string rfid_hlaska = string.Format("--------------balikTagu != null-----------------" + Environment.NewLine);
                            //ExceptionHandler2.Handle(rfid_hlaska, "Log_RFID_Tagy", "txt");
                            ...
                            foreach (var item in e.TagIDs)
                            {
                                // Najděte existující záznamy v balíčku tagů, které odpovídají aktuálnímu TagID
                                var existujiciZaznamy = balikTagu.Where(x => x.EPC == item.EPC && x.ID_Antena == item.ID_Antena).ToList();

                                if (existujiciZaznamy.Count == 0)
                                {
                                    // Pokud neexistuje žádný záznam pro aktuální TagID, přidejte ho do balíčku tagů
                                    balikTagu.Add(new Bag_Tags { EPC = item.EPC, ID_Antena = item.ID_Antena, CountTag = 1 });
                                }
                                else
                                {
                                    // Pokud existuje záznam pro aktuální TagID, zvyšte CountTag u kazdeho nalezeneho zaznnamu o +1
                                    //oprav to
                                    foreach (var ttag in balikTagu)
                                    {
                                        if (ttag.EPC == item.EPC)
            ...
            try
            {
                ...
                #region stare RFID MaR 29.4.2024
                if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vse_ON || AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vykladka_ON)
                //if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON)
                {
                    string EPC = string.Empty;
                    string Antena = string.Empty;
        ...
        public void SetText_ToUC(string txt, string source)
        {
            ...
            #region RFID sprava rezim
            if (source == Const_Kontrola_Linky)
                rfiD_UC2.Tag_Text_W = txt;
            else if (source == Const_Zapis_Linky)
                rfiD_UC2.Tag_Text_R = txt;
            else if (source == Const_Zapis_Odvadeni)
                rfiD_UC2.Tag_Text_R = txt;
            else if (source == Const_Kontrola_Odvadeni)
                rfiD_UC2.Tag_Text_W = txt;
        ...
        #region Nakladka events

        private void FrmMain_nakladka_EventHandler(object sender, Nakladka_EventArgs e)
        {
            bool vysledek = false;
            //uspani vlakna
            Thread.Sleep(AgroSledovaniVozikuConfig.config.RFID_sprava[0].cas_rfid + 10);
            string log_hlaska = string.Empty;
            ...
            if (interniCisloLinka == 0)
            {
                ...
                log_hlaska = string.Format("NAKLADKA_INFO - Vyhodnoceni falesne nakladky na lince: {0} -- je falesna, interni cislo linky uz bylo pouzito! ", e.CisloLinky);
                ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
            ...
            else
            {
                ...
                if (balikTagu != null && balikTagu.Count == 0)
                {
                    log_hlaska = string.Format("NAKLADKA_INFO - Vyhodnoceni falesne nakladky na lince: {0} -- je falesna, nebyl lokalizovan TAG! ", e.CisloLinky);
                    ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
                ...
                else
                {
                    ...
                    log_hlaska = string.Format("NAKLADKA_INFO - Vyhodnoceni falesne nakladky na lince: {0} -- TAG byl lokalizovan! ", e.CisloLinky);
                    ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
                    ...
                    lock (balik_TAG_lock)
                    {
                        foreach (var item in balikTagu)
                        {
                            string nakladka_hlaska = string.Format("NAKLADKA_INFO - TAG {0} byl lokalizovan {1}x! ", item.EPC, item.CountTag);
                            ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");

                            int akce_nakladka_TAGY = 9;
                            LV.OnLogsEvent(new Logs_EventArgs(0, akce_nakladka_TAGY, nakladka_hlaska, LoginID, AGRO.status_0));
                        }
                    }
            ...
            if (vysledek && !AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_stara_logika_ON)
            {

                log_hlaska = string.Format("NAKLADKA_INFO - Vyhodnoceni falesne nakladky na lince: {0} -- neni falesna, interni cislo linky nastaveno na {0}! ", e.CisloLinky);
                ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
                ...
                {
                    interniCisloLinka = e.CisloLinky;
                } 
            }
            else if(AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON)
            {

                log_hlaska = string.Format("NAKLADKA_INFO - Vyhodnoceni falesne nakladky na lince: {0} -- STARÁ LOGIKA, interni cislo linky nastaveno na {0}! ", e.CisloLinky);
                ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
        ...
        private void LoadConfiguration()
        {
            try
            {
                ...
                if (AgroSledovaniVozikuConfig.ExistiFile())
                {
                   ...
                   // AgroSledovaniVozikuConfig.config.TEST.AddTESTRow(1000);
...
#region ADAM1
                    ...
                    chb_ADAM1_Com_P2P.Checked = AgroSledovaniVozikuConfig.config.ADAM_1[0].communication_P2P;
                    chb_ADAM1_Com_TCP.Checked = AgroSledovaniVozikuConfig.config.ADAM_1[0].communication_TCP;
                    ...
                    chb_ADAM2_Com_P2P.Checked = AgroSledovaniVozikuConfig.config.ADAM_2[0].communication_P2P;
                    chb_ADAM2_Com_TCP.Checked = AgroSledovaniVozikuConfig.config.ADAM_2[0].communication_TCP;
                    ...
                    chb_ADAM3_Com_P2P.Checked = AgroSledovaniVozikuConfig.config.ADAM_3[0].communication_P2P;
                    chb_ADAM3_Com_TCP.Checked = AgroSledovaniVozikuConfig.config.ADAM_3[0].communication_TCP;
...
#region ADAM DI 1
                    ...
                    tb_ADAM1_Linka1_END.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_END.ToString();
                    tb_ADAM1_Linka1_RUN.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_RUN.ToString();
                    tb_ADAM1_Linka1_OK.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_OK.ToString();
                    tb_ADAM1_Linka1_CALL.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_CALL.ToString();

                    tb_ADAM1_Linka2_END.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_END.ToString();
                    tb_ADAM1_Linka2_RUN.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_RUN.ToString();
                    tb_ADAM1_Linka2_OK.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_OK.ToString();
                    tb_ADAM1_Linka2_CALL.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_CALL.ToString();

                    tb_ADAM1_Linka3_END.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_END.ToString();
                    tb_ADAM1_Linka3_RUN.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_RUN.ToString();
                    tb_ADAM1_Linka3_OK.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_OK.ToString();
                    tb_ADAM1_Linka3_CALL.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_CALL.ToString();

                    tb_ADAM1_L1.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L1.ToString();
                    tb_ADAM1_L2.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L2.ToString();
                    tb_ADAM1_L3.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L3.ToString();
                    tb_ADAM1_L4.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L4.ToString();

                    tb_ADAM1_RucnyVstup_END.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_END.ToString();
                    tb_ADAM1_RucnyVstup_RUN.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_RUN.ToString();
                    tb_ADAM1_RucnyVstup_OK.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_OK.ToString();
                    tb_ADAM1_RucnyVstup_CALL.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_CALL.ToString();

                    tb_ADAM1_Strec1_OK.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_OK.ToString();
                    tb_ADAM1_Strec1_Bit1.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit1.ToString();
                    tb_ADAM1_Strec1_Bit2.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit2.ToString();
                    tb_ADAM1_Strec1_Bit4.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit4.ToString();

                    tb_ADAM1_Strec2_OK.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_OK.ToString();
                    tb_ADAM1_Strec2_Bit1.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_Bit1.ToString();
                    tb_ADAM1_Strec2_Bit2.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_Bit2.ToString();
                    tb_ADAM1_Strec2_Bit4.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_Bit4.ToString();

                    tb_ADAM1_Strec_Vyber.Text = AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec_vyber.ToString();
...
#region WEB API TISK
                    ...
                    chB_APITISK_logs.Checked = AgroSledovaniVozikuConfig.config.WEBAPI_TISK[0].Logging;
                    ...
                    tB_TEST_cas_nakladka.Text = AgroSledovaniVozikuConfig.config.RFID_sprava[0].cas_rfid.ToString();
                    chB_RFID_vse.Checked = AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vse_ON;
                    chB_RFID_nakladka.Checked = AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON;
                    chB_RFID_vykladka.Checked = AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vykladka_ON;
                    chB_RFID_TEST_rezim.Checked = AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_TEST_rezim_ON;
                    chB_RFID_nakladka_overeni.Checked = AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON;
                    chB_RFID_stara_logika.Checked = AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_stara_logika_ON;
        ...
        private void AdamSensorDataReady_1(int sensor)
        {
            ...
            ADAM1_signal = true;
            priznakFalNakladky = false;
            string log_hlaska = string.Empty;

            try
            {
                if (sensor == AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_RUN)
                {
                    ...
                    if ( AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON)
                    {
                        ...
                        //ExceptionHandler2.Handle(rfid_hlaska_0, "Log_RFID_Tagy", "txt");
                        rfid_pocet = 0;
                        string rfid_hlaska = string.Format("----START---- Linka {0}  Načteno {1}" + Environment.NewLine, AGRO.linka_1, rfid_pocet);
                        ExceptionHandler2.Handle(rfid_hlaska, "Log_RFID_Tagy", "txt");

                        timer_rfid_linka.Change(AgroSledovaniVozikuConfig.config.RFID_sprava[0].cas_rfid, System.Threading.Timeout.Infinite);
                    ...
                    else if(AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vse_ON)
                    {
                        #region obsluzne rutiny - 1) zapnuti RFID cteni/zapis
                    ...
                    #region Overeni falesne nakladky
                    //vyhodit proces pro zpracovani dat cislo nakladky vs hodnota RFID
                    ...
                    else if(interniCisloLinka == AGRO.linka_1)
                    {
                        //beze zmeny
                    }
                    else
                    {
                        ...
                        if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON)
                        {
                            //priznak spatne nakladky
                            //priznakFalNakladky = true;

                            log_hlaska = string.Format("NAKLADKA_INFO - PŘÍZNAK falešné nakládky z linky: {1}! start vyhodnoceni nakladky na lince: {0}", AGRO.linka_1, interniCisloLinka);
                            ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");

                            int akce_nakladka = 7;
                            LV.OnLogsEvent(new Logs_EventArgs(0, akce_nakladka, log_hlaska, LoginID, AGRO.status_0));
                            OnNakladkaEvent(new Nakladka_EventArgs(AGRO.linka_1));
                ...
                else if (sensor == AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_RUN)
                {
                    ...
                    if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON)
                    {
                        ...
                        //ExceptionHandler2.Handle(rfid_hlaska_0, "Log_RFID_Tagy", "txt");
                        rfid_pocet = 0;
                        string rfid_hlaska = string.Format("----START---- Linka {0}  Načteno {1}" + Environment.NewLine, AGRO.linka_2, rfid_pocet);
                        ExceptionHandler2.Handle(rfid_hlaska, "Log_RFID_Tagy", "txt");
                    ...
                    else if (interniCisloLinka == AGRO.linka_2)
                    {
                        //beze zmeny
                    }
                    else
                    {
                        //log_hlaska = string.Format("NAKLADKA_INFO_Linka_[{0}] nebyla vykladka? neni vycistena interniCisloLinka({1})!!", AGRO.linka_2, interniCisloLinka);
                        ...
                        if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON)
                        {
                            ...
                            log_hlaska = string.Format("NAKLADKA_INFO - PŘÍZNAK falešné nakládky z linky: {1}! start vyhodnoceni nakladky na lince: {0}", AGRO.linka_2, interniCisloLinka);
                            ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");

                            int akce_nakladka = 7;
                            LV.OnLogsEvent(new Logs_EventArgs(0, akce_nakladka, log_hlaska, LoginID, AGRO.status_0));
                            OnNakladkaEvent(new Nakladka_EventArgs(AGRO.linka_2));
                ...
                else if (sensor == AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_RUN)
                {
                    ...
                    if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON)
                    {
                        ...
                        //ExceptionHandler2.Handle(rfid_hlaska_0, "Log_RFID_Tagy", "txt");
                        rfid_pocet = 0;
                        string rfid_hlaska = string.Format("----START---- Linka {0}  Načteno {1}" + Environment.NewLine, AGRO.linka_3, rfid_pocet);
                        ExceptionHandler2.Handle(rfid_hlaska, "Log_RFID_Tagy", "txt");
                    ...
                    else if (interniCisloLinka == AGRO.linka_3)
                    {
                        //beze zmeny
                    }
                    else
                    {
                        //log_hlaska = string.Format("NAKLADKA_INFO_Linka_[{0}] nebyla vykladka? neni vycistena interniCisloLinka({1})!!", AGRO.linka_3, interniCisloLinka);
                        ...
                        if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON)
                        {
                            ...
                            log_hlaska = string.Format("NAKLADKA_INFO - PŘÍZNAK falešné nakládky z linky: {1}! start vyhodnoceni nakladky na lince: {0}", AGRO.linka_3, interniCisloLinka);
                            ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");

                            int akce_nakladka = 7;
                            LV.OnLogsEvent(new Logs_EventArgs(0, akce_nakladka, log_hlaska, LoginID, AGRO.status_0));
                            OnNakladkaEvent(new Nakladka_EventArgs(AGRO.linka_3));
                ...
                else if (sensor == AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_OK)
                {
                    ...
                    else if (interniCisloLinka == AGRO.rucni_vstup)
                    {
                        //beze zmeny
                    }
                    else
                    {
                        //log_hlaska = string.Format("NAKLADKA_INFO_Linka_[{0}] nebyla vykladka? neni vycistena interniCisloLinka({1})!!", AGRO.rucni_vstup, interniCisloLinka);
                        ...
                        if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON)
                        {
                            ...
                            log_hlaska = string.Format("NAKLADKA_INFO - PŘÍZNAK falešné nakládky z linky: {1}! start vyhodnoceni nakladky na lince: {0}", AGRO.rucni_vstup, interniCisloLinka);
                            ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");

                            int akce_nakladka = 7;
                            LV.OnLogsEvent(new Logs_EventArgs(0, akce_nakladka, log_hlaska, LoginID, AGRO.status_0));
                            OnNakladkaEvent(new Nakladka_EventArgs(AGRO.rucni_vstup));
                        ...
                        else
                        {
                            ...
                            log_hlaska = string.Format("NAKLADKA_INFO - nedoslo k vykladce z linky: {1}! nakladam na lince: {0}", AGRO.rucni_vstup, interniCisloLinka);
                            ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
                ...
                else if (sensor == AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec_vyber)
                {
                ...
                if (sensor == AgroSledovaniVozikuConfig.config.ADAM_2_DI[0].Strec1_OK)
                {
                    ...
                    try
                    {
                        if (AgroSledovaniVozikuConfig.config.Vykladka[0].zapnuti_zalozni_logiky)
                        {
                            ...
                            {
                                //bool[] vaha1_ERROR_pole = adam_2.getDIStatus();
                                if (adam_1.actualState_DI != null && adam_1.actualState_DI.Length >= AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L2)
                                {
                                    bool[] L2_pole = adam_1.actualState_DI;
                                    if (L2_pole != null)
                                    {
                                        var_L2 = L2_pole[AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L2];
                                    }
                                }

                                if (adam_1.actualState_DI != null && adam_1.actualState_DI.Length >= AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L3)
                                {
                                    bool[] L3_pole = adam_1.actualState_DI;
                                    if (L3_pole != null)
                                    {
                                        var_L3 = L3_pole[AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L3];
                                    }
                                }

                                if (adam_1.actualState_DI != null && adam_1.actualState_DI.Length >= AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L4)
                                {
                                    bool[] L4_pole = adam_1.actualState_DI;
                                    if (L4_pole != null)
                                    {
                                        var_L4 = L4_pole[AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L4];
                                    }
                                }
...
#region UC inicializace + deklarace ADAM DI 
                ...
                bool Strec1_OK = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_OK, vyber_adam_1);
                bool Strec1_Bit1 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit1, vyber_adam_1);
                bool Strec1_Bit2 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit2, vyber_adam_1);
                bool Strec1_Bit4 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit4, vyber_adam_1);
                bool Linka1_CALL = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_CALL, vyber_adam_1);
                bool Linka1_END = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_END, vyber_adam_1);
                bool Linka1_OK = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_OK, vyber_adam_1);
                bool Linka1_RUN = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_RUN, vyber_adam_1);

                bool Linka2_CALL = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_CALL, vyber_adam_1);
                bool Linka2_END = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_END, vyber_adam_1);
                bool Linka2_OK = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_OK, vyber_adam_1);
                bool Linka2_RUN = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_RUN, vyber_adam_1);

                bool Linka3_CALL = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_CALL, vyber_adam_1);
                bool Linka3_END = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_END, vyber_adam_1);
                bool Linka3_OK = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_OK, vyber_adam_1);
                bool Linka3_RUN = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_RUN, vyber_adam_1);

                bool RucniVstup_CALL = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_CALL, vyber_adam_1);
                bool RucniVstup_END = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_END, vyber_adam_1);
                bool RucniVstup_OK = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_OK, vyber_adam_1);
                bool RucniVstup_RUN = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_RUN, vyber_adam_1);

                bool Strec2_OK = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_OK, vyber_adam_2);

                bool L1_val = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L1, vyber_adam_1);
                bool L2_val = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L2, vyber_adam_1);
                bool L3_val = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L3, vyber_adam_1);
                bool L4_val = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L4, vyber_adam_1);
...
#region L1-L4 a smer -- interpretace
                ...
                //L1 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L1, vyber_adam_1);
                //L2 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L2, vyber_adam_1);
                //L3 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L3, vyber_adam_1);
                //L4 = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L4, vyber_adam_1);
                //Smer = GetAdamDI(AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec_vyber, vyber_adam_1);
...
#region UC RFID
                if (RFID_Linky_IN != null)
                {
                    ...
                    rfiD_UC2.RFID_1_Connected = true;
                ...
                else
                {
                    ...
                    rfiD_UC2.RFID_1_Connected = false;
                }

                if (RFID_Linky_OUT != null)
                {
                    rfiD_UC1.RFID_2_Connected = true;
                    rfiD_UC2.RFID_2_Connected = true;
                ...
                else
                {
                    ...
                    rfiD_UC2.RFID_2_Connected = false;
                }
#endregion
        ...
        private void RFID_Antena_zobraz(int pozice, string reader)
        {
            try
            {
                ...
                if (reader == "reader_1")
                {
                    ...
                    {
                        case 1:
                            rfiD_UC2.ChB_R1_ANT1 = true;
                            break;
                        case 2:
                            rfiD_UC2.ChB_R1_ANT2 = true;
                            break;
                        case 3:
                            rfiD_UC2.ChB_R1_ANT3 = true;
                            break;
                        case 4:
                            rfiD_UC2.ChB_R1_ANT4 = true;
                            break;
                        default:
                            break;
                    }
                ...
                if (reader == "reader_2")
                {
                    ...
                    {
                        case 1:
                            rfiD_UC2.ChB_R2_ANT1 = true;
                            break;
                        case 2:
                            rfiD_UC2.ChB_R2_ANT2 = true;
                            break;
                        case 3:
                            rfiD_UC2.ChB_R2_ANT3 = true;
                            break;
                        case 4:
                            rfiD_UC2.ChB_R2_ANT4 = true;
                            break;
                        default:
                            break;
                    }
        ...
        private void vizualizace(bool input, string source)
        {
            try
            {
                ...
                if (AgroSledovaniVozikuConfig.config.Vizualizace[0].Enable)
                {
                    ...
                    if (rfiD_UC2.Tag_Text_W != NenactenoEPC && rfiD_UC2.Tag_Text_W.Length == 24)
                        trida.tagText = rfiD_UC2.Tag_Text_W.Substring(0, 2);
...
#region ADAM1
                ...
                AgroSledovaniVozikuConfig.config.ADAM_1[0].communication_P2P = chb_ADAM1_Com_P2P.Checked;
                AgroSledovaniVozikuConfig.config.ADAM_1[0].communication_TCP = chb_ADAM1_Com_TCP.Checked;
                ...
                AgroSledovaniVozikuConfig.config.ADAM_2[0].communication_P2P = chb_ADAM2_Com_P2P.Checked;
                AgroSledovaniVozikuConfig.config.ADAM_2[0].communication_TCP = chb_ADAM2_Com_TCP.Checked;
                ...
                AgroSledovaniVozikuConfig.config.ADAM_3[0].communication_P2P = chb_ADAM3_Com_P2P.Checked;
                AgroSledovaniVozikuConfig.config.ADAM_3[0].communication_TCP = chb_ADAM3_Com_TCP.Checked;
...
#region ADAM DI 1
                 ...
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_END = int.Parse(tb_ADAM1_Linka1_END.Text);
                AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_RUN = int.Parse(tb_ADAM1_Linka1_RUN.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_OK = int.Parse(tb_ADAM1_Linka1_OK.Text);
                AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka1_CALL = int.Parse(tb_ADAM1_Linka1_CALL.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_END = int.Parse(tb_ADAM1_Linka2_END.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_RUN = int.Parse(tb_ADAM1_Linka2_RUN.Text);
                AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_OK = int.Parse(tb_ADAM1_Linka2_OK.Text);
                AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka2_CALL = int.Parse(tb_ADAM1_Linka2_CALL.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_END = int.Parse(tb_ADAM1_Linka3_END.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_RUN = int.Parse(tb_ADAM1_Linka3_RUN.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_OK = int.Parse(tb_ADAM1_Linka3_OK.Text);
                AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Linka3_CALL = int.Parse(tb_ADAM1_Linka3_CALL.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L1 = int.Parse(tb_ADAM1_L1.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L2 = int.Parse(tb_ADAM1_L2.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L3 = int.Parse(tb_ADAM1_L3.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].L4 = int.Parse(tb_ADAM1_L4.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_END = int.Parse(tb_ADAM1_RucnyVstup_END.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_RUN = int.Parse(tb_ADAM1_RucnyVstup_RUN.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_OK = int.Parse(tb_ADAM1_RucnyVstup_OK.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].RucniVstup_CALL = int.Parse(tb_ADAM1_RucnyVstup_CALL.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_OK = int.Parse(tb_ADAM1_Strec1_OK.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit1 = int.Parse(tb_ADAM1_Strec1_Bit1.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit2 = int.Parse(tb_ADAM1_Strec1_Bit2.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec1_Bit4 = int.Parse(tb_ADAM1_Strec1_Bit4.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_OK = int.Parse(tb_ADAM1_Strec2_OK.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_Bit1 = int.Parse(tb_ADAM1_Strec2_Bit1.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_Bit2 = int.Parse(tb_ADAM1_Strec2_Bit2.Text);
                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec2_Bit4 = int.Parse(tb_ADAM1_Strec2_Bit4.Text);

                 AgroSledovaniVozikuConfig.config.ADAM_1_DI[0].Strec_vyber = int.Parse(tb_ADAM1_Strec_Vyber.Text);
...
#region WEB API TISK
                ...
                AgroSledovaniVozikuConfig.config.WEBAPI_TISK[0].Logging = chB_APITISK_logs.Checked;
                ...
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].cas_rfid = int.Parse( tB_TEST_cas_nakladka.Text);
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vse_ON = chB_RFID_vse.Checked;
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_ON = chB_RFID_nakladka.Checked;
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_vykladka_ON = chB_RFID_vykladka.Checked;
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_TEST_rezim_ON = chB_RFID_TEST_rezim.Checked;
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_nakladka_overeni_ON = chB_RFID_nakladka_overeni.Checked;
                AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_stara_logika_ON = chB_RFID_stara_logika.Checked;
            ...
            try
            {
                ...
                //if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_TEST_rezim_ON)
                //{
                //    RFID_reload();
                //    tabControl1.TabPages.Add(tP_RFID);
                //}
                //else
                //{
                //    tabControl1.TabPages.Remove(tP_RFID);
                //}
            }
            catch (Exception ex)
            {
        ...
        private void NastavRFIDRezim(bool nastavuji)
        {
            try
            {
                if (nastavuji)
                {
                    //chB_RFID_REZIM_nakladka_ON.Checked = AgroSledovaniVozikuConfig.config.RFID_1[0].Enable;
                    //chB_RFID_REZIM_vykladka_ON.Checked = AgroSledovaniVozikuConfig.config.RFID_2[0].Enable;

                    AgroSledovaniVozikuConfig.config.RFID_1[0].Enable = chB_RFID_REZIM_nakladka_ON.Checked;
                    AgroSledovaniVozikuConfig.config.RFID_2[0].Enable = chB_RFID_REZIM_vykladka_ON.Checked;
                }
                else
                {
                    chB_RFID_REZIM_nakladka_ON.Checked = AgroSledovaniVozikuConfig.config.RFID_1[0].Enable;
                ...
                if (AgroSledovaniVozikuConfig.config.RFID_1[0].Enable)
                {
                    chB_RFID_REZIM_nakladka_ON.BackColor = Color.Green;
                }
                else
                {
                    chB_RFID_REZIM_nakladka_ON.BackColor = Color.Transparent;
                }

                if (AgroSledovaniVozikuConfig.config.RFID_2[0].Enable)
                {
                    chB_RFID_REZIM_vykladka_ON.BackColor = Color.Green;
                }
                else
                {
                    chB_RFID_REZIM_vykladka_ON.BackColor = Color.Transparent;
                }
...
#region Archivace
    ...
    string text)
        ...
        /// <summary>
        /// RFID rezim pro nastaveni
        /// </summary>
        /// <param name="init">false pokud se nevola metoda pri nacteni formulare</param>
        private void RFID_sprava(bool init = false)
        {
            if (AgroSledovaniVozikuConfig.config.RFID_sprava[0].RFID_TEST_rezim_ON)
            {


                if (!tabControl1.TabPages.Contains(tP_RFID))
                {
                    tabControl1.TabPages.Add(tP_RFID);
                }
            }
            ...
            if (RFID_Linky_IN != null)
            {
                l_RFID_DLL_info.Text = RFID_Linky_IN.Info_DLL();
            }
            else if (RFID_Linky_OUT != null)
            {
                l_RFID_DLL_info.Text = RFID_Linky_OUT.Info_DLL();
            }
        }

        private void RFID_reload()
        {
            // throw new NotImplementedException();
        ...
        private void btn_RFID_lokalizace_Click(object sender, EventArgs e)
        {
        ...
        public void LokalizujTag()
        {
            try
            {
                string idTag = string.Empty;
                int cisloAnteny = -1;
                int dobaLokalizace = -1;
                bool vysledek = false;


#if DEBUG
                tB_RFID_lok_ID_tag.Text = "776088888888888888888093";
                tB_RFID_lok_antena.Text = "2";
                tB_RFID_lok_doba.Text = "3000";
#endif


                if (
                    !string.IsNullOrEmpty(tB_RFID_lok_ID_tag.Text)
                    && !string.IsNullOrEmpty(tB_RFID_lok_antena.Text)
                    && !string.IsNullOrEmpty(tB_RFID_lok_doba.Text)
                    )
                {
                    idTag = tB_RFID_lok_ID_tag.Text;
                    cisloAnteny = int.Parse(tB_RFID_lok_antena.Text);
                    dobaLokalizace = int.Parse(tB_RFID_lok_doba.Text);

                }

                if (RFID_Linky_IN != null)
                {
                    vysledek = RFID_Linky_IN.PerformTagLocationing(idTag, cisloAnteny, dobaLokalizace);
                }

                if (vysledek)
                {
                    tB_RFID_lok_vysledek.Text = "nalezen";
                }
                else
                {
                    tB_RFID_lok_vysledek.Text = "nenalezen";
                }
        ...
        private void btn_RFID_REZIM_restart_Click(object sender, EventArgs e)
        {
            NastavRFIDRezim(true);

            bool vysledek = ReinicializaceRFID();

            if(vysledek)
            {
                btn_RFID_REZIM_restart.BackColor = Color.Transparent;



                //#region konfigurace anten TEST
                //if (RFID_Linky_IN !=null)
                //{
                //    List<ushort> anteny = new List<ushort>();

                //    if (!string.IsNullOrEmpty(txB_RFID_REZIM_cislo_antena.Text))
                //    {
                //        ushort cisloAnteny = ushort.Parse(txB_RFID_REZIM_cislo_antena.Text);

                //        anteny.Add(cisloAnteny);

                //    }

                //    RFID_Linky_IN.Start_Read_tags(anteny, null, string.Empty);
                //    Thread.Sleep(2000);
                //    RFID_Linky_IN.Stop_Read_tags();  
            ...
            {
                btn_RFID_REZIM_restart.BackColor = Color.Red;
            }

        }

        private bool ReinicializaceRFID()
        {
            bool vysledek = false;
        ...
        private void TestovaciCteniTagu(bool zapnout_vypnout)
        {
        ...
        private void btn_RFID_REZIM_start_read_Click(object sender, EventArgs e)
        {
            try
            {
                TestovaciCteniTagu(true);

                btn_RFID_REZIM_start_read.Enabled = false;
                btn_RFID_REZIM_stop_read.Enabled = true;
            }
            catch (Exception ex)
            {
        ...
        private void btn_RFID_REZIM_stop_read_Click(object sender, EventArgs e)
        {
            try
            {
                TestovaciCteniTagu(false);


                btn_RFID_REZIM_start_read.Enabled = true;
                btn_RFID_REZIM_stop_read.Enabled = false;
            }
            catch (Exception ex)
            {
        ...
        private void btn_TEST_TISK_Sent_Click(object sender, EventArgs e)
        {

            try
            {
                Fask.WEBAPI.API_BusinessObjects.FASK_Events_row o = null;
                o = new Fask.WEBAPI.API_BusinessObjects.FASK_Events_row();
                o.NMBRPAL = "33333333333333333333";
                o.material = "TEST DATA";
                o.PackType = "FP";
                o.barcodeSended = "TEST DATA";
                o.ITEMDESC = "TEST DATA";
                o.WEIGHT = 666;
                o.status = AGRO.status_40;

                LV.TiskPosliData(o, AGRO.bocedi_4_K);
            }
            catch (Exception ex)
            {
    ...
    public class Nakladka_EventArgs : EventArgs
    {
        //public int CisloLinky { get; set; }

        public int CisloLinky { get; }

        public Nakladka_EventArgs(int cl)
        {
            CisloLinky = cl;
        }
    }
