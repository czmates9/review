...
using FASK.SledovaniVyroby.Module.Vyroba_Agro_Kapalky_SV.Classes;
using System.Threading;
using Fask.Logging;
using static FASK.SledovaniVyroby.Module.Vyroba_Agro_Kapalky_SV.Classes.Logika_Voziky;
using FASK.SledovaniVyroby.Main.Configuration;
using Fask.Constants;

namespace FASK.SledovaniVyroby.Module.Vyroba_Agro_Kapalky_SV
{
    ...
    public partial class frmMain : Form, IModuleConnector
    {
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            bt_Linka_1.Text = Fask.Constants.AGRO.BTN_Linka5_K;
            bt_RV.Text = Fask.Constants.AGRO.BTN_RV_K;
            bt_bocedi1.Text = Fask.Constants.AGRO.BTN_Bocedi4_K;
        ...
        private void CallBack_Timer_Strec1(object state)
        {
            try
            {
                ...
                if (interniCisloLinka != 0)
                {
                    ...
                    LV.OnVykladkaEvent(new Vykladka_EventArgs(interniCisloLinka, Fask.Constants.AGRO.bocedi_4_K));
                }
                else
                {
                    if(OnlineLog_Bocedi_1)
                    {
                        ...
                        LV.OnLogsEvent(new Logs_EventArgs(Fask.Constants.AGRO.bocedi_4_K, akce, popisek, LoginID, 0));
                    }
                    else
                    { 
                        //vse je ok
        ...
        private void CallBack_Timer_Vaha1(object state)
        {
            try
            {
                ...
                if (vaha_1_data)
                {
                    ...
                    FE_objekt = LV.Najdi_zaznam(Fask.Constants.AGRO.status_34, Fask.Constants.AGRO.status_44);
                    ...
                    if (FE_objekt != null && FE_objekt.faskGUID.HasValue)
                    {
                        ...
                        OnLogikaZAdamaEvent(new LogikaZAdama_EventArgs(Fask.Constants.AGRO.status_34));
                    ...
                    else
                    {
                        ...
                        OnLogikaZAdamaEvent(new LogikaZAdama_EventArgs(Fask.Constants.AGRO.status_24));
                    }

                    OnLogikaZAdamaEvent(new LogikaZAdama_EventArgs(Fask.Constants.AGRO.status_44));
                }
            }
            catch (Exception ex)
            {
        ...
        private void FrmMain_LogikaZAdama_EventHandler(object sender, LogikaZAdama_EventArgs e)
        {
            ...
            try
            {
                if (e.CisloStatus == Fask.Constants.AGRO.status_24)
                {
                ...
                else if (e.CisloStatus == Fask.Constants.AGRO.status_34)
                {
                ...
                else if (e.CisloStatus == Fask.Constants.AGRO.status_44)
                {
                ...
                else if (e.CisloStatus == Fask.Constants.AGRO.status_64)
                {
            ...
            try
            {
                if (sensor == AgroSledovaniVozikuConfig.config.ADAM_DI[0].Linka1_RUN)
                {
                    ...
                    if (interniCisloLinka == 0)
                    {
                        ...
                        {
                            interniCisloLinka = Fask.Constants.AGRO.linka_5_K;
                        }
                    }
                    else
                    {
                ...
                else if (sensor == AgroSledovaniVozikuConfig.config.ADAM_DI[0].RucniVstup_OK)
                {
                    ...
                    if (interniCisloLinka == 0)
                    {
                        lock (interniCisloLinka_object)
                        {
                            interniCisloLinka = Fask.Constants.AGRO.rucni_vstup_K; 
                        }
                    }
                    else
                    {
                        ...
                        log_hlaska = string.Format("NAKLADKA_INFO_Linka_[{0}] neni vycistena interniCisloLinka = ({1})!!", Fask.Constants.AGRO.rucni_vstup_K,interniCisloLinka);
                        ExceptionHandler2.Handle(log_hlaska, "Log_LV", "txt");
        ...
        private void AdamSensorDataReady_2(int sensor)
        {
            ...
            try
            {
                ...
                else if (sensor == AgroSledovaniVozikuConfig.config.ADAM_2_DI[0].Tisk1_END)
                {
                    ...
                    OnLogikaZAdamaEvent(new LogikaZAdama_EventArgs(Fask.Constants.AGRO.status_64));
        ...
        private void bt_archivace_Click(object sender, EventArgs e)
        {
            try
            {
                if (sender is Button)
                {
                    ...
                    if (btn.Text.Trim() == Fask.Constants.AGRO.BTN_Linka5_K)
                    {
                        Archivace(
                            Fask.Constants.AGRO.linka_5_K,
                            Fask.Constants.AGRO.status_0,
                            AgroSledovaniVozikuConfig.config.DB_Config[0].Separator,
                            Fask.Constants.AGRO.BTN_Linka5_K
                            );
                    }
                    else if (btn.Text.Trim() == Fask.Constants.AGRO.BTN_RV_K)
                    {
                        Archivace(Fask.Constants.AGRO.rucni_vstup_K,
                            Fask.Constants.AGRO.status_0,
                            AgroSledovaniVozikuConfig.config.DB_Config[0].Separator,
                            Fask.Constants.AGRO.BTN_RV_K
                            );
                    }
                    else if (btn.Text.Trim() == Fask.Constants.AGRO.BTN_Bocedi4_K)
                    {
                        Archivace(-1,
                            Fask.Constants.AGRO.status_24,
                            AgroSledovaniVozikuConfig.config.DB_Config[0].Separator_strec,
                            Fask.Constants.AGRO.BTN_Bocedi4_K);
                    }
                }
            }
            catch (Exception ex)
