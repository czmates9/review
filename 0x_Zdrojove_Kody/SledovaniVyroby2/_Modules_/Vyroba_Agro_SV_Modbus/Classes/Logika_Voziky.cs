    ...
    public class Logika_Voziky
    {
        ...
        private void Logika_Voziky_vykladka_EventHandler(object sender, Vykladka_EventArgs e)
        {
            ...
            try
            {
                ...
                int statusNakladka = Fask.Constants.AGRO.status_0;
                int statusVykladka = Fask.Constants.AGRO.status_21;
                bool zapis_OK = false;
                bool falesnyZaznam = false;

                statusNakladka = Fask.Constants.AGRO.status_0; 
                statusVykladka = Fask.Constants.AGRO.status_21; 
                zapis_OK = false; 
                falesnyZaznam = false;

                #region Dohledání/Vytvoření záznamu nakládky
                Fask.Tracing.Trac.Write("START dohledání záznamu nakládky", tracId);
                if (cisloLinky == Fask.Constants.AGRO.rucni_vstup_R || cisloLinky == Fask.Constants.AGRO.falsak_R)
                {
                    //objekt_Fe_data.id = 33;
                    objekt_Fe_data.loginid = "44";
                    if (cisloLinky == Fask.Constants.AGRO.rucni_vstup_R)
                    {
                    ...
                    else
                    {
                        ...
                        objekt_Fe_data.machineid = Fask.Constants.AGRO.falsak_R.ToString();
                    ...
                    try
                    {
                        ...
                        string sscc_FP_1 = generatorSSCC.Get_Next_SSCC(AGRO.rucni_vstup_R, 1, AGRO.rucni_vstup_R); // TODO konfigurace ID stroje
                ...
                if (objekt_Fe_data != null && objekt_Fe_data.faskGUID.HasValue)
                {
                    ...
                    if (e.CisloStrec == Fask.Constants.AGRO.bocedi_3_R)
                    {
                ...
                else
                {
                    ...
                    if (cisloLinky == Fask.Constants.AGRO.linka_4_R || cisloLinky == Fask.Constants.AGRO.rucni_vstup_R)
                    {
                    ...
                    {
                        objekt_Fe_data_falsak.machineid = Fask.Constants.AGRO.falsak_R.ToString();
                        cisloLinky_FP = Fask.Constants.AGRO.falsak_R;
                    }
        ...
        public Fask.WEBAPI.API_BusinessObjects.FASK_Events_row Najdi_zaznam(int Status, int StatusNew)
        {
            ...
            try
            {
                ...
                if (Status == Fask.Constants.AGRO.status_0)
                {
                    TextSeparator = AgroSledovaniVozikuConfig.config.DB_Config[0].Separator;
                }
                else if (Status == Fask.Constants.AGRO.status_1)
                {
                    TextSeparator = AgroSledovaniVozikuConfig.config.DB_Config[0].Separator_strec;
                }
                else if (StatusNew == Fask.Constants.AGRO.status_41)
                {
                    if (_parent.uC_Vaha.Vaha_1_Connected() && StatusNew == Fask.Constants.AGRO.status_41 && !_parent.vaha_1_ERROR && Status != Fask.Constants.AGRO.status_21)
                    {
                ...
                else if (Status == Fask.Constants.AGRO.status_41)
                {
        ...
        public void TiskPrijem(int cisloStatus)
        {
            ...
            {
                int cislo_bocedi = 0;
                if (cisloStatus == Fask.Constants.AGRO.status_21)
                {
                    cislo_bocedi = Fask.Constants.AGRO.bocedi_3_R;
                }
                ...
                if (cisloStatus == Fask.Constants.AGRO.status_21)
                {
                    FE_objekt = Najdi_zaznam(cisloStatus, Fask.Constants.AGRO.status_41);
#if DEBUG
        ...
        public void TiskPrijem_Vaha(int cisloStatus)
        {
            ...
            {
                int cislo_bocedi = 0;
                if (cisloStatus == Fask.Constants.AGRO.status_31)
                {
                    cislo_bocedi = Fask.Constants.AGRO.bocedi_3_R;
                }
        ...
        public void TiskPosliData(Fask.WEBAPI.API_BusinessObjects.FASK_Events_row o, int bocedi_cislo_drahy)
        {
            ...
            try
            {
                ...
                if (o == null)
                {
                    ...
                    if (bocedi_cislo_drahy == Fask.Constants.AGRO.bocedi_3_R)
                    {
                        o.status = Fask.Constants.AGRO.status_41;
                    }
                ...
                if (txt != "OK")
                {
                    ...
                    this.OnLogsEvent(new Logs_EventArgs(cisloLinky, akce, txt, 0, AGRO.status_40));
                }
