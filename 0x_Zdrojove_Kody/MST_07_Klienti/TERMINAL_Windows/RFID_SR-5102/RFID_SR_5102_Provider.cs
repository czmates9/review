...
#region TID
        ...
        /// </summary>
            public RFID_SR_5102_Provider()
            {
                try
                {
                    string CmdComAddr = FASK.MST_WINDOWS.Main.Configuration.Config.RFID_Address;
                    FASK.RFID_SR_5102.ComPorty port = (FASK.RFID_SR_5102.ComPorty)Enum.Parse(typeof(FASK.RFID_SR_5102.ComPorty), FASK.MST_WINDOWS.Main.Configuration.Config.RFID_ComPort);
                    FASK.RFID_SR_5102.Baudrate baud = (FASK.RFID_SR_5102.Baudrate)Enum.Parse(typeof(FASK.RFID_SR_5102.Baudrate), FASK.MST_WINDOWS.Main.Configuration.Config.RFID_Baudrate);
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                if (status != "OK")
                {
                    FASK.MST_WINDOWS.ErrorLog.Log.WriteException("RFID Start: " + status);
                    timer.Dispose();
                    isopen = false;
                }
                else
        ...
        public void Stop()
        {
            try
            {
                ...
                if (status != "OK" && status != "Port is not open.")
                {
                    timer.Dispose();
                    isopen = false;
                    FASK.MST_WINDOWS.ErrorLog.Log.WriteException("RFID Stop: " + status);
