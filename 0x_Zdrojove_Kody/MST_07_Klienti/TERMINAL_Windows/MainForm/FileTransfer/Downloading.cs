        ...
        public static bool DownloadFileFromServer(string fileciselnik, bool silent, bool overwrite)
        {
            ...
            try
            {
                ...
                */
                ...
                wr.Timeout = 1000;
                //long celkovoKstazeni = ftransferService.GetFileSize(MST_Global.TerminalID, fileciselnik);

                try
                {
                    using (System.Net.WebResponse response = wr.GetResponse())
                    {
                        long celkovoKstazeni = response.ContentLength;
                            ...
                            do
                            {
                                ...
                                do
                                {
                                    try
                                    {
                                        ...
                                        if (celkovoKstazeni == 0)
                                            procento = 0;
                                        else
                                            procento = (((decimal)dataRead / (decimal)celkovoKstazeni)) * 100;
                                        ...
                                        if (downloadspeed == 0)
                                            ...
                                            speed = ((downloadspeed == 0 ? 1 : downloadspeed) * 1024);
            ...
            catch (Exception ex)
            {
                ...
                if (!silent)
                {
                    ...
                    {
                        Log.Write(ex);
                        FlexibleMessageBox.Show( ex.Message, "FileTransfer", System.Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                    }
                    else
                    {
                        Log.Write(ex);
                        Log.Write(ex.InnerException);
                        FlexibleMessageBox.Show(ex.Message + ",Innerexception:" + ex.InnerException.Message, "FileTransfer", System.Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                    }
                }
                                ...
                                do
                                {
                                    try
                                    {
                                        ...
                                        if (filesize == 0)
                                            ...
                                            procento = (((decimal)dataRead / (decimal)filesize)) * 100;
                                        ...
                                        if (!silent)
                                        {
                                            //if (pso != null && pso is Fask.MST_W.Prijem_4.PrijemServiceOperations)
                                            //{
                                            //    Fask.MST_W.Prijem_4.PrijemServiceOperations _pso = (Fask.MST_W.Prijem_4.PrijemServiceOperations)pso;
                                            //    _pso.WriteDesc(String.Format("Procenta: {0:###.00}% \nRychlost: {1:#####.00}kB/s\nOdhad: {2:###.00}s", procento, downloadspeed, lastingSeconds));
                                            //}
            ...
            catch (Exception ex)
            {
                ...
                if (!silent)
                {
                    ...
                    else
                    {
                        Log.Write(ex);
                        Log.Write(ex.InnerException);
                        //MessageBoxBig.Show(ex.Message + ",Innerexception:" + ex.InnerException.Message, "FileTransfer", System.Windows.Forms.MessageBoxButtons.OK, MessageBoxBigIcon.Critical);
                    }
