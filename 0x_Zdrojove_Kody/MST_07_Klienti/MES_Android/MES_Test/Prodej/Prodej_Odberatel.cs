...
{
    [Activity(Label = "@string/Prodej_Odberatele_Label", Theme = "@style/AppTheme")]
    public class Prodej_Odberatel : Base_Aktivita, NavigationView.IOnNavigationItemSelectedListener, ICiselniky_Datum
    {
        #region Parametry
        ...
        Prodej_Odberatele_Item_Adapter mAdapter;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            try
            {
                SetContentView(Resource.Layout.Prodej_Odberatele);
                ...
                mRecycleView = FindViewById<RecyclerView>(Resource.Id.Prodej_Odberatele_recyclerView);
                mLayoutManager = new LinearLayoutManager(this);
                mRecycleView.SetLayoutManager(mLayoutManager);

                InitAdapter();

                mToolbar = FindViewById<Android.Support.V7.Widget.Toolbar>(Resource.Id.Prodej_Odberatele_toolbar);
                mDrawerLayout = FindViewById<DrawerLayout>(Resource.Id.Prodej_Odberatele_drawer_layout);
                ...
                mNavigationView = FindViewById<NavigationView>(Resource.Id.Prodej_Odberatele_nav_view);
        ...
        private void InitAdapter()
        {
            try
            {
                ...
                Fask.SQLiteDBs.DataSets.Odberatele.CZMST090DataTable dtOdberatele = new Fask.SQLiteDBs.DataSets.Odberatele.CZMST090DataTable();

                using (Fask.SQLiteDBs.Controllers.SQLite_Controller_Odberatele conOdb = new Fask.SQLiteDBs.Controllers.SQLite_Controller_Odberatele(DataInfo_Static.CiselnikOdberateleDB))
                {
                    conOdb.Fill(dtOdberatele);
                }

                if (dtOdberatele.Count == 1)
                {
                    _Item.Odberatel = dtOdberatele[0];
                    StartAktivityNasledujici(typeof(Prodej_Davky), _Item);
                }

                mAdapter = new Prodej_Odberatele_Item_Adapter(this, new Prodej_Odberatele_Seznam(dtOdberatele));
        ...
        public void SetDateToMenu(int ID_String, string FileName)
        {
            ...
            if (cnt == 1)
            {
                ...
                if (ID_String == Resource.String.Odberatele)
                    ID_Menu = Resource.Id.nav_Ciselnik_proOdberatele;
        ...
        private void MAdapter_ItemClick(object sender, int e)
        {
            try
            {
                _Item.Odberatel = mAdapter.PolozkaSeznam.mItems[e];
        ...
        public bool OnNavigationItemSelected(IMenuItem item)
        {
            ...
            if (id == Resource.Id.nav_HlMenu_ProOdberatele)
            {
                StartAktivityNasledujici(typeof(Prodej_Davky), null);
            }
            else if (id == Resource.Id.nav_Ciselnik_proOdberatele)
            {
                CiselnikOdberatele();
            }
        ...
        {
            MenuInflater.Inflate(Resource.Menu.Prodej_Odberatele_Search, menu);

            var item = menu.FindItem(Resource.Id.ProdejOdberatele_search);
        ...
        private async void CiselnikOdberatele()
        {
            try
            {

                Ciselniky_Helper _Helper = new Ciselniky_Helper();
                bool state = await _Helper.SynchronizeCiselniky(CiselnikServiceOperations.Operation.Odberatele, this, Konfigurace_Singleton.Instance.Prodej.SkladID);
