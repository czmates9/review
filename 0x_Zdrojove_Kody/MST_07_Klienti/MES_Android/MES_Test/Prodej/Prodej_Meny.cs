...
{
    [Activity(Label = "@string/Prodej_Meny_Label", Theme = "@style/AppTheme")]
    public class Prodej_Meny : Base_Aktivita, NavigationView.IOnNavigationItemSelectedListener, ICiselniky_Datum
    {
        #region Parametry
        ...
        Prodej_Meny_Item_Adapter mAdapter;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            try
            {
                SetContentView(Resource.Layout.Prodej_Meny);
                ...
                menaID = Intent?.GetStringExtra(DataInfo_Static.MenaID);

                mRecycleView = FindViewById<RecyclerView>(Resource.Id.Prodej_Meny_recyclerView);
                mLayoutManager = new LinearLayoutManager(this);
                mRecycleView.SetLayoutManager(mLayoutManager);

                InitAdapter();

                mToolbar = FindViewById<Android.Support.V7.Widget.Toolbar>(Resource.Id.Prodej_Meny_toolbar);
                mDrawerLayout = FindViewById<DrawerLayout>(Resource.Id.Prodej_Meny_drawer_layout);
                ...
                mNavigationView = FindViewById<NavigationView>(Resource.Id.Prodej_Meny_nav_view);
        ...
        private void InitAdapter()
        {
            try
            {
                Fask.SQLiteDBs.DataSets.Meny.CZMST097DataTable dtMeny = new Fask.SQLiteDBs.DataSets.Meny.CZMST097DataTable();

                using (Fask.SQLiteDBs.Controllers.SQLite_Controller_Meny conMeny = new Fask.SQLiteDBs.Controllers.SQLite_Controller_Meny(DataInfo_Static.CiselnikMenyDB))
                {
                    conMeny.Fill(dtMeny);
                }
                ...
                mAdapter = new Prodej_Meny_Item_Adapter(this, new Prodej_Meny_Seznam(dtMeny));
        ...
        public void SetDateToMenu(int ID_String, string FileName)
        {
            ...
            if (cnt == 1)
            {
                ...
                if (ID_String == Resource.String.Meny)
                    ID_Menu = Resource.Id.nav_Ciselnik_proMeny;
        ...
        private void MAdapter_ItemClick(object sender, int e)
        {
            try
            {
                _Item.Mena = mAdapter.PolozkaSeznam.mItems[e];
        ...
        public bool OnNavigationItemSelected(IMenuItem item)
        {
            ...
            if (id == Resource.Id.nav_HlMenu_ProMeny)
            {
                StartAktivityNasledujici(typeof(Prodej_Davky), null);
            }
            else if (id == Resource.Id.nav_Ciselnik_proMeny)
            {
                CiselnikMeny();
            }
        ...
        {
            MenuInflater.Inflate(Resource.Menu.Prodej_Meny_Search, menu);

            var item = menu.FindItem(Resource.Id.ProdejMeny_search);
        ...
        private async void CiselnikMeny()
        {
            try
            {

                Ciselniky_Helper _Helper = new Ciselniky_Helper();
                bool state = await _Helper.SynchronizeCiselniky(CiselnikServiceOperations.Operation.Meny, this, Konfigurace_Singleton.Instance.Prodej.SkladID);
