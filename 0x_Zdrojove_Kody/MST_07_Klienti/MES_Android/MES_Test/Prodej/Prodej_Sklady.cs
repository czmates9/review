...
{
    [Activity(Label = "@string/Prodej_Sklady_Label", Theme = "@style/AppTheme")]
    public class Prodej_Sklady : Base_Aktivita, NavigationView.IOnNavigationItemSelectedListener, ICiselniky_Datum
    {
        #region Parametry
        ...
        Prodej_Sklady_Item_Adapter mAdapter;
        ...
        private string TypSkladu = string.Empty;
        private string TypVratky = string.Empty;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            try
            {
                SetContentView(Resource.Layout.Prodej_Sklady);
                ...
                TypSkladu = Intent?.GetStringExtra(DataInfo_Static.TypSkladu);
                TypVratky = Intent?.GetStringExtra(DataInfo_Static.TypVratky);


                mRecycleView = FindViewById<RecyclerView>(Resource.Id.Prodej_Sklady_recyclerView);
                mLayoutManager = new LinearLayoutManager(this);
                mRecycleView.SetLayoutManager(mLayoutManager);

                InitAdapter();

                mToolbar = FindViewById<Android.Support.V7.Widget.Toolbar>(Resource.Id.Prodej_Sklady_toolbar);
                mDrawerLayout = FindViewById<DrawerLayout>(Resource.Id.Prodej_Sklady_drawer_layout);
                ...
                mNavigationView = FindViewById<NavigationView>(Resource.Id.Prodej_Sklady_nav_view);
                mNavigationView.SetNavigationItemSelectedListener(this);

                if (!string.IsNullOrEmpty(TypSkladu))
                {
                    if (TypSkladu == DataInfo_Static.TypSkladu_Zdroj)
                    {
                        SupportActionBar.SetTitle(Resource.String.Prodej_SkladZdroj_label);
                    }
                    else if (TypSkladu == DataInfo_Static.TypSkladu_Cil)
                    {
                        SupportActionBar.SetTitle(Resource.String.Prodej_SkladCil_Label);
                    }
                }
        ...
        private void InitAdapter()
        {
            try
            {
                Fask.SQLiteDBs.DataSets.Sklady.CZMST093DataTable dtSklady = new Fask.SQLiteDBs.DataSets.Sklady.CZMST093DataTable();

                using (Fask.SQLiteDBs.Controllers.SQLite_Controller_Sklady conSklady = new Fask.SQLiteDBs.Controllers.SQLite_Controller_Sklady(DataInfo_Static.CiselnikSkladyDB))
                {
                    conSklady.Fill(dtSklady);
                }
                ...
                if (dtSklady.Count == 1)
                {
                    if (!string.IsNullOrEmpty(TypSkladu))
                    {
                        if (TypSkladu == DataInfo_Static.TypSkladu_Zdroj)
                        {
                            _Item.SkladZdroj = dtSklady[0];
                        }
                        else if (TypSkladu == DataInfo_Static.TypSkladu_Cil)
                        {
                            _Item.SkladCil = dtSklady[0];
                        }
                ...
                mAdapter = new Prodej_Sklady_Item_Adapter(this, new Prodej_Sklady_Seznam(dtSklady));
        ...
        public void SetDateToMenu(int ID_String, string FileName)
        {
            ...
            if (cnt == 1)
            {
                ...
                if (ID_String == Resource.String.Sklady)
                    ID_Menu = Resource.Id.nav_Ciselnik_proSklady;
        ...
        private void StartAktivityNasledujici(Type typAktivity, Prodej_Davka_Item _Item)
        {
            ...
            if (_Item != null)
            {
                if (TypVratky == DataInfo_Static.ProdejItem)
                {
                ...
                if (TypVratky == DataInfo_Static.SkladRow)
                {
                    Bundle bundle = new Bundle();
                    bundle.PutBinder(DataInfo_Static.object_SkladCil, new WrapperForBinder<Fask.SQLiteDBs.DataSets.Sklady.CZMST093Row>(_Item.SkladCil));
                    intent.PutExtra(DataInfo_Static.SkladCil, bundle);
                }
        ...
        private void MAdapter_ItemClick(object sender, int e)
        {
            try
            {
                if (!string.IsNullOrEmpty(TypSkladu))
                {
                    if (TypSkladu == "Zdroj")
                    {
                        _Item.SkladZdroj = mAdapter.PolozkaSeznam.mItems[e];
                    }
                    else if (TypSkladu == "Cil")
                    {
                        _Item.SkladCil = mAdapter.PolozkaSeznam.mItems[e];
                    }
                }
        ...
        public bool OnNavigationItemSelected(IMenuItem item)
        {
            ...
            if (id == Resource.Id.nav_HlMenu_ProSklady)
            {
                StartAktivityNasledujici(typeof(Prodej_Davky), null);
            }
            else if (id == Resource.Id.nav_Ciselnik_proSklady)
            {
                CiselnikSklady();
            }
        ...
        {
            MenuInflater.Inflate(Resource.Menu.Prodej_Sklady_Search, menu);

            var item = menu.FindItem(Resource.Id.ProdejSklady_search);
        ...
        private async void CiselnikSklady()
        {
            try
            {

                Ciselniky_Helper _Helper = new Ciselniky_Helper();
                bool state = await _Helper.SynchronizeCiselniky(CiselnikServiceOperations.Operation.Sklady, this, Konfigurace_Singleton.Instance.Prodej.SkladID);
