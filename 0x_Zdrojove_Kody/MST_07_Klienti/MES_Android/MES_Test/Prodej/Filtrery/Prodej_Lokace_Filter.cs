...
{
    public class Prodej_Lokace_Filter : Filter
    {
        private readonly Prodej_Lokace_Item_Adapter _adapter;
        public Prodej_Lokace_Filter(Prodej_Lokace_Item_Adapter adapter)
        {
            _adapter = adapter;
        }
        ...
        protected override FilterResults PerformFiltering(ICharSequence constraint)
        {
            try
            {
                ...
                using (constraint)
                {
                    ...
                    // 2
                    var results = new Prodej_Lokace_Seznam(new Fask.SQLiteDBs.DataSets.Lokace.CZMST094DataTable());

                    // 3
                    if (_adapter.PolozkaSeznam_Original == null)
                    {
                        _adapter.PolozkaSeznam_Original = new Prodej_Lokace_Seznam((Fask.SQLiteDBs.DataSets.Lokace.CZMST094DataTable)_adapter.PolozkaSeznam.mItems.Copy());
                    ...
                    if (constraint == null || string.IsNullOrEmpty(constraint.ToString()))
                    {
                        ...
                        results = null;
                        results = new Prodej_Lokace_Seznam((Fask.SQLiteDBs.DataSets.Lokace.CZMST094DataTable)_adapter.PolozkaSeznam_Original.mItems.Copy());
                    ...
                    if (_adapter.PolozkaSeznam_Original != null && _adapter.PolozkaSeznam_Original.mItems.Count > 0)
                    {
                        ...
                        {
                            Fask.SQLiteDBs.DataSets.Lokace.CZMST094Row item = _adapter.PolozkaSeznam_Original.mItems[i];


                            if (item.LOCNCODE.Contains(constraint.ToString().Trim()))
                            {
                                var row = results.mItems.NewCZMST094Row();
                                ...
                                results.mItems.AddCZMST094Row(row);
                            }
        ...
        protected override void PublishResults(ICharSequence constraint, FilterResults results)
        {
            try
            {
                ...
                {
                    if (values is Prodej_Lokace_Seznam)
                    {
                        var a = (Prodej_Lokace_Seznam)values;
                        ...
                        foreach (var itemo in a.mItems.Rows)
                        {
                            ...
                            Fask.SQLiteDBs.DataSets.Lokace.CZMST094Row item = (Fask.SQLiteDBs.DataSets.Lokace.CZMST094Row)itemo;

                            var row = _adapter.PolozkaSeznam.mItems.NewCZMST094Row();
                            foreach (System.Data.DataColumn col in _adapter.PolozkaSeznam_Original.mItems.Columns)
                            {
                                row[col.ColumnName] = item[col.ColumnName] == null ? DBNull.Value : item[col.ColumnName];
                            }

                            _adapter.PolozkaSeznam.mItems.AddCZMST094Row(row);
                        }
