...
{
    public class Prodej_SberDat_Filter : Filter
    {
        private readonly Prodej_SberDat_Item_Adapter _adapter;
        public Prodej_SberDat_Filter(Prodej_SberDat_Item_Adapter adapter)
        {
            _adapter = adapter;
        }
        ...
        protected override FilterResults PerformFiltering(ICharSequence constraint)
        {
            try
            {
                ...
                using (constraint)
                {
                    ...
                    // 2
                    var results = new Prodej_SberDat_Seznam(new Fask.SQLiteDBs.DataSets.Prodej.CZMST_DIDataTable());

                    // 3
                    if (_adapter.PolozkaSeznam_Original == null)
                    {
                        _adapter.PolozkaSeznam_Original = new Prodej_SberDat_Seznam((Fask.SQLiteDBs.DataSets.Prodej.CZMST_DIDataTable)_adapter.PolozkaSeznam.mItems.Copy());
                    ...
                    if (constraint == null || string.IsNullOrEmpty(constraint.ToString()))
                    {
                        ...
                        results = null;
                        results = new Prodej_SberDat_Seznam((Fask.SQLiteDBs.DataSets.Prodej.CZMST_DIDataTable)_adapter.PolozkaSeznam_Original.mItems.Copy());
                    ...
                    if (_adapter.PolozkaSeznam_Original != null && _adapter.PolozkaSeznam_Original.mItems.Count > 0)
                    {
                        ...
                        {
                            Fask.SQLiteDBs.DataSets.Prodej.CZMST_DIRow item = _adapter.PolozkaSeznam_Original.mItems[i];


                            if (item.ITEMDESC.Contains(constraint.ToString().Trim()))
                            {
                                var row = results.mItems.NewCZMST_DIRow();
                                ...
                                results.mItems.AddCZMST_DIRow(row);
                            }
        ...
        protected override void PublishResults(ICharSequence constraint, FilterResults results)
        {
            try
            {
                ...
                {
                    if (values is Prodej_SberDat_Seznam)
                    {
                        var a = (Prodej_SberDat_Seznam)values;
                        ...
                        foreach (var itemo in a.mItems.Rows)
                        {
                            ...
                            Fask.SQLiteDBs.DataSets.Prodej.CZMST_DIRow item = (Fask.SQLiteDBs.DataSets.Prodej.CZMST_DIRow)itemo;

                            var row = _adapter.PolozkaSeznam.mItems.NewCZMST_DIRow();
                            foreach (System.Data.DataColumn col in _adapter.PolozkaSeznam_Original.mItems.Columns)
                            {
                                row[col.ColumnName] = item[col.ColumnName] == null ? DBNull.Value : item[col.ColumnName];
                            }

                            _adapter.PolozkaSeznam.mItems.AddCZMST_DIRow(row);
                        }
