...
{
    public class Prodej_VyberMaterial_Filter : Filter
    {
        private readonly Prodej_VyberMaterial_Item_Adapter _adapter;
        public Prodej_VyberMaterial_Filter(Prodej_VyberMaterial_Item_Adapter adapter)
        {
            _adapter = adapter;
        }
        ...
        protected override FilterResults PerformFiltering(ICharSequence constraint)
        {
            try
            {
                ...
                using (constraint)
                {
                    ...
                    // 2
                    var results = new Prodej_VyberMaterial_Seznam(new ProdejService.Location.CZMST_SkladLokace_StavDataTable());

                    // 3
                    if (_adapter.PolozkaSeznam_Original == null)
                    {
                        _adapter.PolozkaSeznam_Original = new Prodej_VyberMaterial_Seznam((ProdejService.Location.CZMST_SkladLokace_StavDataTable)_adapter.PolozkaSeznam.mItems.Copy());
                    ...
                    if (constraint == null || string.IsNullOrEmpty(constraint.ToString()))
                    {
                        ...
                        results = null;
                        results = new Prodej_VyberMaterial_Seznam((ProdejService.Location.CZMST_SkladLokace_StavDataTable)_adapter.PolozkaSeznam_Original.mItems.Copy());
                    ...
                    if (_adapter.PolozkaSeznam_Original != null && _adapter.PolozkaSeznam_Original.mItems.Count > 0)
                    {
                        ...
                        {
                            ProdejService.Location.CZMST_SkladLokace_StavRow item = _adapter.PolozkaSeznam_Original.mItems[i];


                            if (item.SERLTNUM.Contains(constraint.ToString().Trim()) || item.LOCNCODE.Contains(constraint.ToString().Trim()))
                            {
                                var row = results.mItems.NewCZMST_SkladLokace_StavRow();
                                ...
                                results.mItems.AddCZMST_SkladLokace_StavRow(row);
                            }
        ...
        protected override void PublishResults(ICharSequence constraint, FilterResults results)
        {
            try
            {
                ...
                {
                    if (values is Prodej_VyberMaterial_Seznam)
                    {
                        var a = (Prodej_VyberMaterial_Seznam)values;
                        ...
                        foreach (var itemo in a.mItems.Rows)
                        {
                            ...
                            ProdejService.Location.CZMST_SkladLokace_StavRow item = (ProdejService.Location.CZMST_SkladLokace_StavRow)itemo;

                            var row = _adapter.PolozkaSeznam.mItems.NewCZMST_SkladLokace_StavRow();
                            foreach (System.Data.DataColumn col in _adapter.PolozkaSeznam_Original.mItems.Columns)
                            {
                                row[col.ColumnName] = item[col.ColumnName] == null ? DBNull.Value : item[col.ColumnName];
                            }

                            _adapter.PolozkaSeznam.mItems.AddCZMST_SkladLokace_StavRow(row);
                        }
