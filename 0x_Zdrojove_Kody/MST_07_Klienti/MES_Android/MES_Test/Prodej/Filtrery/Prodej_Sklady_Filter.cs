...
{
    public class Prodej_Sklady_Filter : Filter
    {
        private readonly Prodej_Sklady_Item_Adapter _adapter;
        public Prodej_Sklady_Filter(Prodej_Sklady_Item_Adapter adapter)
        {
            _adapter = adapter;
        }
        ...
        protected override FilterResults PerformFiltering(ICharSequence constraint)
        {
            try
            {
                ...
                using (constraint)
                {
                    ...
                    // 2
                    var results = new Prodej_Sklady_Seznam(new Fask.SQLiteDBs.DataSets.Sklady.CZMST093DataTable());

                    // 3
                    if (_adapter.PolozkaSeznam_Original == null)
                    {
                        _adapter.PolozkaSeznam_Original = new Prodej_Sklady_Seznam((Fask.SQLiteDBs.DataSets.Sklady.CZMST093DataTable)_adapter.PolozkaSeznam.mItems.Copy());
                    ...
                    if (constraint == null || string.IsNullOrEmpty(constraint.ToString()))
                    {
                        ...
                        results = null;
                        results = new Prodej_Sklady_Seznam((Fask.SQLiteDBs.DataSets.Sklady.CZMST093DataTable)_adapter.PolozkaSeznam_Original.mItems.Copy());
                    ...
                    if (_adapter.PolozkaSeznam_Original != null && _adapter.PolozkaSeznam_Original.mItems.Count > 0)
                    {
                        ...
                        {
                            Fask.SQLiteDBs.DataSets.Sklady.CZMST093Row item = _adapter.PolozkaSeznam_Original.mItems[i];


                            if (item.skl_desc.Contains(constraint.ToString().Trim()))
                            {
                                var row = results.mItems.NewCZMST093Row();
                                ...
                                results.mItems.AddCZMST093Row(row);
                            }
        ...
        protected override void PublishResults(ICharSequence constraint, FilterResults results)
        {
            try
            {
                ...
                {
                    if (values is Prodej_Sklady_Seznam)
                    {
                        var a = (Prodej_Sklady_Seznam)values;
                        ...
                        foreach (var itemo in a.mItems.Rows)
                        {
                            ...
                            Fask.SQLiteDBs.DataSets.Sklady.CZMST093Row item = (Fask.SQLiteDBs.DataSets.Sklady.CZMST093Row)itemo;

                            var row = _adapter.PolozkaSeznam.mItems.NewCZMST093Row();
                            foreach (System.Data.DataColumn col in _adapter.PolozkaSeznam_Original.mItems.Columns)
                            {
                                row[col.ColumnName] = item[col.ColumnName] == null ? DBNull.Value : item[col.ColumnName];
                            }

                            _adapter.PolozkaSeznam.mItems.AddCZMST093Row(row);
                        }
