...
{
    public class OperaceVyber_Filter : Filter
    {
        private readonly OperaceVyber_Item_Adapter _adapter;
        public OperaceVyber_Filter(OperaceVyber_Item_Adapter adapter)
        {
            _adapter = adapter;
        }
        ...
        protected override FilterResults PerformFiltering(ICharSequence constraint)
        {
            try
            {
                ...
                using (constraint)
                {
                    ...
                    // 2
                    var results = new OperaceVyber_Seznam(new Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPDataTable());

                    // 3
                    if (_adapter.PolozkaSeznam_Original == null)
                    {
                        _adapter.PolozkaSeznam_Original = new OperaceVyber_Seznam((Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPDataTable)_adapter.PolozkaSeznam.mItems.Copy());
                    ...
                    if (constraint == null || string.IsNullOrEmpty(constraint.ToString()))
                    {
                        ...
                        results = null;
                        results = new OperaceVyber_Seznam((Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPDataTable)_adapter.PolozkaSeznam_Original.mItems.Copy());
                    ...
                    if (_adapter.PolozkaSeznam_Original != null && _adapter.PolozkaSeznam_Original.mItems.Count > 0)
                    {
                        ...
                        {
                            Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPRow item = _adapter.PolozkaSeznam_Original.mItems[i];


                            if (item.SOPNUMBE.Contains(constraint.ToString().Trim()))
                            {
                                var row = results.mItems.NewCZPRO_VPPRow();
                                ...
                                results.mItems.AddCZPRO_VPPRow(row);
                            }
        ...
        protected override void PublishResults(ICharSequence constraint, FilterResults results)
        {
            try
            {
                ...
                {
                    if (values is OperaceVyber_Seznam)
                    {
                        var a = (OperaceVyber_Seznam)values;
                        ...
                        foreach (var itemo in a.mItems.Rows)
                        {
                            ...
                            Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPRow item = (Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPRow)itemo;

                            var row = _adapter.PolozkaSeznam.mItems.NewCZPRO_VPPRow();
                            foreach (System.Data.DataColumn col in _adapter.PolozkaSeznam_Original.mItems.Columns)
                            {
                                row[col.ColumnName] = item[col.ColumnName] == null ? DBNull.Value : item[col.ColumnName];
                            }

                            _adapter.PolozkaSeznam.mItems.AddCZPRO_VPPRow(row);
                        }
