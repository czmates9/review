...
namespace Fask.PrinterProviderWebService
{
    ...
    {
        private MES_Android._WebReferences_Globals.TiskServiceSession printServerTiskService = null;
        private MES_Android._WebReferences_Globals.TestServiceSession printServerTestService = null;
		private MES_Android._WebReferences_Globals.TiskTestServiceSession printServerTiskTestService = null;
        ...
        public string ConfigFilename
        {
            get
            {
                 if (String.IsNullOrEmpty(_ConfigFilename))
                    return Path.Combine(MES_Android.Classes.DataInfo_Static.PathDir, "Fask.PrinterProviderWebService.xml");
                else
                    return Path.Combine(MES_Android.Classes.DataInfo_Static.PathDir, _ConfigFilename);
            ...
            if (configuration == null)
            {
                ...
                if (configuration.Params.Count == 0)
                {
                    configuration.Params.AddParamsRow(
                        @"http://192.168.1.101/MST_Win_Kom_Server_7_Dasenka",
                        "Alias oznaceni tiskarny",
                        10000,
                        "Nazev Tiskarny",
                        false
                        );
     ...
     //   }
        ...
        public void InitializePrinter()
        {
            ...
            if (!Params.Address.StartsWith("http")) // tohle by mnelo ošetřit stav http:// i https://
                surl += "http://";
            ...
            if (printServerTiskService == null)
                printServerTiskService = new MES_Android._WebReferences_Globals.TiskServiceSession();
            ...
            if (printServerTestService == null)
                printServerTestService = new MES_Android._WebReferences_Globals.TestServiceSession();

            printServerTestService.Url = surl + "Test.asmx";
            printServerTestService.Timeout = Params.Timeout;
            printServerTestService.UpdateWebServiceCredentials();

            if (printServerTiskTestService == null)
                printServerTiskTestService = new MES_Android._WebReferences_Globals.TiskTestServiceSession();

			printServerTiskTestService.Url = surl + "TiskTest.asmx";
			printServerTiskTestService.Timeout = Params.Timeout;
            printServerTiskTestService.UpdateWebServiceCredentials();
        ...
        private MES_Android.PrintServerTiskService.TiskParams prepareTiskParams()
        {
            MES_Android.PrintServerTiskService.TiskParams tiskParams = new MES_Android.PrintServerTiskService.TiskParams();
            tiskParams.CONFIG_NAME = Params.PrinterName; // to je vse ???
            return tiskParams;
        }

        private MES_Android.PrintServerTiskService.DSValues prepareTiskValues(Dictionary<string, string> data)
        {
            MES_Android.PrintServerTiskService.DSValues tiskValues = new MES_Android.PrintServerTiskService.DSValues();
        ...
        {
            List<MES_Android.PrintServerTiskService.DSValues> dataRows = new List<MES_Android.PrintServerTiskService.DSValues>();
