...
{
    public class GS1_StriktniNorma : BaseCode
    {
        ...
        private  string[] aiis;
        private  int minLengthOfAI = 1;
        private  int maxLengthOfAI = 4;
        private  char groupSeparator = (char)29;
        private  string ean128StartCode = "]C1";
        private  bool hasCheckSum = true;

        public  bool HasCheckSum
        {
            get { return this.hasCheckSum; }
            set { this.hasCheckSum = value; }
        }

        public  char GroupSeparator
        {
            get { return this.groupSeparator; }
            set { this.groupSeparator = value; }
        }

        public  string EAN128StartCode
        {
            get { return this.ean128StartCode; }
            set { this.ean128StartCode = value; }
        }

        public GS1_StriktniNorma(List<string> codes) : base(codes)
        {
       ...
       public  Dictionary<AII, string> Result = new Dictionary<AII, string>();
        ...
        public static GS1_StriktniNorma  Parse(List<string> dataList)
        {
            if (dataList.Count == 0)
                return null;

            GS1_StriktniNorma gS1_StriktniNorma = new GS1_StriktniNorma(dataList);
            ...
            foreach (var item in dataList)
            {
                ...
                if (data.StartsWith(gS1_StriktniNorma.EAN128StartCode))
                    data = data.Substring(gS1_StriktniNorma.EAN128StartCode.Length);
                // cut off the check sum
                if (gS1_StriktniNorma.HasCheckSum)
                ...
                while (index < data.Length)
                {
                    ...
                    var ai = gS1_StriktniNorma.GetAI(data, ref index);
                    if (ai == null)
                    {
                        //if (throwException)
                        //    throw new InvalidOperationException("AI not found");
                        return gS1_StriktniNorma;
                    }
                    // get the data to the current AI
                    string code = gS1_StriktniNorma.GetCode(data, ai, ref index);
                    gS1_StriktniNorma.Result[ai] = code;
                } 
            }
