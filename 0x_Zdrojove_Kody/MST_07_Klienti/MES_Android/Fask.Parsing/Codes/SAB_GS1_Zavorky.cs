...
namespace Fask.Parsing.Codes
{
    ...
    /// </summary>
    public class SAB_GS1_Zavorky : BaseCode
		...
		, Interfaces.IPocetNasnimanychVariant
    {
        ...
        public SAB_GS1_Zavorky(List<string> codes)
            : base(codes)
        {
        }

        public static SAB_GS1_Zavorky Parse(string data)
        {
        ...
        //public static SAB_GS1_Zavorky Parse(List<string> list_dataFull)
        //{
        //    try
        //    {
        ...
        //        SAB_GS1_Zavorky gs1 = new SAB_GS1_Zavorky(list_dataFull);
        ...
        public static SAB_GS1_Zavorky Parse(List<string> list_dataFull)
        {
            try
            {
                ...
                foreach (var dataFull in list_dataFull)
                {
                    string dataTrim = dataFull.Trim();

					if (dataTrim[0] != '(')
						return null;

                    List<string> dataSplit = dataTrim.Split(new char[] { '(' }).ToList();

                    foreach (string data in dataSplit)
                    {
                        if ((data.Length > 0) && (data.Substring(0, 2) == ai_gtin))
                        {
                            gs1.GTIN = data.Substring(3);
							gs1._pocetNalezenychVariant++;
                        }
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_lot))
                        {
                            string lot = data.Substring(3);
                            if (lot.Length > 20)
                                throw new Exception(String.Format("GS1 LOT an..20 => {0}", lot.Length));
                            gs1.LOT = lot;
							gs1._pocetNalezenychVariant++;
                        }
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_production_date)) // (11) datum vyroby
                        {
                            string productiondate_rrmmdd = string.Empty;
                            try
                            {
                                productiondate_rrmmdd = data.Substring(3);
                                gs1.PRODUCTIONDATE = Common.Dates.Date_RRMMDD(productiondate_rrmmdd);
								gs1._pocetNalezenychVariant++;
                            }
                            catch (Exception)
                            {
                                throw new Exception(String.Format("GS1 PROD DATE is not datetime => {0}", productiondate_rrmmdd));
                            }
                        }
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_expiration)) // (17) Pouzitelnost do (expirace)
                        {
                            string rrmmdd = string.Empty;
                            try
                            {
                                rrmmdd = data.Substring(3);
                                gs1.EXPIRATION = Common.Dates.Date_RRMMDD(rrmmdd);
								gs1._pocetNalezenychVariant++;
                            }
                            catch (Exception)
                            {
                                throw new Exception(String.Format("GS1 EXP is not datetime => {0}", rrmmdd));
                            }
                        }
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_variantaProduktu))
                        {
                            string variantaProduktu = data.Substring(3);
                            gs1.VariantaProduktu = int.Parse(variantaProduktu);
							gs1._pocetNalezenychVariant++;
                        }
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_serialnumber))
                        {
                            string serialnumber = data.Substring(3);
                            if (serialnumber.Length > 20)
                                throw new Exception(String.Format("GS1 SN an..20 => {0}", serialnumber.Length));
                            gs1.SerialNumber = serialnumber;
							gs1._pocetNalezenychVariant++;
                        }
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_variablecount)) // (30) Variable count
                        {
                            string variablecount = data.Substring(3);
                            if (variablecount.Length > 8)
                                throw new Exception(String.Format("GS1 Variable Count n..8 => Length:{0}", variablecount.Length));
                            if (variablecount.ToCharArray().Any(x => !char.IsNumber(x)))
                                throw new Exception(String.Format("GS1 Variable Count n..8 => NotNumber"));
                        ...
                        else if ((data.Length > 0) && (data.Substring(0, 2) == ai_count)) // (37) count
                        {
                            string count = data.Substring(3);
                            if (count.Length > 8)
                                throw new Exception(String.Format("GS1 Count n..8 => Length:{0}", count.Length));
                            if (count.ToCharArray().Any(x => !char.IsNumber(x)))
                                throw new Exception(String.Format("GS1 Count n..8 => NotNumber"));
