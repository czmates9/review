...
namespace Fask.Parsing.Codes
{
    ...
    /// </summary>
    public class SAB_GS1_BELDICO : BaseCode
		...
		, Interfaces.ICodeInterniFiremni93
    {
		...
		public SAB_GS1_BELDICO(List<string> codes)
            : base(codes)
        {
        }

		public static SAB_GS1_BELDICO Parse(string data)
        {
            return Parse(new List<string>() { data });
        }

		public static SAB_GS1_BELDICO Parse(List<string> list_dataFull)
        {
            try
            {
				...
				SAB_GS1_BELDICO gs1 = new SAB_GS1_BELDICO(list_dataFull);
                    ...
                    foreach (string dataSplitItem in dataSplit)
                    {
                        ...
                        while ((data.Length > 2) && (datalenghtchanged))
                        {
                            ...
                            if ((data.Length > 0) && (data.Substring(0, 2) == ai_lot))
                            {
                                ...
                                if (lot.Length > 20)
									throw new Exception(String.Format("SAB_GS1_BELDICO LOT an..20 => {0}", lot.Length));
                                gs1.LOT = lot;
                            ...
                            {
                                string rrrrmmdd = data.Substring(2, 6);
								rrrrmmdd += "28";

                                gs1.EXPIRATION = Common.Dates.Date_RRRRMM(rrrrmmdd);
                            ...
                            if ((data.Length > 0) && (data.Substring(0, 2) == ai_serialnumber))
                            {
                                ...
                                if (serialnumber.Length > 20)
									throw new Exception(String.Format("SAB_GS1_BELDICO SN an..20 => {0}", serialnumber.Length));
                            ...
                            if ((data.Length > 0) && (data.Substring(0, 2) == ai_variablecount)) // (30) Variable count
                            {
                                ...
                                if (variablecount.Length > 8)
									throw new Exception(String.Format("SAB_GS1_BELDICO Variable Count n..8 => Length:{0}", variablecount.Length));
                                if (variablecount.ToCharArray().Any(x => !char.IsNumber(x)))
									throw new Exception(String.Format("SAB_GS1_BELDICO Variable Count n..8 => NotNumber"));
                            ...
                            if ((data.Length > 0) && (data.Substring(0, 2) == ai_count)) // (37) count
                            {
                                ...
                                if (count.Length > 8)
									throw new Exception(String.Format("SAB_GS1_BELDICO Count n..8 => Length:{0}", count.Length));
                                if (count.ToCharArray().Any(x => !char.IsNumber(x)))
									throw new Exception(String.Format("SAB_GS1_BELDICO Count n..8 => NotNumber"));
							...
							if ((data.Length > 0) && (data.Substring(0, 2) == ai_interniParam93)) // (93) Uživatelsky parametr
							{
								...
								if (param93.Length > 30)
									throw new Exception(String.Format("SAB_GS1_BELDICO interní použití, firemní informace an..30 => {0}", param93.Length));
								gs1.Param93 = param93;
							...
							if ((data.Length > 0) && (data.Substring(0, 3) == ai_DoplnInfoProdukt240)) // (240) DOPLŇKOVÁ IDENTIFIKACE PRODUKTU
							{
								...
								if (dopPar240.Length > 30)
									throw new Exception(String.Format("SAB_GS1_BELDICO doplňková identifikace produktu an..30 => {0}", dopPar240.Length));
