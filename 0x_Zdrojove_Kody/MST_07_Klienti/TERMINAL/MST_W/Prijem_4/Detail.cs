...
namespace Fask.MST_W.Prijem_4
{
    public partial class Detail : System.Windows.Forms.Form
    {
        ...
        private string sklid = string.Empty;
        private DataSet data = null;
        private delegate void DelegateNoParam();
        private string message = string.Empty;

        public Detail(string ponumber, string sklid)
        {
            InitializeComponent();
        ...
        private void Detail_Load(object sender, EventArgs e)
        {
            ...
            try
            {                
                //prijemservice.BeginDetail(ponumber.Trim(), sklid.Trim(), new AsyncCallback(PrijemkaDetail), null);
                Prijem_4.PrijemMain.prijemInstance.globalObject.service_prijem.BeginDetail(ponumber.Trim(), sklid.Trim(), new AsyncCallback(PrijemkaDetail), null);
            }
            catch (Exception ex)
            {
        ...
        private void PrijemkaDetail(IAsyncResult ares)
        {
            try
            {
                data = Prijem_4.PrijemMain.prijemInstance.globalObject.service_prijem.EndDetail(ares);
            }
            catch (Exception ex)
            {
                message = ex.Message;
