...
namespace Fask.MST_W_Server.BL
{
    public class OstatniBL
    {
        ...
        public OstatniBL(Fask.Server.Interfaces.IWebModule provider)
        {
        ...
        public void Initialize(Func<string, string> map_path_function)
        {
            ...
            try
            {
                ...
                string providerAssemblyPath = string.Empty;// Konfigurace.Classes.Globals_Konfig_WebConfig.Konfigurace.Providers[0].Provider_Vydej;
        ...
        {
            StatusObject so = StatusObject.Create(MyPath.Path.StatusObjectDirectory, StatusObject.Operations.OstatniGenerateDavka, sopnumbe, skl_id);
            if (so.Exists)
            {
        ...
        {
            TracId tracid = new TracId(null, null, null, "Ostatni.GenerateDavka:" + (sopnumbe ?? string.Empty) + "," + (skl_id ?? string.Empty));
            try
            {
                #region trace
                ...
                try
                {
                    if ((provider != null) && (provider is Fask.Server.Interfaces.Ostatni.IOstatni))
                    {
                        ...
                        StatusInfo si = (provider as Fask.Server.Interfaces.Ostatni.IOstatni).Ostatni_GenerateDavka(objednavka, sklad);
                        return si.ID;
                    }
                }
                catch (Exception ex)
                ...
                string msg = "Provider v OstatniBL.cs > 'GenerateDavka(string sopnumbe, string skl_id)' nenastaven.";
