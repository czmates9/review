...
namespace Fask.Module.Ingres.SAD.Routines
{
	/// <summary>
    ...
    class AfterProcessAction
    {
        ...
        {
            IngresConnection adpaconnection = null;

            try 
            {
                adpaconnection = new IngresConnection(connstring);
                IngresCommand adpacommand = new IngresCommand(aDP_Action);
                adpacommand.CommandType = CommandType.StoredProcedure;

                adpacommand.CommandTimeout = AfterDataProcessed_Action_CommandTimeout;

                adpacommand.Parameters.Add(new IngresParameter(aDP_Action_P1, DbType.String, 20));
                adpacommand.Parameters.Add(new IngresParameter(aDP_Action_P2, DbType.String, 20));
