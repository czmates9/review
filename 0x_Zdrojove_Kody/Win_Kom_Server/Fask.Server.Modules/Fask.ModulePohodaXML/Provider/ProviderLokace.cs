    ...
    public partial class Provider : Fask.Server.Interfaces.Lokace.ILokace
    {
        ...
        public Fask.Server.Interfaces.DataSets.Location Lokace_ShowMaterial(string itemnmbr, string serltnum, string skl_id, uint? NumberOfRecords, bool ShowEmpty)
        {
            ...
            try
            {
                ...
                command.CommandText = "SELECT " + (NumberOfRecords.HasValue ? ("TOP " + NumberOfRecords.Value) : string.Empty) + "* FROM " + TABLE_CZMST_SKLADLOKACE_STAV + " WHERE " +
        ...
        private void ProcessVydej(LokacePohyb record, SqlCommand Command, SqlConnection Connection, SqlTransaction Transaction, SqlDataAdapter Adapter)
        {
            ...
            if (newQuantity < 0)
            {
                ...
                if (Globals_V1.Konfigurace.LokMech[0].Vydej_PovolZapornyStav)
                {
        ...
        public bool Lokace_VariantySortimentExists(string itemnmbr, string skl_id, string locncode)
        {
            ...
            try
            {
                ...
                // overovani cilove lokace ... pouze kontrola existence lokace
                //Kontrola, jestli je lokace a sklad v DB
                command.CommandText = "SELECT COUNT(*) FROM " + TABLE_CZMST_SkladLokace_LokaceVariantySortiment + " WHERE itemnmbr=@itemnmbr AND LOCNCODE=@locncode AND SKL_ID=@skl_id";
		...
		private void ProcessPrijem(LokacePohyb record, SqlCommand command, SqlConnection connection, SqlTransaction transaction, SqlDataAdapter adapter)
		{
			...
			else
			{
				...
				command.CommandText =
					"INSERT INTO " + TABLE_CZMST_SKLADLOKACE_STAV +
		...
		private void Lokace_MoveItem(LokacePohyb record, SqlCommand command, SqlConnection connection, SqlTransaction transaction, SqlDataAdapter adapter)
		{
			...
			if (record.POHYB_TYPE == TypeOfRecord.D)// && Source)
			{
                ...
                string ITEMNMBR_Cil = record.ITEMNMBR;
                string ITEMDESC_Cil = string.Empty;

                if (!string.IsNullOrEmpty(record.SKL_ID_SRC) && (record.SKL_ID_DST.Trim() != record.SKL_ID_SRC.Trim()))
                {
                    Database.Pohoda.Get_MapingLokMechID(record.ITEMNMBR, record.SKL_ID_DST, record.SKL_ID_SRC, out ITEMNMBR_Cil, out ITEMDESC_Cil);
                    record.ITEMNMBR = ITEMNMBR_Cil;
                    record.ITEMDESC = ITEMDESC_Cil;
