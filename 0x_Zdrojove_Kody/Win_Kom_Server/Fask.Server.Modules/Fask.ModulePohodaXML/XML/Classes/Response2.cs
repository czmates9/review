        ...
        private void Load()
        {
            ...
            if (xmlnodesresponsepackitems.Count > 0) //neco tam je ... 
            {
                ...
                else
                {
                   ...
                   //Logging.Log.writeErrorLog("Provider.PohodaXML", "Response.Load()", statusLog);
					Fask.Logging.ExceptionHandler2.Handle(Fask.Logging.LogLevel.Error, this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, statusLog);
                    if (Status == "Importovaný záznam již existuje.")
                    {                        
                        Status = "OK2";
                        //Logging.Log.writeErrorLog("Provider.PohodaXML", "Response.Load()", "Status změněn na: " + Status);
						Fask.Logging.ExceptionHandler2.Handle(Fask.Logging.LogLevel.Error, this.GetType().ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, "Status změněn na: " + Status);
                    }
                }
            }
            else
