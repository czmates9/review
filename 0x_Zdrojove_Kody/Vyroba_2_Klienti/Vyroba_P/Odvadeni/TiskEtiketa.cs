...
namespace Fask.Vyroba_P.Odvadeni
{
    public class TiskEtiketa
    {
        public static bool TiskPriprava(Form owner, Fask.SQLiteDBs.DataSets.Vyroba.LoginsRow idpracovnik, Fask.SQLiteDBs.DataSets.Vyroba.MachinesRow idmachine, Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPHRow rowvph, Fask.SQLiteDBs.DataSets.Vyroba.CZPRO_VPPRow rowvpp, Fask.SQLiteDBs.DataSets.Vyroba.ProductionRow productionRow)
        {
            ...
            using (Tisk.FormSablonaVyber ftisksablona = new Fask.Vyroba_P.Tisk.FormSablonaVyber())
            {
            ...
            try
            {
                ...
                using (Tisk.FormDictionaryModify frmdictmod = new Fask.Vyroba_P.Tisk.FormDictionaryModify(dictParamsSablona, dictParams))
                {
            ...
            #region Tisk
            if (Vyroba_P.Tisk.Tiskarna.Tiskni(sablona.PrinterName, sablona.Soubor, dictParamsSablona, rowvph.SOPNUMBE.Trim() + " : " + sablona.Nazev.Trim()))
            { // vytisteno
                ...
                Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Production_PRD.Insert_UserEvents(Settings.LastProductionUserID, idmachine == null ? string.Empty : idmachine.id, DateTime.Now, Settings.UEventTiskEtikety, idpracovnik.id, Settings.TerminalID, rowvph.SOPNUMBE, Guid.NewGuid());
            }
            else
            {
