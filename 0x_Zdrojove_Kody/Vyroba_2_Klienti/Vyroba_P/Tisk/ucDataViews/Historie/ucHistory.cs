...
namespace Fask.Vyroba_P.ucDataViews.Historie
{
    public partial class ucHistory : UserControl
    {
        ...
        private void timerUpdateDataThreadStart()
        {
            ...
            try
            {
                ...
                if (Settings.ModulPrehledOdvoduPouzitDataZeServeru)
                {
                    try
                    {
                        WebServiceVyroba.FiltersHistory filters = new Fask.Vyroba_P.WebServiceVyroba.FiltersHistory();
                        ...
                        dsVweb = Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.vyrobaServis.Production_HistoryFilter(filters, Settings.ModulPrehledPocetHodinHistorieServer);
                    }
                ...
                Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__ProductionHist_PRD.FillProduction_HistoryFilter_Production(
                ...
                if (filtrNedokonceneZakazky)
                {
                    ...
                    var dtProdHist2 = Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__ProductionHist_PRD.GetProduction_HistoryFilter2_Production( Settings.ModulPrehledPocetHodinHistorie, filtrDatumDo, filtrDatumOd );
                    dataCE.ProductionHist.Merge(dtProdHist2);
                }
                // naplnění tabulky Logins
                Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.Fill_Logins(dataCE.Logins);

                // naplneni tabulkzy Machines
                Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.Fill_Machines(dataCE.Machines);

                // naplnění názvu operace
                Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.Fill_Operations(dataCE.Operations);

                // naplneni nazvu korekci
                Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__Vyroba_PRD.Fill_Corrects(dataCE.Corrects);
        ...
        private void buttonStroj_Click(object sender, EventArgs e)
        {
            try
            {
                using (Odvadeni.FormIDMachine fidstroj = new Fask.Vyroba_P.Odvadeni.FormIDMachine())
                {
            ...
            try
            {
                ...
                if (!DesignMode)
                {
                    try
                    {
                        ...
                        //taprohist.Connection.ConnectionString = "Data source=" + Path.Combine(MySystem.MyPath.DataDirectory, Constants.ProductionHist + Constants.PRD);
                        int rowsDeleted = Fask.Vyroba_P.Forms.FormMain.Instance_FormMain.globalObject.controller_Vyroba__ProductionHist_PRD.DeleteOlderThanDateeve_Production(DateTime.Now.AddDays(-Settings.ModulPrehledOdvoduMaxDaysInHistory));
        ...
        private void buttonFiltrCas_Click(object sender, EventArgs e)
        {
            try
            {
                using (Odvadeni.FormDatumOdDo fdatum = new Fask.Vyroba_P.Odvadeni.FormDatumOdDo())
                {
        ...
        private void buttonFiltrZakazka_Click(object sender, EventArgs e)
        {
            ...
            {
                // zadani zakazky
                using (OdvadeniNadop.FormVyberZakazka fzakazka = new Fask.Vyroba_P.OdvadeniNadop.FormVyberZakazka())
                {
...
//#endif
        ...
        private void buttonFiltrPrehled_Click(object sender, EventArgs e)
        {
            try
            {
                using (ucDataViews.Historie.dlgFilterSelector dlgfs = new Fask.Vyroba_P.ucDataViews.Historie.dlgFilterSelector())
                {
                    ...
                    if (dlgfs.Stroj == Stroj.Nastaveny)
                    {
                        ...
                        {
                            using(Odvadeni.FormIDMachine fidmachine = new Fask.Vyroba_P.Odvadeni.FormIDMachine())
                            {
        ...
        private void buttonStatistikaOdvadeni_Click(object sender, EventArgs e)
        {
            try
            {
                using (Fask.Vyroba_P.Forms.FormPrehledFiltr ff = new Fask.Vyroba_P.Forms.FormPrehledFiltr())
                {
